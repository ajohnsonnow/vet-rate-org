...
6. Document the _why_ (ADRs), not the _what_.
...
Pass 1 minimum: _"Review your answer. What's unclear, wrong, or incomplete? Provide an improved version."_
...
Default: zero comments. Add only when _why_ is non-obvious. Never explain _what_ well-named code already does.
...
A push of a feature/fix branch is a request to land it: once verified, merge to `main`, then delete the branch locally and on the remote. Standing authorization for that merge + delete — not for skipping gates, force-pushing, or merging unverified work.
