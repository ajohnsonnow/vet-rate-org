...
- **Run procedure**: `npm run dev` → `http://localhost:5173`; import the pre-built packet `E:\Williams_C-FIle\vet-rate-packet-williams-robert-2026-03-21.json` (0.72 MB, schema v2.0, 38 importedFiles, 16 claims, 11 ratings) via **My Packet → Import**. This single import seeds nearly every downstream tool.
- **Canonical correctness check**: Williams's real **combined rating = 80%** — PTSD 50 (9411), lumbosacral/DDD 20 (5242), L femoral radiculopathy 20 (8520), R femoral radiculopathy 10 (8620), L hip 10 + R hip 10 (5252, bilateral pair), L knee ITB 10 (5260), bilateral pes planus 10 (5276), tinnitus 10 (6260), plus several 0% conditions. **The app MUST compute 80%.**
...
2. **PII/PHI is real.** `E:\Williams_C-FIle` contains a real veteran's SSN, file number, DOB, and diagnoses.
   ...
   - For any committed test that needs packet shape, generate a **redacted synthetic fixture** (real schema, fake name/SSN/file-number, Williams's _condition+rating_ set) under `tests/fixtures/`.
...
5. Create `tests/fixtures/redacted-packet.json` (synthetic PII, real schema + Williams's condition/rating set) and add `tests/e2e/cfile-packet.spec.ts` that imports it and asserts claims load + persist. **No real PII in the committed fixture/test.**
...
### Sprint 2 — Calculation correctness vs the Williams 80% ground truth (FIX dual-calc divergence)

**Goal:** protect the app's core promise — the combined-rating math — and make all tools agree.

**Steps**

1. Add `src/__tests__/utils/williamsGroundTruth.test.js`: feed Williams's exact 16-condition set through **both** `ratingCalculator.js` and `vaCalculator.js`; assert **both === 80%** with correct bilateral handling (L/R hips 5252 pair, L/R radiculopathy 8520/8620). (Condition list + DCs are not PII — safe to commit.)
...
4. Add `tests/e2e/tactical-calculator.spec.ts` driving the TacticalCalculator UI with Williams's set → assert displayed combined = **80%**.
...
**Acceptance:** both calculators = 80% for Williams; UI shows 80%; bilateral detection robust; pay-rate spot-checks pass; Retro Pay returns dollars.
...
2. For each tool, record one matrix row in `TEST-RESULTS.md`: _renders? / consumes packet data? / primary action works? / notes_. Deep tools (calculators, ingestion, privacy, domain) are already covered in S1/S2/S4/S5 — here they get a smoke confirmation; the long tail gets smoke + a targeted check of its primary action.
...
**Goal:** prove the zero-knowledge thesis on Williams's real data.
...
2. Switch to **Gemini**, submit a prompt containing Williams's SSN-last-4 / file number; confirm the request body to `generativelanguage.googleapis.com` contains only `[REDACTED]` tokens (validates `piiScrubber.js`). Confirm Gemini opt-in is **OFF by default**.
...
1. Spot-verify Williams's diagnostic codes (9411, 5242, 8520/8620, 5276, 6260, 5252) against **live eCFR 38 CFR Part 4** (WebFetch `ecfr.gov`). Build an _app-vs-eCFR_ table; flag stale criteria in `disabilityData.json` (`lastVerifiedDate 2026-01-18`). Refresh via the scraper pipeline (`scripts/scrapers/va_data_pipeline.py`) **only if** it runs cleanly in-session; otherwise document the refresh as a plan item.
...
**Deliverable:** `audit/2026-06-cfile/AUDIT-REPORT.md` containing: harness baseline + final state; the 48-tool coverage matrix; the Williams **80%** ground-truth result; privacy egress evidence; domain spot-check tables; a11y results; a severity-ranked list of every bug found; and a clear **FIXED-this-session (with commit refs) vs OPEN** split. **PII-free.**
...
- **Compensation completeness:** TDIU (38 CFR 4.16 — Williams at 80% w/ PTSD is a textbook candidate), SMC (K–T), CRSC/CRDP — currently uncalculated; even a "not yet supported" banner prevents under-claiming thousands.
...
- A committed test asserts the app computes **80%** for Williams's condition set; `tests/e2e/tactical-calculator.spec.ts` passes.
