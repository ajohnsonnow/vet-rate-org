...
> (plus the running `TEST-RESULTS.md`). Source data: `E:\Johnson_C-FIle` (real veteran C-File — handle as PII).
...
Goal: (1) **thoroughly and exhaustively test every tool against the real C-File** at `E:\Johnson_C-FIle`, (2) **fix everything feasible** in the same session, and (3) leave behind a **saved improvement plan** for everything not fixed.

A read-only recon (9 agents) established the ground truth this plan is built on:

- **Run procedure**: `npm run dev` → `http://localhost:5173`; import the pre-built packet `E:\Johnson_C-FIle\vet-rate-packet-johnson-anthony-2026-03-21.json` (0.72 MB, schema v2.0, 38 importedFiles, 16 claims, 11 ratings) via **My Packet → Import**. This single import seeds nearly every downstream tool.
...
2. **PII/PHI is real.** `E:\Johnson_C-FIle` contains a real veteran's SSN, file number, DOB, and diagnoses.
...
**Goal:** prove `E:\Johnson_C-FIle` flows fully into the app through every entry path.
...
**Steps**
1. Add `src/__tests__/utils/johnsonGroundTruth.test.js`: feed Johnson's exact 16-condition set through **both** `ratingCalculator.js` and `vaCalculator.js`; assert **both === 80%** with correct bilateral handling (L/R hips 5252 pair, L/R radiculopathy 8520/8620). (Condition list + DCs are not PII — safe to commit.)
