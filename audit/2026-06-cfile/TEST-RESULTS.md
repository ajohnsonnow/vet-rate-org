...
> **PII constraint:** `E:\Williams_C-FIle` is real veteran data — never committed. Redacted synthetic fixture lives at `tests/fixtures/redacted-packet.json` (Sprint 1).
...
```js
const OUTPUT_PATH =
  "E:\\Williams_C-FIle\\vet-rate-packet-williams-robert-2026-03-21.json";
```
...
Combined rating implied: ~80% (matches Williams ground truth; Sprint 2 verifies the calculator).
...
## Sprint 2 — Calculation correctness vs the Williams 80% ground truth

### Findings

**Both calculators agree: 80% for the Williams 9-condition set.**
...
| Describe                                   | Tests                                                             |
| ------------------------------------------ | ----------------------------------------------------------------- |
| Williams 80% — ratingCalculator.js         | 3: final 80%, exact ~82.99, nearest-10                            |
| Williams 80% — vaCalculator.js             | 3: combineMultipleRatings=83, rounding, agreement                 |
...
### eCFR spot-check — Williams's 7 diagnostic codes
