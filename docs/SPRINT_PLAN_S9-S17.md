...
- **2026‑06‑06** — S16 (piiScrubber + lhci portion) complete + committed locally: `dcd2d5b` NFKC‑normalize before PII scan + fix leaky `legalAnswerer` scrub; `e21d876` hardened red‑team PII traps (zero‑width‑spliced, full‑width, NBSP); `d3f449b` promoted lhci `categories:seo` warn→error (LCP/TBT/`categories:performance` held at **warn** pending a real CI throttled‑mobile baseline). The "piiScrubber 3 TODOs" were never literal `// TODO`s — they were three obfuscation edge cases (zero‑width / soft‑hyphen splice, full‑width / fancy‑unicode digits, NBSP separators), now folded away pre‑scan and trapped by 5 `PII_TRAPS`. Evidence at [audit/S16_WORKLIST.md](audit/S16_WORKLIST.md). **Key‑rotation + device‑deauth remain owner‑gated** — paused before writing crypto pending owner decisions on the rotation/deauth threat model + device‑identity scope.
