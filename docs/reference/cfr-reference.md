# CFR Reference

Guide to relevant Code of Federal Regulations sections for VA disability claims.

---

## Title 38 - Veterans' Benefits

Title 38 of the Code of Federal Regulations contains all VA-related regulations.

### Access the eCFR

**Official Source:** [ecfr.gov/current/title-38](https://www.ecfr.gov/current/title-38)

---

## Part 3 - Adjudication

Rules for how VA decides claims.

### Key Sections

| Section | Topic |
|---------|-------|
| **§ 3.303** | Principles of service connection |
| **§ 3.304** | Direct service connection |
| **§ 3.307** | Presumptive service connection |
| **§ 3.309** | Presumptive diseases |
| **§ 3.310** | Secondary service connection |
| **§ 3.317** | Gulf War presumptives |
| **§ 3.318** | Agent Orange presumptives |

### § 3.310 - Secondary Service Connection

> "Disability which is proximately due to or the result of a service-connected disease or injury shall be service connected."

Key points:

- Secondary conditions must be caused OR aggravated
- Medical nexus required
- Must have existing service-connected condition

---

## Part 4 - Schedule for Rating Disabilities

The rating schedule containing criteria for all rated conditions.

### General Provisions

| Section | Topic |
|---------|-------|
| **§ 4.1** | Essentials of evaluative rating |
| **§ 4.2** | Interpretation of examination reports |
| **§ 4.3** | Reasonable doubt |
| **§ 4.6** | Evaluation of evidence |
| **§ 4.7** | Higher rating when symptoms between levels |
| **§ 4.10** | Functional impairment |

### § 4.3 - Reasonable Doubt

> "When there is an approximate balance of positive and negative evidence... the benefit of the doubt will be given to the claimant."

### § 4.7 - Higher of Two Evaluations

> "Where there is a question as to which of two evaluations shall be applied, the higher evaluation will be assigned if the disability picture more nearly approximates the criteria required for that rating."

---

## Body System Sections

### § 4.71a - Musculoskeletal System

Rating criteria for:

- Spine conditions
- Joint conditions (shoulders, knees, ankles, etc.)
- Muscle injuries
- Range of motion measurements

**Key Codes:**

| Code Range | Body Area |
|------------|-----------|
| 5000-5024 | Bones, joints, muscles |
| 5200-5230 | Shoulder and arm |
| 5250-5284 | Hip and thigh |
| 5256-5263 | Knee and leg |
| 5270-5284 | Ankle and foot |
| 5235-5243 | Spine |

### § 4.97 - Respiratory System

Rating criteria for:

- Sleep apnea (6847)
- Asthma (6602)
- COPD
- Restrictive lung disease

**Key Codes:**

| Code | Condition |
|------|-----------|
| 6602 | Asthma |
| 6847 | Sleep Apnea |

### § 4.104 - Cardiovascular System

Rating criteria for:

- Heart disease
- Hypertension
- Arrhythmias
- Vascular conditions

**Key Codes:**

| Code | Condition |
|------|-----------|
| 7005 | Arteriosclerotic heart disease |
| 7101 | Hypertension |

### § 4.124a - Neurological System

Rating criteria for:

- Peripheral neuropathy
- Radiculopathy
- Paralysis
- Neuritis

### § 4.130 - Mental Disorders

The General Rating Formula for Mental Disorders:

| Rating | Criteria Summary |
|--------|------------------|
| **100%** | Total occupational and social impairment |
| **70%** | Deficiencies in most areas |
| **50%** | Reduced reliability and productivity |
| **30%** | Occasional decrease in efficiency |
| **10%** | Mild symptoms |
| **0%** | Diagnosed, symptoms controlled |

**Key Codes:**

| Code | Condition |
|------|-----------|
| 9201 | Schizophrenia |
| 9400 | Generalized Anxiety |
| 9411 | PTSD |
| 9434 | Major Depressive Disorder |

---

## DeLuca Factors (§ 4.40, 4.45)

Additional factors for musculoskeletal ratings:

- **§ 4.40** — Functional loss
- **§ 4.45** — The joints
- Pain on movement
- Weakness
- Fatigability
- Incoordination
- Flare-ups

---

## Using CFR in Claims

### In Your Statements

Reference relevant sections:

> "Per 38 CFR § 3.310, my [secondary condition] was caused by my service-connected [primary condition]."

### For Understanding Criteria

Read the specific criteria for your condition to understand:

- What the VA evaluates
- What separates rating levels
- How to document your symptoms

### With Your VSO/Attorney

Knowing the regulations helps you:

- Ask informed questions
- Understand decisions
- Prepare appeals

---

## Staying Current

Regulations change. Check:

- eCFR for current text
- VA announcements for changes
- VSO guidance for interpretation
