...
const PDF_PATH =
  "E:/VS_Studio/Williams_C-FIle/Williams Service Records DD214 ALL.pdf";
...
async function extractPDFText(pdfPath) {
  ...
  try {
    ...
    const outputPath = "E:/VS_Studio/Williams_C-FIle/extracted_text.txt";
...
extractPDFText(PDF_PATH).then((text) => {
  if (text) {
    ...
    for (const marker of markers) {
      ...
      console.log(
        `   ${found ? "✅" : "❌"} "${marker}": ${found ? "Found" : "NOT FOUND"}`,
      );
    }
  }
});
