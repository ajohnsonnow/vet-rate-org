...
async function checkRecentComments(repo, issueNumber, keywords) {
  try {
    const { status, data } = await fetchGitHub(
      `/repos/${repo}/issues/${issueNumber}/comments?per_page=10&sort=created&direction=desc`,
    );
...
async function checkLatestRelease() {
  try {
    const { status, data } = await fetchGitHub(
      "/repos/mlc-ai/web-llm/releases?per_page=5",
    );
...
// Check one monitored issue and report findings; returns true if it signals a fix
async function reportIssue(issue) {
  ...
  console.log(
    `   📅 Last updated: ${new Date(status.updated).toLocaleDateString()}`,
  );
  ...
  const comments = await checkRecentComments(
    issue.repo,
    issue.issue,
    issue.keywords,
  );
  let fixSignal = status.state === "closed";
  ...
  if (comments.hasPositiveSignal) {
    ...
    comments.positiveComments?.forEach((c) => {
      console.log(
        `      - ${c.author} (${new Date(c.date).toLocaleDateString()}): ${c.preview}`,
      );
    });
  }
...
// Check web-llm releases and report findings; returns true if it signals a fix
async function reportReleases() {
  ...
  console.log(
    `   Latest: ${releases.latest.tag} (${new Date(releases.latest.date).toLocaleDateString()})`,
  );
...
// Print the closing summary/next-steps block
function reportOutcome(hasFixSignal) {
  console.log("");
  console.log(
    "═══════════════════════════════════════════════════════════════════",
  );
  ...
  if (hasFixSignal) {
    ...
    console.log(
      "  • Use Chrome Canary with: --enable-dawn-features=allow_unsafe_apis",
    );
...
async function main() {
  ...
  console.log(
    "║     Checking MLC-AI GitHub for updates...                      ║",
  );
  console.log(
    "╚════════════════════════════════════════════════════════════════╝",
  );
  ...
  for (const issue of ISSUES_TO_CHECK) {
    if (await reportIssue(issue)) hasFixSignal = true;
  }

  if (await reportReleases()) hasFixSignal = true;

  reportOutcome(hasFixSignal);
