#!/usr/bin/env node
/**
 * fetch-m21-5.mjs — fetch the VA M21-5 Appeals and Reviews manual (S44).
 *
 * M21-5 is the VA's guide for processing legacy appeals and Higher-Level
 * Reviews (HLR) under the Appeals Modernization Act — created 2019, lives
 * under the same KnowVA "Compensation and Pension" tree as M21-1 (found by
 * querying that parent topic directly: /topic/554400000001924, which lists
 * 38 CFRs / M21-1 / M21-3 (Training) / M21-4 (internal workload & quality-
 * review procedures) / M21-5 / Rate Tables / Subscribe Here as siblings).
 * Only M21-5 was judged a genuine gap for this app (disability ratings/
 * claims research): M21-3/M21-4 are internal VA staff-training and
 * operational/quality-review material, not veteran-facing adjudication
 * criteria; Rate Tables duplicates src/data/vaPayRatesHistorical.js (already
 * VA.gov-sourced and tested); the 38 CFRs mirror duplicates our authoritative
 * eCFR.gov-sourced legal-index shard.
 *
 * Mechanism: IDENTICAL to fetch-m21-1.mjs (same KnowVA v11 JSON content API,
 * same portal, same BFS-the-topic-tree-then-page-articles approach) — just a
 * different root topic id. Deliberately a separate file (not a generalized
 * "fetch-knowva-manual.mjs --manual=m21-5") to match this codebase's
 * established one-file-per-source convention (fetch-cavc.mjs vs.
 * fetch-cavc-historical.mjs, etc.) — keeps each source's citation/output
 * conventions independently adjustable without a shared abstraction leaking
 * one manual's quirks into another's.
 *
 * Citation format differs from M21-1's Roman-numeral Part.Chapter.Section
 * convention — M21-5 article names are plain "M21-5, Chapter N, Section X -
 * Title" (verified live against real articles, not assumed).
 *
 * Output: scripts/legal-ingestion/.work/m21-5.jsonl. Chunked+embedded into
 * its own shard (public/dkb-index/m21_5/) by build-m21-5-shard.mjs — NOT
 * folded into the eCFR monolith (S29 regression-free convention).
 *
 * Env knobs:
 *   M21_5_MAX_ARTICLES=<n>   cap total articles (bounded verification runs)
 *   M21_5_MIN_ARTICLES=<n>   coverage floor for a FULL run (default 28; a live
 *                            bounded-crawl check found 19 article-bearing
 *                            topics claiming ~36 articles total — this is a
 *                            genuinely small manual, unlike M21-1. Re-verify
 *                            against a live root query before trusting this
 *                            default if the manual grows substantially)
 *   M21_5_THROTTLE_MS=<n>    per-request delay (default 350)
 *   M21_5_PORTAL_ID / M21_5_ROOT_TOPIC   override IDs if the portal reorganizes
 */
...
const PORTAL_ID = process.env.M21_5_PORTAL_ID || "554400000001018";
const ROOT_TOPIC = process.env.M21_5_ROOT_TOPIC || "554400000018297";
const LANG = "en-us";
const ARTICLE_PAGE_SIZE = 50;
const THROTTLE_MS = Number(process.env.M21_5_THROTTLE_MS) || 350;
const MAX_ARTICLES = Number(process.env.M21_5_MAX_ARTICLES) || 0; // 0 = unlimited
const MIN_ARTICLES = Number(process.env.M21_5_MIN_ARTICLES) || 28;
const MAX_TOPIC_FETCHES = 500; // M21-5 is a small manual (12 top-level chapters); generous but bounded
...
async function apiJson(pathAndQuery) {
  ...
  if (!res.ok) throw new Error(`m21-5 ${url} → HTTP ${res.status}`);
  return res.json();
}
...
/**
 * Derive an M21-5 citation from its "M21-5, Chapter N, Section X - Title"
 * article-name convention (verified live: e.g. "M21-5, Chapter 7, Section B -
 * Notice of Disagreement (NOD)"). Falls back to a truncated title so a
 * record is never citation-less (front-matter articles like the Overview/
 * Table of Contents don't carry a Chapter/Section).
 * @param {string} name
 * @returns {string}
 */
export function citationFor(name) {
  const s = String(name || "").trim();
  const m = /Chapter\s+(\d+),?\s+Section\s+([A-Z])/i.exec(s);
  if (m) return `M21-5 Ch. ${m[1]}, Sec. ${m[2].toUpperCase()}`;
  return s ? `M21-5 — ${s.slice(0, 80)}` : "M21-5";
}
...
async function crawlLeafTopics() {
  ...
  while (queue.length && fetches < MAX_TOPIC_FETCHES) {
    ...
    try {
      ...
      console.warn(`[m21-5] topic ${id} children failed: ${e.message}`);
      continue;
    }
    await sleep(THROTTLE_MS);
  }
...
async function recordsForLeaf(leaf, seenArticleIds, limit) {
  ...
  try {
    ...
    console.warn(`[m21-5] articles for topic ${leaf.id} failed: ${e.message}`);
    return [];
  }
  const specs = [];
  for (const art of articles) {
    ...
    specs.push({
      ...
      title: String(art.name || "").trim() || `M21-5 article ${art.id}`,
      body,
...
async function main() {
  ...
  console.log(
    `[m21-5] crawling topic tree from root ${ROOT_TOPIC} (portal ${PORTAL_ID})…`,
  );
  const leaves = await crawlLeafTopics();
  if (leaves.length === 0) {
    throw new Error(
      "M21-5 topic crawl found ZERO article-bearing topics — the content API " +
  ...
  console.log(
    `[m21-5] ${leaves.length} article-bearing topics found (~${claimed} articles claimed)`,
  );
  ...
  if (records.length === 0) {
    throw new Error("M21-5 fetch produced ZERO records with non-empty content");
  }

  // Coverage floor — only on a full (uncapped) run. A bounded verification run
  // (M21_5_MAX_ARTICLES set) is exempt so testing doesn't false-alarm.
  if (!MAX_ARTICLES && records.length < MIN_ARTICLES) {
    throw new Error(
      `M21-5 coverage floor: got ${records.length} articles, expected ≥ ${MIN_ARTICLES}. ` +
  ...
  const outPath = path.join(WORK_DIR, "m21-5.jsonl");
  writeFileSync(outPath, out);
  const rel = path.relative(path.resolve(__dirname, "..", ".."), outPath);
  const cap = MAX_ARTICLES ? ` (capped at ${MAX_ARTICLES})` : "";
  console.log(`[m21-5] wrote ${records.length} article records → ${rel}${cap}`);
}

if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch((e) => {
    console.error(`[m21-5] FAILED: ${e.message}`);
    process.exit(1);
  });
}
