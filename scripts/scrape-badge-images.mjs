...
// Pipe an HTTP response to disk; resolves with outputPath, rejects on stream error.
function saveResponseToFile(response, outputPath) {
...
// Follow one redirect hop and save the resulting response to disk.
function followRedirectAndSave(redirectUrl, outputPath, resolve, reject) {
    ...
    .get(
      ...
      (redirectResponse) => {
        if (redirectResponse.statusCode !== 200) {
          reject(
            new Error(`HTTP ${redirectResponse.statusCode} after redirect`),
          );
          return;
        }
        saveResponseToFile(redirectResponse, outputPath).then(resolve, reject);
      },
    )
    .on("error", reject);
}
  ...
  return new Promise((resolve, reject) => {
    const request = https.get(
      url,
      {
        headers: {
          "User-Agent":
            "VetRate.org/1.0 (https://vet-rate.org; contact@vet-rate.org) Node.js badge-scraper",
          Accept: "image/*,*/*;q=0.9",
        },
      ...
      (response) => {
        ...
        if (
          ...
          followRedirectAndSave(redirectUrl, outputPath, resolve, reject);
          return;
        }
        ...
        saveResponseToFile(response, outputPath).then(resolve, reject);
      },
    );
...
// Download every entry in one badge/tab source set into outputDir, recording
// successes/failures onto the shared `results` accumulator.
async function downloadEntrySet(entries, outputDir, icon, results) {
  for (let i = 0; i < entries.length; i++) {
    const [id, item] = entries[i];
    console.log(`[${i + 1}/${entries.length}] ${icon}${item.name}`);
    ...
    if (fs.existsSync(outputPath)) {
      ...
      if (stats.size > 100) {
        console.log(
          `   ✅ Already exists: ${filename} (${(stats.size / 1024).toFixed(1)} KB)`,
        );
        results.success.push({ id, name: item.name, file: filename });
        continue;
      }
    }
    ...
    try {
      ...
      await downloadFromWikimedia(item.file, outputPath);
      const stats = fs.statSync(outputPath);
      console.log(
        `   ✅ Saved: ${filename} (${(stats.size / 1024).toFixed(1)} KB)`,
      );
      results.success.push({ id, name: item.name, file: filename });
    } catch (err) {
      console.log(`   ❌ Failed: ${err.message}`);
      results.failed.push({ id, name: item.name, error: err.message });
    }
...
async function scrapeBadges() {
  ...
  console.log(
    "║       MILITARY BADGE IMAGE SCRAPER (Wikimedia Commons)           ║",
  );
  console.log(
    "╚══════════════════════════════════════════════════════════════════╝\n",
  );
  ...
  await downloadEntrySet(
    Object.entries(BADGE_SOURCES),
    BADGES_DIR,
    "🎖️  ",
    results,
  );
  ...
  await downloadEntrySet(Object.entries(TAB_SOURCES), TABS_DIR, "📛 ", results);
