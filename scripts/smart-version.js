...
function determineVersionBump(commitAnalysis, fileAnalysis) {
  ...
  if (commitAnalysis.major.length > 0 || fileAnalysis.schemaChanges.length > 0) {
    return {
      type: 'major',
  ...
  if (hasPatchChanges) {
    return {
      ...
      evidence: [
        ...
        ...(commitAnalysis.unknown.length > 0 ? [`${commitAnalysis.unknown.length} other commits`] : []),
      ],
    };
  }
...
function main() {
  ...
  if (staged || unstaged) {
    ...
    if (staged) console.log(`   • Staged: ${staged.split('\n').filter(f => f).length} files`);
    if (unstaged) console.log(`   • Unstaged: ${unstaged.split('\n').filter(f => f).length} files`);
  }
  ...
  if (recommendation.evidence.length > 0) {
    console.log('\n   Evidence:');
    recommendation.evidence.slice(0, 5).forEach(e => console.log(`   • ${e}`));
