...
// Bug-report category keywords in priority order — first match wins,
...
function categorizeBugReport(cat) {
...
function parseBugReportText(text) {
  ...
  if (categoryMatch) {
    bug.category = categorizeBugReport(categoryMatch[1].trim());
  }
  ...
  const descMatch = text.match(
    // eslint-disable-next-line sonarjs/slow-regex -- fuzz-tested: `$` guarantees the lookahead always eventually matches, so the lazy scan can't get stuck; 0ms at 200k adversarial chars
...
function addBugToFile(bug) {
  ...
  content =
    content.slice(0, markerEnd) + newBugEntry + content.slice(markerEnd);
...
async function main() {
  ...
  if (!CATEGORIES.includes(bug.category)) {
    console.log(
      `⚠️  Unknown category "${bug.category}", defaulting to "Other"`,
    );
    bug.category = "Other";
  }
