...
const GLOSSARY_SOURCE_DEPRECATED = path.join(
  rootDir,
  "src/_deprecated/utils/vaGlossary.js",
);
const GLOSSARY_SOURCE = fs.existsSync(GLOSSARY_SOURCE_ACTIVE)
  ? GLOSSARY_SOURCE_ACTIVE
  : GLOSSARY_SOURCE_DEPRECATED;
...
const getCategoryLetter = (term) => {
  ...
  if (/[A-C]/.test(first) || term.startsWith("C&P") || term.startsWith("CFR"))
    return "A-C";
...
function parseGlossarySource() {
  ...
  while (i < body.length) {
    ...
    if (
      entry.term &&
      !EXCLUDE_TERMS.includes(entry.term) &&
      !glossary.has(entry.term)
    ) {
...
function updateUserManual(newGlossaryContent, write = false) {
  ...
  const glossaryRegex =
    // eslint-disable-next-line sonarjs/slow-regex -- fuzz-tested (200k adversarial chars, 0ms); operates on this repo's own trusted UserManual.jsx source file
    /(glossary:\s*\{\s*title:\s*"Glossary",\s*content:\s*`)([^`]*?)(`\s*,?\s*\})/s;
  ...
  const oldTermCount = (oldContent.match(/\*\*[A-Za-z0-9&\s-]+\*\*/g) || [])
    .length;
  const newTermCount = (
    newGlossaryContent.match(/\*\*[A-Za-z0-9&\s-]+\*\*/g) || []
  ).length;
  ...
  if (write) {
    ...
    log(
      `✅ Updated glossary in UserManual.jsx (${oldTermCount} → ${newTermCount} terms)`,
      "green",
    );
  }
...
export function syncGlossary(write = false) {
  ...
  try {
    ...
    if (result.changed) {
      if (write) {
        log(
          `   ✅ Glossary updated: ${result.oldCount} → ${result.newCount} terms`,
          "green",
        );
      } else {
        log(
          `   ⚠️  Glossary needs update: ${result.oldCount} → ${result.newCount} terms`,
          "yellow",
        );
