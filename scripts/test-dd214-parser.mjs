...
function extractBranchAndComponent(upperText) {
  const branch =
    upperText.includes("ARMY") || upperText.includes("ARNG") ? "Army" : null;
  const component =
    upperText.includes("ARNG") || upperText.includes("GUARD")
      ? "National Guard"
      : null;
...
function extractRank(text) {
  const rankMatch = text.match(/4a?\.\s*GRADE.*?RANK\s*([A-Z]{2,4})/i);
...
function extractPayGrade(text) {
  const payMatch = text.match(/4b?\.\s*PAY\s+GRADE\s*([EO]-?\d)/i);
...
function extractMOS(text) {
  const mosMatch = text.match(
    /PRIMARY\s+SPECIALTY\s*(\d{2}[A-Z]\d{2})\s+([A-Z][A-Z\s]*?)(?:--|\/\/|\d)/i,
  );
...
function extractServiceDates(text) {
  const entryMatch = text.match(
    /12a\..*?DATE.*?ENTERED.*?(\d{4})\s*\|\s*(\d{2})\s*\|\s*(\d{2})/i,
  );
  const serviceStartDate = entryMatch
    ? `${entryMatch[1]}-${entryMatch[2]}-${entryMatch[3]}`
    : null;

  const sepMatch = text.match(
    /12b\..*?SEPARATION.*?DATE.*?(\d{4})\s*\|\s*(\d{2})\s*\|\s*(\d{2})/i,
  );
  const serviceEndDate = sepMatch
    ? `${sepMatch[1]}-${sepMatch[2]}-${sepMatch[3]}`
    : null;
...
function extractAwards(text) {
  ...
  const awardsText = awardsMatch[1]
    .replace(/\([^)]{0,500}\)/g, " ") // Remove parentheses (bounded: real award qualifiers are always short; confirmed 14.5s->0.1s on 100k unmatched "(" via fuzz timing)
...
function extractDischargeType(text) {
  const dischargeMatch = text.match(/24\.\s*CHARACTER.*?SERVICE\s*([A-Z]+)/i);
...
console.log(
  `MOS:         ${result.mos || "❌ NOT FOUND"} - ${result.mosTitle || ""}`,
);
...
console.log("\n" + "=".repeat(60));
console.log(
  `SCORE: ${passCount}/${checks.length} checks passed (${Math.round((passCount / checks.length) * 100)}%)`,
);
