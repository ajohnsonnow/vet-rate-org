...
const disabilityDataPath = path.join(
  __dirname,
  "../src/data/disabilityData.json",
);
const disabilityData = JSON.parse(fs.readFileSync(disabilityDataPath, "utf8"));
const ACTUAL_DISABILITY_COUNT = disabilityData.disabilities
...
const SUSPICIOUS_PATTERNS = [
  ...
  },
  {
    ...
    exemptions: ["HARDCODED_NUMBERS_AUDIT.md"],
  },
];
...
if (violations.length === 0) {
  ...
  console.log(
    "💡 Fix these before committing. See HARDCODED_NUMBERS_AUDIT.md for guidance.\n",
  );
  process.exit(1);
}
