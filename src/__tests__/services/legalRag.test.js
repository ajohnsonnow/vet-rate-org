...
describe("legalRag — BM25 scoring", () => {
  ...
  it("applies length normalization (shorter doc, same tf, scores higher)", () => {
    const twoDocs = buildBM25Index(["alpha beta", "alpha beta gamma delta epsilon"]);
...
describe("legalRag — hybridFuse (RRF)", () => {
  ...
  it("computes fused score as the weighted RRF sum", () => {
    ...
    const expected =
      RRF_W_DENSE / (RRF_K + 2) + RRF_W_BM25 / (RRF_K + 1);
  ...
  it("honors topK", () => {
    ...
    const bm25 = [0, 0, 0, 0];
    expect(hybridFuse({ cosine, bm25, threshold: 0.35, topK: 2 })).toHaveLength(2);
  });
});
...
describe("legalRag — mmrRerank (S22 diversity reranking)", () => {
  ...
  it("returns the pool unchanged (truncated) when pool.length <= topK", () => {
    const pool = [{ index: 0, cosine: 0.9 }, { index: 1, cosine: 0.8 }];
  ...
  it("caches vector lookups (getVector called once per candidate, not once per comparison)", () => {
    ...
    const counts = calls.reduce((m, i) => m.set(i, (m.get(i) || 0) + 1), new Map());
...
describe("legalRag — getChunksByCitation", () => {
  ...
  it("returns all cached chunks sharing a citation, in order", async () => {
    ...
    const chunksJsonl = [
      { id: "s/4.71a/0", source: "ecfr", citation: "38 CFR § 4.71a", title: "t", text: "one", source_url: "u", fetched_at: "f" },
      { id: "s/4.71a/1", source: "ecfr", citation: "38 CFR § 4.71a", title: "t", text: "two", source_url: "u", fetched_at: "f" },
      { id: "s/4.1/0", source: "ecfr", citation: "38 CFR § 4.1", title: "t", text: "solo", source_url: "u", fetched_at: "f" },
    ].map((r) => JSON.stringify(r)).join("\n");
    const bin = mkBin([Array.from(a.q8), Array.from(b.q8), Array.from(c.q8)]);
    const manifest = { version: "v0.1.0", embedding_dim: EMBED_DIM, sources: { ecfr: 3 } };

    const originalFetch = globalThis.fetch;
    globalThis.fetch = vi.fn(async (url) => {
      if (url.endsWith("/manifest.json")) return new Response(JSON.stringify(manifest), { status: 200 });
      if (url.endsWith("/chunks/ecfr.jsonl")) return new Response(chunksJsonl, { status: 200 });
      if (url.endsWith("/vectors/ecfr.bin")) return new Response(bin.buffer, { status: 200 });
      return new Response("nf", { status: 404 });
    });
    vi.doMock("@huggingface/transformers", () => ({
      pipeline: async () => async () => ({ data: Float32Array.from(a.q8, (v) => v / 127) }),
    }));
...
describe("legalRag — query() hybrid rescue path", () => {
  ...
  it("surfaces a lexically-matched chunk the dense score put below threshold", async () => {
    ...
    const chunksJsonl = [
      { id: "c0", source: "ecfr", citation: "38 CFR § 4.1", title: "t0", text: "musculoskeletal essentials", source_url: "u0", fetched_at: "f" },
      { id: "c1", source: "ecfr", citation: "38 CFR § 4.2", title: "t1", text: "interpretation reports", source_url: "u1", fetched_at: "f" },
      { id: "c2", source: "ecfr", citation: "38 CFR § 4.14", title: "t2", text: "zebra pyramiding avoidance", source_url: "u2", fetched_at: "f" },
    ].map((r) => JSON.stringify(r)).join("\n");
  ...
  it("dense-only mode ignores BM25 (opts.hybrid=false)", async () => {
    ...
    const chunksJsonl = [
      { id: "d0", source: "ecfr", citation: "38 CFR § 4.1", title: "t0", text: "alpha", source_url: "u", fetched_at: "f" },
      { id: "d1", source: "ecfr", citation: "38 CFR § 4.2", title: "t1", text: "beta", source_url: "u", fetched_at: "f" },
      { id: "d2", source: "ecfr", citation: "38 CFR § 4.14", title: "t2", text: "zebra", source_url: "u", fetched_at: "f" },
    ].map((r) => JSON.stringify(r)).join("\n");
    const bin = mkBin([Array.from(a.q8), Array.from(b.q8), Array.from(negA)]);
    const manifest = { version: "v0.1.0", embedding_dim: EMBED_DIM, sources: { ecfr: 3 } };
    globalThis.fetch = vi.fn(async (url) => {
    ...
    const { query } = await import("../../services/legalRag.js");
    const res = await query("zebra", { topK: 3, threshold: 0.35, hybrid: false });
  ...
  it("MMR (default on, real lambda=0.7) prefers a distinct 2nd result over a near-duplicate; opts.mmr=false reverts to raw relevance order", async () => {
    ...
    const chunksJsonl = [
      { id: "e0", source: "ecfr", citation: "38 CFR § 4.1", title: "t0", text: "one", source_url: "u", fetched_at: "f" },
      { id: "e1", source: "ecfr", citation: "38 CFR § 4.2", title: "t1", text: "two", source_url: "u", fetched_at: "f" },
      { id: "e2", source: "ecfr", citation: "38 CFR § 4.3", title: "t2", text: "three", source_url: "u", fetched_at: "f" },
    ].map((r) => JSON.stringify(r)).join("\n");
    ...
    vi.doMock("@huggingface/transformers", () => ({
      pipeline: async () => async () => ({ data: Float32Array.from(query, (v) => v / 127) }),
    }));
    ...
    const withoutMmr = await runQuery("query text", { topK: 2, threshold: 0, mmr: false });
