...
export const AICurrentBadge = ({ className = "" }) => {
  ...
  if (aiStatus.effectiveMode === AI_MODES.LOCAL) {
    return (
      <div
        className={`inline-flex items-center gap-2 px-3 py-2 bg-green-500/20 border border-green-500/50 rounded-lg ${className}`}
      >
  ...
  if (aiStatus.effectiveMode === AI_MODES.CLOUD) {
    return (
      <div
        className={`inline-flex items-center gap-2 px-3 py-2 bg-blue-500/20 border border-blue-500/50 rounded-lg ${className}`}
      >
  ...
  return (
    <div
      className={`inline-flex items-center gap-2 px-3 py-2 bg-gray-500/20 border border-gray-500/50 rounded-lg ${className}`}
    >
...
export default function AIReadyCheck({
  ...
  const [localInitializing, setLocalInitializing] = useState(
    isLocalAIInitializing(),
  );
  ...
  if (localInitializing) {
    return (
      <div
        className={`bg-cyan-50 dark:bg-cyan-900/20 border border-cyan-200 dark:border-cyan-800 rounded-xl p-4 ${className}`}
      >
          ...
          <div className="flex-1">
            ...
            {!compact && (
              <p className="text-sm text-cyan-700 dark:text-cyan-300">
                The Neural Engine is loading. Please wait before sending
                messages.
              </p>
            )}
          </div>
  ...
  return (
    <div
      className={`bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-4 ${className}`}
    >
        ...
        <div className="flex-1">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            ...
            >
              ...
              <div className="text-left flex-1">
                ...
                {deviceCapability.advice?.batteryWarning &&
                  !deviceCapability.isUnsupported && (
            ...
            >
              ...
              <div className="text-left flex-1">
                ...
                <p className="text-xs text-gray-600 dark:text-gray-400">
                  {deviceCapability.advice?.cloudAI.description ||
          ...
          {deviceCapability.advice?.dataSaverWarning && (
            <p className="text-xs text-amber-600 dark:text-amber-400 mt-3 text-center">
              📶 Data Saver mode detected. Local AI requires downloading
              0.7-4GB. Consider using Wi-Fi or Cloud AI.
            </p>
          )}
        </div>
      </div>
