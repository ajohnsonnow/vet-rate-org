...
const AISettingsModal = ({ onClose, onOpenLocalAI, onReportBug }) => {
  ...
  useEffect(() => {
    ...
    if (
      savedPreset &&
      ["LEGAL", "CREATIVE", "ADVERSARIAL", "BALANCED"].includes(savedPreset)
    ) {
        ...
        try {
          ...
          if (adapter) {
            ...
            const gpuName =
              info.description ||
              info.device ||
              `${(info.vendor || "Unknown").toUpperCase()} GPU`;
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-4">
              ...
              >
                ...
                {deviceCapability.tier === DEVICE_TIERS.UNSUPPORTED && (
                  <p className="text-xs text-gray-500 dark:text-gray-500 mt-2">
                    💡 <strong>Recommendation:</strong> Use Cloud AI below - it
                    works on all devices and is still very secure.
                  </p>
                )}
              </div>
            )}
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p
                      className={`font-semibold ${localAIReady ? "text-green-700 dark:text-green-300" : "text-gray-700 dark:text-gray-300"}`}
                    >
                      {localAIReady
                        ? "Local AI Running!"
                        : "Local AI Not Set Up"}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500 dark:text-gray-400">
                            {gpuInfo.vendor} • Mode:{" "}
                            {gpuInfo.preference ===
                            GPU_PREFERENCES.HIGH_PERFORMANCE
                              ? "🚀 High Performance"
                              : gpuInfo.preference === GPU_PREFERENCES.LOW_POWER
                                ? "🔋 Power Saver"
                                : "🤖 Auto"}
                          </p>
                        </div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400">
              {deviceCapability.tier === DEVICE_TIERS.UNSUPPORTED ? (
                <>
                  <strong>Why isn't Local AI available?</strong> Your device
                  doesn't support WebGPU, which is required for running AI
                  models locally. This is common on older Android versions (10
                  and below) and some iOS devices.
                </>
              ) : deviceCapability.tier === DEVICE_TIERS.LEGACY ? (
                <>
                  <strong>⚠️ Performance Warning:</strong> Your device may
                  struggle with Local AI due to limited GPU/memory. Consider
                  using Cloud AI for a smoother experience, or try the smallest
                  model in Local AI settings.
                </>
              ) : (
                <>
                  <strong>Requirements:</strong> Modern browser with WebGPU
                  support (Chrome 113+, Edge 113+). Downloads a 0.7-4GB model to
                  your device. <strong>Unload</strong> frees GPU memory when not
                  in use.
                </>
              )}
            </p>
          ...
          <div className="space-y-4">
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-purple-600 dark:text-purple-400 mt-1">
                    Gemini uses <strong>8,000 web-optimized</strong> entries
                    from our DKB (38 CFR, BVA, CAVC).
                  </p>
                  <p className="text-xs text-amber-600 dark:text-amber-500 mt-1">
                    ⚡ <strong>Note:</strong> Full database has 130K+ entries.
                    Load Local AI (above) for complete access.
                  </p>
                </div>
              </div>
            </div>
            ...
            {(deviceCapability.tier === DEVICE_TIERS.UNSUPPORTED ||
              ...
              <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                <p className="text-sm text-blue-700 dark:text-blue-300">
                  ✅ <strong>Best option for your device!</strong> Cloud AI
                  works on all devices and is fast and reliable. Your data is
                  still protected by Google's enterprise-grade security.
                </p>
              </div>
            )}
            ...
            <div className="p-3 bg-amber-50 dark:bg-amber-900/20 rounded-lg border-2 border-amber-300 dark:border-amber-700">
              <p className="text-sm text-amber-700 dark:text-amber-300 font-medium">
                ⚠️ <strong>Data Center Warning:</strong> Using your Gemini API
                key sends your condition names, symptoms, and any text you enter
                to Google's data centers. Your data will be processed on remote
                servers.
              </p>
              <p className="text-xs text-amber-600 dark:text-amber-400 mt-2">
                💡 <strong>Your API key is yours</strong> - we never see it and
                it stays in your browser. But any AI queries you make will
                travel to Google. Use Local AI above for 100% offline,
                zero-transmission privacy.
              </p>
            </div>

            <p className="text-sm text-gray-600 dark:text-gray-400">
              Get your free API key from Google AI Studio to enable AI-powered
              features. Faster than Local AI but requires internet.
            </p>
          ...
          <div className="bg-gray-100 dark:bg-gray-800 rounded-xl p-4">
            ...
            <ul className="text-sm text-gray-600 dark:text-gray-400 space-y-2">
              <li>
                • <strong>Local AI (Recommended)</strong> - 100% private, works
                offline, data never leaves your device
              </li>
              <li>
                • <strong>Cloud AI (Gemini)</strong> - Faster responses, but{" "}
                <strong>sends data to Google's data centers</strong>
              </li>
              <li>
                • <strong>BYOK (Bring Your Own Key)</strong> - Your API key
                stays in YOUR browser - we never see it
              </li>
              <li>
                • <strong>Response Length</strong> - Adjust token limits above
                to control response length and resource usage
              </li>
              <li>
                • <strong>Unload AI</strong> frees up GPU memory when you're
                done using AI features
              </li>
            </ul>
          </div>
        </div>
