  ...
  useEffect(() => {
    ...
    if (!navigator.gpu) {
      ...
      setErrorDetails({
        type: "webgpu",
        message:
          "WebGPU is required for Vision AI. Please use Chrome 113+, Edge 113+, or Arc browser on a desktop computer.",
        canRetry: false,
      });
      return;
    }
  ...
  const onDrop = useCallback(async (acceptedFiles) => {
    ...
    try {
      ...
      if (isPdfFile(file)) {
        ...
        setStatusMsg(
          `Rendering page 1/${metadata.numPages} at ${Math.round(optimalScale * 72)} DPI...`,
        );
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-red-700 dark:text-red-300 mt-1">
                Vision AI requires WebGPU, which is not available in your
                current browser.
              </p>
        ...
        <div className="mt-6 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
          ...
          <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <ResultField
              ...
              value={`${extractedData.rank || "—"} (${extractedData.payGrade || "—"})`}
            />
            <ResultField
              label="MOS/Rating"
              value={extractedData.mos}
              highlight
            />
            ...
            <ResultField
              label="Entry Date"
              value={extractedData.entryDateFormatted}
            />
            <ResultField
              label="Separation Date"
              value={extractedData.separationDateFormatted}
            />
            ...
            <ResultField
              ...
              value={extractedData.characterOfService}
              highlight
            />
            <ResultField
              label="Separation Code"
              value={extractedData.separationCode}
            />
            <ResultField
              label="Reentry Code"
              value={extractedData.reentryCode}
            />
            ...
            {extractedData.awards && extractedData.awards.length > 0 && (
              <div className="col-span-1 md:col-span-2">
                <ResultField
                  ...
                  value={extractedData.awards
                    .map((a) => a.abbreviation || a.name)
                    .join(", ")}
                />
              </div>
            )}
          ...
          <div className="bg-yellow-50 dark:bg-yellow-900/20 p-3 text-xs text-yellow-800 dark:text-yellow-200 border-t border-yellow-100 dark:border-yellow-800 flex items-start gap-2">
            ...
            <span>
              <strong>Important:</strong> Always verify AI extraction against
              the original document. Vision AI may misread degraded or
              handwritten text.
            </span>
          </div>
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="font-mono text-gray-800 dark:text-gray-100 font-medium flex justify-between items-center">
        <span
          className={`${!hasValue ? "text-gray-400 dark:text-gray-500 italic font-normal" : ""}`}
        >
          {hasValue ? (revealed ? value : "•••-••-••••") : "Not Found"}
        </span>
        {sensitive && hasValue && (
          <button
