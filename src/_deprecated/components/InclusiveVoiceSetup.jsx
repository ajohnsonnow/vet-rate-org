...
const InclusiveVoiceSetup = ({
  ...
  const handlePreview = useCallback((voice) => {
    previewVoice(
      voice,
      "I'm here to support you with your claim. Together, we've got this.",
    );
  }, []);
  ...
  if (step === "SAFETY") {
    return (
      <div
        className={`p-6 bg-slate-900 border-2 border-yellow-500/50 rounded-xl shadow-xl ${className}`}
      >
        ...
        <p className="text-slate-300 mb-6 leading-relaxed">
          Before we enable voice features, please confirm you are in a safe,
          private space to discuss your service and medical history.
        </p>

        <ul className="space-y-3 mb-6 text-sm text-slate-400">
          <li className="flex items-center gap-3">
            <span className="text-green-400">✓</span>I am in a private place
            where others won't overhear
          </li>
          <li className="flex items-center gap-3">
            <span className="text-green-400">✓</span>I feel grounded and ready
            to discuss my claim
          </li>
          <li className="flex items-center gap-3">
            <span className="text-green-400">✓</span>I know I can stop this
        ...
        <div className="bg-slate-800/50 rounded-lg p-4 mb-6 border border-slate-700">
          <p className="text-xs text-slate-400 flex items-center gap-2">
            ...
            <span>
              <strong>Quick Exit:</strong> Triple-tap the{" "}
              <kbd className="px-1.5 py-0.5 bg-slate-700 rounded text-xs">
                Esc
              </kbd>{" "}
              key at any time to instantly hide this app.
            </span>
          </p>
        </div>
  ...
  if (step === "LANGUAGE") {
    return (
      <div
        className={`p-6 bg-slate-900 border border-blue-500/30 rounded-xl ${className}`}
      >
        <h2 className="text-xl font-bold text-white mb-2">
          Choose Your Language
        </h2>
        <p className="text-slate-400 mb-6 text-sm">
          Select the language you're most comfortable with. The AI will speak to
  ...
  if (step === "SELECT") {
    return (
      ...
      >
        <h2 className="text-xl font-bold text-white mb-2">
          Choose Your Peer Voice
        </h2>
        <p className="text-slate-400 mb-4 text-sm">
          Select the voice that feels most supportive to you. This choice stays
          100% private.
        </p>
        ...
        {isLoading ? (
          ...
          <div className="space-y-4 mb-6 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
            {Object.entries(voices.categorized).map(
  ...
  if (step === "SETTINGS") {
    return (
      ...
      >
        ...
        <p className="text-slate-400 mb-6 text-sm">
          Adjust the voice to your preference. Slower speeds can help with
          focus.
        </p>
