/**
 * DD214 Test Data - JOHNSON, ANTHONY DANIEL
...
export const JOHNSON_DD214_TEST_DATA = {
  ...
  dd214s: [
    ...
    
    {
      ...
      expected: {
        ...
        homeOfRecord: "1127 SE 28TH AVE, PORTLAND, OR 97214",
        lastDutyAssignment: "HHC (-) 1-186 IN (FORWARD) (WPRPT2)",
        separationStation: "SALEM, OR 97309-5047",
        commandTransferred: "HHC (-) 1-186 IN (WPRPT2) 1420 E MAIN ST, ASHLAND, OR 97520",
    ...
    
    {
      ...
      expected: {
        ...
        // Locations
        placeOfEntry: "PORTLAND, OR",
        homeOfRecord: "1009 N E LEVERICH COURT, VANCOUVER, WA 98663",
    ...
    
    {
      ...
      expected: {
        ...
        homeOfRecord: "1009 NE LEVERICH COURT, VANCOUVER, WA 98663",
  ...
  careerSummary: {
    ...
    deployments: [
      { location: "Egypt (Sinai)", mission: "MFO Peacekeeping", year: "2005-2006" },
...
export const JOHNSON_AWARD_MAPPINGS = {
  ...
  "ARMY RESERVE COMPONENTS ACHIEVEMENT MEDAL": "army-reserve-components-achievement-medal",
  ...
  "MULTINATIONAL FORCES AND OBSERVERS MEDAL": "multinational-force-observers-medal",
...
export function validateParserOutput(parsedDD214, expectedDD214Id) {
  const expected = JOHNSON_DD214_TEST_DATA.dd214s.find(d => d.id === expectedDD214Id);
  if (!expected) {
  ...
  if (parsedDD214.name && !parsedDD214.name.includes("JOHNSON")) {
    errors.push(`Name mismatch: got "${parsedDD214.name}", expected contains "JOHNSON"`);
  }
  
  // Validate rank
  if (parsedDD214.rank && parsedDD214.rank !== exp.rank) {
    errors.push(`Rank mismatch: got "${parsedDD214.rank}", expected "${exp.rank}"`);
  }
  
  // Validate MOS
  if (parsedDD214.mos && !parsedDD214.mos.includes("92Y")) {
    errors.push(`MOS mismatch: got "${parsedDD214.mos}", expected contains "92Y"`);
  }
  ...
  for (const pattern of garbagePatterns) {
    if (parsedDD214.name && pattern.test(parsedDD214.name)) {
      errors.push(`Garbage in name: "${parsedDD214.name}" matches instruction pattern`);
    }
    if (parsedDD214.awards) {
      for (const award of parsedDD214.awards) {
        if (pattern.test(award)) {
          errors.push(`Garbage in awards: "${award}" matches instruction pattern`);
        }
      }
    }
  }
