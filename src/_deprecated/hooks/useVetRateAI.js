...
async function loadKnowledgeBase() {
  ...
  loadingPromise = fetch("/data/vet_rate_knowledge.json")
    ...
    .then((data) => {
      ...
      console.log(
        `[VetRate AI] 💎 DKB Loaded: ${knowledgeBase.length} official entries`,
      );
      console.log(
        `[VetRate AI] Sources:`,
      ...
      console.log(
        `[VetRate AI] ⚠️ CKB (Community) is separate - not loaded for AI training`,
      );
...
function findRelevant(query, kb, topK = 7) {
  ...
  const isSecondaryQuery =
    queryLower.includes("secondary") || queryLower.includes("nexus");
  const isPresumptiveQuery =
    queryLower.includes("presumptive") || queryLower.includes("pact");
  const isOGCQuery =
    queryLower.includes("ogc") ||
    queryLower.includes("precedent") ||
    queryLower.includes("opinion");

  const scored = kb.map((item) => {
    const text =
      `${item.instruction || ""} ${item.output || ""} ${item.title || ""} ${item.content || ""}`.toLowerCase();
    ...
    for (const term of queryTerms) {
      if (text.includes(term)) {
        ...
        if (
          term.match(/^\d{4}$/) &&
          (text.includes(`dc ${term}`) || text.includes(`code ${term}`))
        ) {
          score += 5;
        }
    ...
    if (
      isSecondaryQuery &&
      (source === "SECONDARY_CONDITIONS_MATRIX" || type.includes("secondary"))
    ) {
      score *= 2;
    }
    if (
      isPresumptiveQuery &&
      (source === "PACT_ACT_OFFICIAL" || type.includes("presumptive"))
    ) {
      score *= 2;
    }
...
export function useVetRateAI() {
  ...
  const ask = useCallback(async (question) => {
    ...
    try {
      ...
      const sources = relevant.map((r) => ({
        ...
        sourceInfo: SOURCE_COLORS[r.metadata?.source] || {
          color: "text-slate-400",
          bg: "bg-slate-700/50",
          label: "VA",
        },
  ...
  const searchDiagnosticCode = useCallback(
    (code) => {
  ...
  const searchSecondaryCondition = useCallback(
    (primary, secondary) => {
  ...
  const searchRatingCriteria = useCallback(
    (condition) => {
  ...
  const searchOGCOpinion = useCallback(
    (topic) => {
  ...
  const searchPresumptive = useCallback(
    (condition) => {
