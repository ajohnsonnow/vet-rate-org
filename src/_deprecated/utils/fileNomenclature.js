...
export const generateVAFilename = (options) => {
  ...
  const parts = [formatDate(date), sanitizeFilename(veteranName), docType];
...
export const suggestFilename = (file, context = {}) => {
  const { veteranName, currentCondition, statementType } = context;
  ...
  if (
    lowerName.includes("nexus") ||
    lowerName.includes("imr") ||
    lowerName.includes("imo")
  ) {
    docType = DOC_TYPES.NEXUS_LETTER;
  } else if (
    lowerName.includes("buddy") ||
    lowerName.includes("witness") ||
    lowerName.includes("lay")
  ) {
 ...
 */
export const prepareDocumentForDownload = (
  blob,
  suggestedName,
  context = {},
) => {
  const filename =
    context.useStandard !== false
