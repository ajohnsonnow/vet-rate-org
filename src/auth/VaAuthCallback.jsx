...
async function processVaCallback({
  isPopup,
...
function VaCallbackError({ message }) {
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 max-w-md w-full text-center">
...
function VaCallbackSuccess({ isPopup }) {
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 max-w-md w-full text-center">
...
const VaAuthCallback = () => {
  ...
  useEffect(() => {
    ...
    processVaCallback({ isPopup, exchangeCodeForTokens, setError, setStatus });
  }, [exchangeCodeForTokens, isPopup]);

  // Processing state
  if (status === "processing") {
    return <VaCallbackProcessing />;
  }
  ...
  if (status === "error") {
    return (
      <VaCallbackError
        message={error || authError || "An unknown error occurred"}
      />
    );
  }

  // Success state
  return <VaCallbackSuccess isPopup={isPopup} />;
};
