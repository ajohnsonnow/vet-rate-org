...
const VaAuthCallback = () => {
  ...
  useEffect(() => {
    ...
    const handleCallback = async () => {
      console.log('[VA Callback] Processing OAuth callback...', isPopup ? '(popup mode)' : '(redirect mode)');
      ...
      if (errorParam) {
        ...
        if (isPopup && window.opener) {
          window.opener.postMessage({ type: 'VA_AUTH_ERROR', error: errMsg }, window.location.origin);
        }
        return;
      }
      ...
      try {
        ...
        // Notify parent window if in popup
        if (isPopup && window.opener) {
          window.opener.postMessage({ type: 'VA_AUTH_ERROR', error: err.message }, window.location.origin);
        }
      }
    };
