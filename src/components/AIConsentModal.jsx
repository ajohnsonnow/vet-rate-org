...
const WhatThisDoesSection = ({ statementType }) => (
  <div>
...
const ThreePillarsSection = ({ pillars }) => (
      ...
      {pillars.map((pillar, index) => (
        <div key={index} className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-purple-500 dark:text-purple-400 italic mt-0.5">
              {pillar.example}
            </p>
          </div>
        </div>
...
const DataNotSharedSection = ({ items }) => (
...
const PrivacyInfoSection = ({ provider }) => (
  <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
    ...
    <ul className="text-sm text-blue-700 dark:text-blue-300 space-y-1">
      ...
      <li className="flex items-start gap-2">
        ...
        See our{" "}
        <a
          href="/docs/privacy/ai-assistant/"
...
const ConsentModalFooter = ({ onCancel, onConsent }) => (
    ...
    >
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          {/* What it does */}
          <WhatThisDoesSection statementType={statementType} />

          {/* Three Pillars explanation - only show if available */}
          {disclosure.pillars && (
            <ThreePillarsSection pillars={disclosure.pillars} />
          )}

          {/* Data that WILL be shared */}
          <DataSharedSection items={disclosure.dataShared} />

          {/* Data that will NOT be shared */}
          <DataNotSharedSection items={disclosure.notShared} />

          {/* Privacy note */}
          <PrivacyInfoSection provider={disclosure.provider} />
        ...
        {/* Footer with buttons */}
        <ConsentModalFooter onCancel={onCancel} onConsent={onConsent} />
      </div>
    </div>
  );
};
