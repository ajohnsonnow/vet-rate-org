...
/**
 * Pull the JSON payload out of an AI response, stripping any markdown
 * code-fence wrapping and trimming to the outermost { ... } object.
 */
...
function AnalyzerHeader({ onBack }) {
  return (
...
function AIAvailabilityWarning() {
  return (
    <div className="bg-yellow-900/30 border border-yellow-600 text-yellow-200 p-4 rounded-lg">
      ...
      <p className="text-sm mt-1">
        Please configure Cloud AI (Gemini) or initialize Local AI to use this
        feature.
      </p>
    </div>
  );
...
function ModeToggle({ mode, onSelectMode }) {
  return (
...
function ReferenceEvidenceInput({ referenceText, setReferenceText, loading }) {
  return (
...
function TargetStatementInput({ mode, targetText, setTargetText, loading }) {
  return (
    <div className="space-y-2">
      {/* eslint-disable-next-line jsx-a11y/label-has-associated-control */}
...
function AnalyzeButton({
  ...
  const isDisabled = loading || !aiAvailable || hasMissingInput;
    ...
    <div className="flex justify-center">
      <button
        ...
        className={`
            ...
            ${!aiAvailable || hasMissingInput ? "opacity-50 cursor-not-allowed" : ""}
          `}
      >
        {loading ? (
...
function ScoreHeader({ mode, analysis }) {
...
function TextForensicsPanel({ targetText, analysis }) {
  return (
    <div className="bg-gray-900 border border-gray-700 rounded-xl overflow-hidden">
      ...
      <div className="p-6 bg-black/20 max-h-[600px] overflow-y-auto">
        <DiffHighlighter text={targetText} issues={analysis.issues || []} />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="bg-gray-900 border border-gray-700 rounded-xl overflow-hidden">
      <div className="bg-gray-800 p-4 border-b border-gray-700 flex justify-between items-center">
        <h3 className="text-white font-bold flex items-center gap-2">
          <span>⚠️</span> Issues Found ({issues.length})
        </h3>
        {issues.length > 0 && (
          <RedditCopyButton
  ...
  return (
    <div className="bg-blue-900/20 border border-blue-700/50 rounded-xl p-6">
      ...
      <ul className="text-sm text-blue-200 space-y-2">
        ...
        <li>
          • Words like &quot;always&quot; and &quot;never&quot; are hard to
          prove and invite scrutiny
        </li>
        <li>
          • Fix these issues <strong>before</strong> submission to strengthen
          your claim
        </li>
      </ul>
    </div>
...
function ResultsDashboard({ mode, analysis, targetText }) {
  return (
    <div className="mt-8 space-y-6">
      {/* Score Header */}
      <div className="bg-gray-800 border border-gray-700 rounded-xl overflow-hidden">
        <ScoreHeader mode={mode} analysis={analysis} />

        {/* Visual Diff + Issue Cards Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 p-6">
          <TextForensicsPanel targetText={targetText} analysis={analysis} />
          <IssuesPanel analysis={analysis} />
        </div>

        {/* Strengths (Solo mode) */}
        <StrengthsSection mode={mode} analysis={analysis} />
      </div>
...
const AIConsistencyAnalyzer = ({ onBack }) => {
  ...
  const runConsistencyCheck = () =>
    performConsistencyCheck(mode, referenceText, targetText, {
      setLoading,
      setError,
      setAnalysis,
    });
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4 space-y-6">
      {/* Header */}
      <AnalyzerHeader onBack={onBack} />

      {/* AI Availability Warning */}
      {!aiAvailable && <AIAvailabilityWarning />}

      {/* Mode Toggle */}
      <ModeToggle mode={mode} onSelectMode={setMode} />
