...
function getColorBlindOptions(t) {
  return [
    {
...
function saveApiKey(apiKey, setApiKeySaved) {
  if (apiKey.trim()) {
...
function clearApiKey(setApiKey, setApiKeySaved) {
...
function useCloseMenuOnOutsideClick(menuRef, buttonRef, setIsOpen) {
  useEffect(() => {
...
// Close on Escape key
function useCloseMenuOnEscape(isOpen, setIsOpen, buttonRef) {
  useEffect(() => {
...
function useLoadSavedApiKey(setApiKey, setApiKeySaved) {
  useEffect(() => {
...
function AccessibilityToggleButton({ buttonRef, isOpen, onToggle }) {
  const { t } = useLanguage();
  return (
    <button
      ref={buttonRef}
...
function AccessibilityMenuHeader() {
  const { t } = useLanguage();
  return (
...
function FeaturesSummaryBanner() {
  const { t } = useLanguage();
  return (
...
function ThemeModeSection() {
  const { theme, setTheme, isTbiComfort, isAaaContrast } = useTheme();
  return (
      ...
      {isTbiComfort && (
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-2">
          <p className="text-xs text-amber-800 dark:text-amber-200">
            🧠 <strong>TBI Comfort Mode Active</strong> - Warm amber tones
            reduce visual stress for veterans with light sensitivity or
...
function ColorBlindOptionRow({ option, isSelected, onSelect }) {
  return (
    <label
...
function ColorVisionSection() {
  const { t } = useLanguage();
  const { colorBlindMode, setColorBlindMode } = useTheme();
  const colorBlindOptions = getColorBlindOptions(t);
  return (
    <div className="space-y-3">
      <h4 className="font-semibold text-gray-800 dark:text-gray-200 flex items-center gap-2">
        <svg
      ...
      <div className="space-y-2">
        {colorBlindOptions.map((option) => (
          <ColorBlindOptionRow
            key={option.value}
            option={option}
            isSelected={colorBlindMode === option.value}
...
function ApiKeyInput({ apiKey, setApiKey, showApiKey, setShowApiKey }) {
  const { t } = useLanguage();
  return (
...
function ApiKeyActions({ apiKey, apiKeySaved, onSave, onClear }) {
  const { t } = useLanguage();
  return (
    <>
      <div className="flex gap-2">
...
function AffiliationSection({ onOpenAffiliationPicker }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <button
        ...
        onClick={onOpenAffiliationPicker}
...
function ResetSettingsButton() {
  const { t } = useLanguage();
  const { setTheme, setColorBlindMode, setFontSize, setReducedMotion } =
    useTheme();
  return (
    <button
      onClick={() => {
...
function AffiliationPickerModal({ isOpen, onClose }) {
  return (
    <ResponsiveModal
      isOpen={isOpen}
      onClose={onClose}
    ...
    >
      <AffiliationPicker onClose={onClose} />
    </ResponsiveModal>
  );
}
...
export default function AccessibilityMenu() {
  ...
  useCloseMenuOnOutsideClick(menuRef, buttonRef, setIsOpen);
  useCloseMenuOnEscape(isOpen, setIsOpen, buttonRef);
  useLoadSavedApiKey(setApiKey, setApiKeySaved);
    ...
    <div className="relative sm:relative">
      ...
      {isOpen && (
        <AccessibilityDropdown
          ...
          setShowApiKey={setShowApiKey}
          apiKeySaved={apiKeySaved}
          onSaveApiKey={() => saveApiKey(apiKey, setApiKeySaved)}
          onClearApiKey={() => clearApiKey(setApiKey, setApiKeySaved)}
          onOpenAffiliationPicker={() => setAffiliationOpen(true)}
        />
      )}
