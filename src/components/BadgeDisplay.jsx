...
const getDefaultBadgeSVG = (badge, size) => {
  ...
  return (
    <svg viewBox="0 0 100 50" width={width} height={height}>
      {/* Badge background */}
      <ellipse cx="50" cy="25" rx="45" ry="20" fill={fill} stroke="#333" strokeWidth="2" />
      {/* Badge name (abbreviated) */}
      <text x="50" y="28" fontSize="14" textAnchor="middle" fill={textColor} fontWeight="bold">
...
const ServiceStripes = ({ count, branch = 'Army', size = 'md' }) => {
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      <svg width={stripeWidth} height={stripeHeight * count} viewBox={`0 0 35 ${20 * count}`}>
        {Array.from({ length: count }).map((_, i) => (
...
export const BadgeDisplay = ({
  ...
  const aboveBadges = badges.filter(b => 
  ...
  const belowBadges = badges.filter(b =>
...
export const FullUniformDisplay = ({
  ...
  return (
    <div className="full-uniform-display bg-gray-100 dark:bg-gray-800 rounded-lg p-6">
      ...
      {ribbonRack && (
        <div className="ribbon-rack-container my-4">
          {ribbonRack}
        </div>
      )}
      ...
      {(overseasBars.count > 0 || serviceStripes.count > 0) && (
        <div className="sleeve-display flex gap-8 justify-center mt-6 pt-4 border-t border-gray-300 dark:border-gray-600">
          {overseasBars.count > 0 && (
            <OverseasBars count={overseasBars.count} type={overseasBars.type} size={size} />
          )}
          {serviceStripes.count > 0 && (
            <ServiceStripes count={serviceStripes.count} branch={branch} size={size} />
          )}
        </div>
      )}
    </div>
...
export const CombatIndicatorSummary = ({ badges = [], ribbonAwards = [] }) => {
  ...
  const combatRibbonKeywords = [
    'COMBAT', 'CAMPAIGN', 'EXPEDITIONARY', 'WAR', 'OCCUPATION',
    'BRONZE STAR', 'PURPLE HEART', 'VALOR',
  ];
  ...
  const hasCombatIndicators = combatBadges.length > 0 || combatRibbons.length > 0;
  
  if (!hasCombatIndicators) return null;
  
  return (
    <div className="combat-indicator-summary bg-red-50 dark:bg-red-900/20 border border-red-200 
