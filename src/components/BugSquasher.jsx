...
function _detectCurrentModule(state) {
...
function _buildBugReport(formData, appState) {
  ...
  return formatBugReport(reportData);
}

function _extractReportId(generatedReport) {
...
async function _submitBugReport(formData, appState, generatedReport, ctx) {
  ctx.setSubmitting(true);
  ctx.setSubmitError("");

  try {
    const reportId = _extractReportId(generatedReport);
...
async function _copyReportToClipboard(generatedReport, setCopied) {
...
function _autoDetectModule(appState, formData, setFormData) {
  ...
  if (appState) {
    ...
    if (appState.currentModule && !formData.module) {
      ...
      const detectedModule = _detectCurrentModule(appState);
...
function _initialFormData(appState) {
  return {
...
function _computeWizardNav(step, formData, submitted, onClose, setStep) {
  ...
  return {
    canProceedCurrentStep: step === 1 ? canProceedStep1 : canProceedStep2,
    backButtonHandler,
    backButtonLabel,
  };
}
...
function BugSquasherSeverityPicker({ formData, onInputChange }) {
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {Object.entries(BUG_SEVERITY).map(([key, sev]) => (
          <button
            key={key}
            onClick={() => onInputChange("severity", sev)}
...
function BugSquasherStep1Classification({ formData, onInputChange }) {
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <input
          ...
          onChange={(e) => onInputChange("diagnosticCode", e.target.value)}
...
function BugSquasherExpectedActualFields({ formData, onInputChange }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        ...
        <textarea
          ...
          onChange={(e) => onInputChange("expectedBehavior", e.target.value)}
      ...
      <div>
        ...
        <textarea
          ...
          onChange={(e) => onInputChange("actualBehavior", e.target.value)}
...
function BugSquasherDescriptionFields({ formData, onInputChange }) {
  return (
    <>
      <div>
        {/* eslint-disable-next-line jsx-a11y/label-has-associated-control */}
        ...
        <textarea
          ...
          onChange={(e) => onInputChange("userDescription", e.target.value)}
          onKeyDown={(e) => {
      ...
      <div>
        ...
        <textarea
          ...
          onChange={(e) => onInputChange("stepsToReproduce", e.target.value)}
          placeholder="1. Go to...\n2. Click on...\n3. Enter...\n4. Bug appears when..."
          rows={4}
          className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-red-500 focus:border-transparent resize-none"
        />
      ...
      <BugSquasherExpectedActualFields
        formData={formData}
...
function BugSquasherPrivacyControls({ formData, onInputChange }) {
  return (
    <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
      ...
      <div className="space-y-2">
        {PRIVACY_CONTROL_ITEMS.map((item) => (
          ...
          >
            <input
              ...
              onChange={(e) => onInputChange(item.key, e.target.checked)}
...
function BugSquasherTrackingOptions({ formData, onInputChange }) {
  return (
    <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700 rounded-lg p-4 space-y-4">
      ...
      <label className="flex items-start gap-3 cursor-pointer">
        <input
          ...
          onChange={(e) => onInputChange("saveToMyTickets", e.target.checked)}
          className="mt-1 w-4 h-4 text-red-600 rounded focus:ring-red-500"
        />
        <div>
          <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
      ...
      <div>
        ...
        <input
          ...
          onChange={(e) => onInputChange("veteranEmail", e.target.value)}
...
function BugSquasherStep2Description({ formData, onInputChange }) {
  return (
    <div className="space-y-6">
      <BugSquasherDescriptionFields
        formData={formData}
        onInputChange={onInputChange}
      />
      <BugSquasherPrivacyControls
        formData={formData}
        onInputChange={onInputChange}
      />
      <BugSquasherTrackingOptions
        formData={formData}
        onInputChange={onInputChange}
      />
    </div>
...
function BugSquasherWhatHappensNext({ formData }) {
  return (
...
function BugSquasherRoadmapCta({ onOpenRoadmap, onClose }) {
  if (!onOpenRoadmap) return null;
  return (
...
function BugSquasherSuccessPanel({
  formData,
  generatedReport,
  onOpenRoadmap,
  onClose,
    ...
    <div className="bg-green-50 dark:bg-green-900/30 border-2 border-green-300 dark:border-green-600 rounded-lg p-6 text-center">
      ...
      <p className="text-sm text-green-600 dark:text-green-400">
        ...
        <span className="font-mono font-bold">
          {_extractReportId(generatedReport)}
        </span>
      </p>
      {formData.saveToMyTickets && (
      ...
      <BugSquasherRoadmapCta onOpenRoadmap={onOpenRoadmap} onClose={onClose} />
    </div>
  );
}

function BugSquasherReportTextarea({ generatedReport, copied, onCopyReport }) {
  return (
...
function BugSquasherSubmitButton({ submitting, onSubmitReport }) {
  return (
    <button
...
function BugSquasherReviewPanel({
  generatedReport,
  copied,
  submitError,
  submitting,
...
function BugSquasherStep3Review({
  submitted,
  formData,
  generatedReport,
  copied,
...
function BugSquasher({ onClose, appState = {}, onOpenRoadmap }) {
  ...
  // Form state
  const [formData, setFormData] = useState(() => _initialFormData(appState));

  useEffect(() => {
    _autoDetectModule(appState, formData, setFormData);
  ...
  const handleGenerateReport = () => {
    setGeneratedReport(_buildBugReport(formData, appState));
    setStep(3);
  };

  const handleSubmitReport = () =>
    _submitBugReport(formData, appState, generatedReport, {
      setSubmitting,
      setSubmitError,
      setSubmitted,
      setCopied,
  ...
  const handleCopyReport = () =>
    _copyReportToClipboard(generatedReport, setCopied);

  const { canProceedCurrentStep, backButtonHandler, backButtonLabel } =
    _computeWizardNav(step, formData, submitted, onClose, setStep);
  ...
  const footer = (
    <BugSquasherFooter
      ...
      onNext={() => (step === 2 ? handleGenerateReport() : setStep(step + 1))}
      onClose={onClose}
    />
  );
  ...
  return (
    <ResponsiveModal
      ...
      footer={footer}
      header={<BugSquasherHeader step={step} onClose={onClose} />}
    >
      {step === 1 && (
        <BugSquasherStep1Classification
          formData={formData}
