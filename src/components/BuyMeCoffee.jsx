...
function buildSearchMessages(context) {
  ...
  const conditionNameSuffix = context.conditionName
    ...
    : "";
  const packetPlural = pluralize(context.count);

  return {
    search: {
      headline: "Great find! 🎯",
      body: context.count
        ? `You found ${context.count} results${querySuffix}. You're building a strong foundation for your claim. Luna's here if you need more help.`
    ...
    "cap-sim": {
      ...
      body: context.conditionName
        ? `Your ${context.conditionName} preparation is coming along${ratingSuffix}. Practice makes confidence. You're doing great.`
    ...
    "cap-sim-complete": {
      ...
      body: context.rating
        ? `You practiced for a ${context.rating}% rating${conditionNameSuffix}. That preparation will serve you well. Deep breath - you're ready.`
    ...
    packet: {
      ...
      body: context.count
        ? `${context.count} claim${packetPlural} organized and ready. You're building something solid. One step at a time.`
...
function buildDbqLibraryMessage(context) {
  let headline;
  if (context.action === "bulk-download") {
    headline = `${context.count || "Forms"} ready! 📋`;
  } else if (context.action === "pre-fill") {
    headline = "Form prepared! ✏️";
  } else {
    headline = "Form saved! 📥";
  }
  ...
  let body;
  if (context.action === "bulk-download") {
    body = `${context.count || "All requested"} forms downloaded. Having the right paperwork ready reduces stress.`;
  } else if (context.action === "pre-fill") {
    body = `${context.formName || "Your DBQ"} is pre-filled and ready for your doctor. You're making their job easier.`;
  } else {
    body = `${context.formName || "This form"} is saved for offline access. Prepared for anything.`;
  }

  const icon = context.action === "bulk-download" ? "📦" : "📋";

  return { headline, body, icon };
}

function buildDocumentMessages(context) {
  const exportPlural = pluralize(context.count);
  ...
  return {
    ...
    export: {
      ...
      body: context.count
        ? `${context.count} claim${exportPlural} safely backed up. Your hard work is protected. Peace of mind matters.`
    ...
    "dbq-library": buildDbqLibraryMessage(context),
  };
}

function buildClaimMessages(context) {
  const riskPlural = pluralize(context.risks);
  ...
  return {
    ...
    "risk-assessment": {
      ...
      body: context.risks
        ? `${context.risks} area${riskPlural} identified for strengthening. Better to know now than be surprised later.`
...
function buildSupportMessages(context) {
  const symptomPlural = pluralize(context.count);

  return {
    "symptom-logger": {
      headline: "Symptoms recorded! 📝",
      body: context.count
        ? `${context.count} symptom${symptomPlural} documented. Consistent logging builds strong evidence over time.`
...
function buildMessages(context) {
  return {
    ...buildSearchMessages(context),
    ...buildDocumentMessages(context),
    ...buildClaimMessages(context),
    ...buildSupportMessages(context),
  };
}
