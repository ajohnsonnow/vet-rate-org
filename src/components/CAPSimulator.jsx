...
const STRATEGIC_TIPS = {
  prostrating: {
    ...
    content:
      "The CFR defines 'prostrating' as attacks so severe you MUST stop all activity and lie down, usually in a dark/quiet room. If you can 'power through' the pain, it's NOT prostrating. Be honest-if you sometimes have to lie down, say that specifically.",
  },
  rom: {
    title: "Range of Motion (ROM) Testing",
    content:
      "Stop moving EXACTLY when you first feel pain or discomfort. Do NOT push past the pain to show the examiner you're 'trying.' If you demonstrate a full range of motion, they will mark you as 'Normal' regardless of how much it hurts.",
  },
  flare_ups: {
    title: "Flare-Ups Matter More Than You Think",
    content:
      "The VA rates you based on your WORST flare-ups, not your average day. If your back 'locks up' 3-4 times per year requiring bed rest, that's a flare-up. Document the frequency, duration, and what triggers them.",
  },
  social_impairment: {
    title: "Occupational and Social Impairment Keywords",
    content:
      "For mental health claims, use these specific terms if they apply to you: 'panic attacks,' 'memory loss,' 'difficulty concentrating,' 'suicidal ideation,' 'neglecting hygiene,' 'inability to establish relationships.' These are the exact phrases in the rating criteria.",
  },
  medication_side_effects: {
    title: "Medication Side Effects Count",
    content:
      "The medications you take for your service-connected condition can affect your rating. Mention side effects like: drowsiness affecting work, weight gain, sexual dysfunction, GI distress. These are 'residuals of treatment' and factor into your rating.",
  },
  sleep_disturbance: {
    title: "Sleep Issues Are Powerful Evidence",
    content:
      "Chronic sleep impairment affects nearly every condition rating. Be specific: How many hours do you sleep? Do you wake up? How often? Do you have nightmares? Sleep separately from your spouse? This impacts both mental and physical ratings.",
  },
  frequency: {
    title: "Frequency Determines Your Rating",
    content:
      "Don't just say 'often' or 'sometimes.' The examiner needs specifics: 'Once per month,' 'Three times per week,' '10-15 episodes per year.' Keep a symptom log for 30 days before your exam if possible.",
  },
  loss_of_use: {
    title: "Loss of Use = Higher Rating",
    content:
      "If you can't perform a specific function (e.g., can't grip tools, can't squat, can't climb stairs), say that explicitly. Partial loss of use still qualifies. Example: 'I can no longer tie my shoes without assistance' is more powerful than 'My hands hurt.'",
  },
};
...
function _qFlareups() {
  return {
    ...
    intent:
      "Flare-ups are critical - the examiner MUST estimate additional ROM loss during flare-ups per Sharp v. Shulkin.",
    definition:
      "Flare-ups: Periods where symptoms are significantly worse than baseline. The examiner should estimate in degrees how much additional motion is lost during flare-ups.",
    options: [
      {
        value: "severe",
        label:
          "Yes - severe flare-ups that significantly worsen function (weekly or more)",
        weight: 4,
      },
      {
        value: "moderate",
        label: "Yes - moderate flare-ups (several times per month)",
        weight: 3,
      },
      {
        value: "mild",
        label: "Yes - occasional flare-ups (few times per month)",
        weight: 2,
      },
      {
        value: "rare",
        label: "Rare flare-ups (few times per year)",
        weight: 1,
      },
...
function _skinQuestions(_conditionName, _allCriteriaText) {
  let questions = [];
  questions = [
    {
      id: "q_body_area",
      ...
      definition:
        "Exposed areas: head, face, neck, hands. Total body area considers all skin involvement.",
      options: [
        {
          value: "more_40",
          label: "More than 40% of entire body OR exposed areas affected",
          weight: 4,
        },
        {
          value: "20_40",
          label: "20-40% of entire body OR exposed areas affected",
          weight: 3,
        },
        {
          value: "5_20",
          label: "5-20% of entire body OR exposed areas affected",
          weight: 2,
        },
        {
          value: "less_5",
          label: "Less than 5% of body affected",
          weight: 1,
        },
        {
          value: "minimal",
          label: "Minimal or no visible involvement",
          weight: 0,
        },
      ],
      required: true,
    },
    {
      ...
      intent:
        "Treatment type (systemic vs topical) is a key rating factor for skin conditions.",
      definition:
        "Systemic therapy: oral or injected medications. Topical: creams/ointments applied to skin.",
      options: [
        {
          value: "constant_systemic",
          label:
            "Constant or near-constant systemic therapy (immunosuppressives, etc.)",
          weight: 4,
        },
        {
          value: "intermittent_systemic",
          label:
            "Intermittent systemic therapy during the past 12-month period",
          weight: 3,
        },
...
const generateGenericQuestions = (condition) => {
  ...
  // === Body-system-specific question sets ===
  if (bodySystem === "musculoskeletal") {
    questions = _musculoskeletalQuestions(conditionName, allCriteriaText);
  } else if (bodySystem === "mental") {
    questions = _mentalHealthQuestions(conditionName, allCriteriaText);
  } else if (bodySystem === "respiratory") {
    questions = _respiratoryQuestions(conditionName, allCriteriaText);
  } else if (bodySystem === "neurological") {
    questions = _neurologicalQuestions(conditionName, allCriteriaText);
  } else if (bodySystem === "cardiovascular") {
    questions = _cardiovascularQuestions(conditionName, allCriteriaText);
  } else if (bodySystem === "digestive") {
    questions = _digestiveQuestions(conditionName, allCriteriaText);
  } else if (bodySystem === "skin") {
    questions = _skinQuestions(conditionName, allCriteriaText);
  } else if (bodySystem === "ear") {
    questions = _earQuestions(conditionName, allCriteriaText);
  }

  // === DEFAULT/GENERAL QUESTIONS ===
  // If no body system matched or we need more questions, add general ones
  _addGeneralFallbackQuestions(conditionName, questions);
...
function _computePredictedRating(answers, avgWeight, ratingKeys, conditionNameLower) {
  ...
  if (conditionNameLower.includes("tinnitus")) {
    ...
    const hasTinnitus = Object.values(answers).some(
      (v) =>
        v === "constant" ||
        v === "recurrent" ||
        v === "severe" ||
    ...
    return hasTinnitus ? 10 : 0;
  }

  if (ratingKeys.length === 0) {
    // No ratings defined - fall back to generic scale
    if (avgWeight >= 3.5) return 100;
    if (avgWeight >= 2.5) return 50;
    if (avgWeight >= 1.5) return 30;
    if (avgWeight >= 0.5) return 10;
    return 0;
  }

  if (ratingKeys.length === 1) {
    // Only one rating level available
    return avgWeight >= 1 ? ratingKeys[0] : 0;
  }
  ...
  return ratingKeys[levelIndex] || 0;
}

function _buildRatingGaps(predictedRating, ratingKeys, ratings) {
  const gaps = [];
...
function _buildRatingWarnings(condition, ratingSchedule, diagnosticCode, conditionName) {
  ...
  if (
    condition.ratingCriteria?.notes &&
    condition.ratingCriteria.notes.length > 0
  ) {
...
const calculateGenericRating = (answers, condition) => {
  ...
  const predictedRating = _computePredictedRating(
    answers,
    avgWeight,
    ratingKeys,
    conditionNameLower,
  ...
  const gaps = _buildRatingGaps(predictedRating, ratingKeys, ratings);
  const actionItems = _buildExamActionItems(conditionNameLower);

  const diagnosticCode = condition.diagnosticCode || "";
  const ratingSchedule = condition.ratingSchedule || "38 CFR Part 4";
  const conditionName =
    condition.conditionName || condition.condition_name || "this condition";
  const warnings = _buildRatingWarnings(
    condition,
    ratingSchedule,
    diagnosticCode,
    conditionName,
...
function CAPExamPrepListView({
  ...
  const handleExamPrepConditionSelect = (conditionKey) => {
    ...
    setExamPrepTips(
      tips.map((tipKey) => ({ key: tipKey, ...STRATEGIC_TIPS[tipKey] })),
    );
    ...
    terms: [
      {
        ...
        definition:
          'A medical examination conducted by a VA or contracted examiner to evaluate the severity of your claimed conditions. The examiner\'s findings directly impact your disability rating. Also called a "comp and pen" exam.',
        example:
          "I have a C&P exam scheduled for my back condition next Tuesday. The examiner will measure my range of motion and document my symptoms.",
      },
      {
        term: "DBQ (Disability Benefits Questionnaire)",
        definition:
          "A standardized form examiners use to document your condition. Each condition has a specific DBQ with required measurements and questions. Veterans can have private doctors complete DBQs to support claims.",
        example:
          'My private orthopedist completed the "Back (Thoracolumbar Spine) Conditions DBQ" documenting my limited flexion and pain levels.',
      },
      {
        term: "Rating Decision",
        definition:
          "The official VA document that explains the decision on your claim, including assigned percentages, effective dates, and reasoning. This is what you appeal if you disagree.",
        example:
          "My rating decision granted 20% for my back but denied my sleep apnea claim, citing lack of nexus to service.",
      },
      {
        term: "Combined Rating",
        definition:
          'How VA calculates total disability when you have multiple conditions. NOT simple addition - uses "whole person theory" where each additional rating applies to remaining non-disabled percentage. 50% + 30% = 65%, not 80%.',
        example:
          "I have 50% for PTSD and 30% for back pain. My combined rating is 65% (50% + 30% of remaining 50% = 50% + 15% = 65%).",
      },
      {
        term: "Bilateral Factor",
        definition:
          "A 10% bonus added to combined ratings when you have the same type of condition affecting paired body parts (both knees, both arms, both ears). Applied before combining with other ratings.",
        example:
          "I have 10% for my right knee and 10% for my left knee. With the bilateral factor, these combine to 21% before adding my other conditions.",
      },
      {
        term: "Effective Date",
        definition:
          "The date your disability rating begins, which determines when back-pay starts. Usually the date VA received your claim, or date of increase if filing for higher rating.",
        example:
          "My effective date is March 15, 2024 - the day I filed my intent to file. I received 8 months of back-pay.",
      },
      {
        term: "Intent to File (ITF)",
        definition:
          "A submission that reserves your effective date for up to one year while you gather evidence. Crucial for protecting back-pay. Must be followed by actual claim within 12 months.",
        example:
          "I submitted an Intent to File on January 1st. Even though I did not file my full claim until June, my effective date will be January 1st.",
      },
    ],
  },
  {
    ...
    terms: [
      {
        ...
        definition:
          "The legal link between a current disability and military service. Required for any VA compensation. Can be direct (happened in service), secondary (caused by service-connected condition), or presumptive.",
        example:
          "The VA granted service connection for my hearing loss because my audiograms show worsening during service and my MOS involved noise exposure.",
      },
      {
        term: "Direct Service Connection",
        definition:
          "When a condition was directly caused by an event, injury, or illness during military service. Requires: current diagnosis, in-service event, and nexus linking them.",
        example:
          "I hurt my back during a training exercise in 2010. My medical records show treatment in service. This is direct service connection.",
      },
      {
        term: "Secondary Service Connection",
        definition:
          "When an already service-connected condition causes or aggravates a new condition. Does NOT require the new condition to have occurred during service.",
        example:
          "My service-connected right knee injury caused me to walk with a limp, which led to left hip arthritis. The hip condition is secondary to the knee.",
      },
      {
        term: "Presumptive Service Connection",
        definition:
          "Conditions the VA presumes are connected to service based on when/where you served, without requiring individual proof. Includes Agent Orange conditions, Gulf War illnesses, Camp Lejeune diseases, and chronic diseases appearing within one year of discharge.",
        example:
          "As a Vietnam veteran with Type 2 diabetes, I qualify for presumptive service connection due to Agent Orange exposure - I did not need to prove the specific exposure.",
      },
      {
        term: "Aggravation",
        definition:
          "When service permanently worsened a pre-existing condition beyond its natural progression. The VA must rate the degree of aggravation, not the entire condition.",
        example:
          "I had mild asthma before service, but exposure to burn pits made it severe. The VA rated the aggravation - the worsening beyond what would have happened naturally.",
      },
      {
        term: "Nexus",
        definition:
          'The medical link connecting your current disability to military service. Often requires a medical opinion stating the condition is "at least as likely as not" (50% or greater probability) related to service.',
        example:
          "My doctor wrote a nexus letter stating my sleep apnea is at least as likely as not caused by my service-connected PTSD, citing medical literature on the connection.",
      },
      {
        term: "Nexus Letter",
        definition:
          'A written medical opinion from a doctor establishing the connection between your condition and service. Should include rationale, cite medical evidence, and use the "at least as likely as not" standard.',
        example:
          'My nexus letter states: "It is my medical opinion that the veteran\'s current lumbar degenerative disc disease is at least as likely as not related to the documented in-service back injury in 2008."',
      },
    ],
  },
  {
    ...
    terms: [
      {
        ...
        definition:
          "How far a joint can move, measured in degrees. Primary rating factor for spine and joint conditions. CRITICAL: Measured to the point where pain STOPS you, not where you can force yourself to go.",
        example:
          "When bending forward, I can only reach about knee-level before sharp back pain forces me to stop - that is about 60 degrees of flexion. Normal is 90 degrees.",
      },
      {
        term: "Flexion",
        definition:
          "Bending movement that decreases the angle between body parts. For the spine, this is bending forward. For the knee, this is bending the leg back.",
        example:
          "My lumbar flexion is limited to 45 degrees due to pain, where normal is 90 degrees. This limitation affects my 20% spine rating.",
      },
      {
        term: "Extension",
        definition:
          "Straightening movement that increases the angle between body parts. For the spine, this is bending backward. Limited extension affects ratings.",
        example:
          "I can barely extend my back at all - only about 10 degrees before severe pain. Normal extension is 30 degrees.",
      },
      {
        term: "Ankylosis",
        definition:
          'Complete fixation of a joint in one position - the joint cannot move at all. Can be "favorable" (functional position) or "unfavorable" (non-functional). Warrants highest ratings.',
        example:
          "My cervical spine is ankylosed in a forward-flexed position - I cannot look straight ahead. This unfavorable ankylosis warrants a 40% rating.",
      },
      {
        term: "Flare-ups",
        definition:
          "Periods when symptoms are significantly worse than baseline. Per Sharp v. Shulkin, examiners MUST estimate additional functional loss during flare-ups, even if not occurring during the exam.",
        example:
          "During flare-ups, which happen 2-3 times per month, my back pain increases from 5/10 to 9/10 and I lose an additional 20 degrees of flexion.",
      },
      {
        term: "Functional Loss",
        definition:
          "Impairment beyond just limited motion - includes pain, weakness, fatigability, incoordination, and lack of endurance. Per DeLuca v. Brown, must be considered even if ROM appears adequate.",
        example:
          "Even though my knee bends to 120 degrees, I have significant functional loss due to pain, giving way, and inability to stand for more than 10 minutes.",
      },
      {
        term: "Prostrating",
        definition:
          'So severe that physical activity must stop and the person must lie down, usually in a dark, quiet room. Used in migraine ratings - the CFR requires attacks to be "completely prostrating" for 30% or higher.',
        example:
          "When I have a prostrating migraine, I cannot work, drive, or do anything except lie in bed with the lights off for 4-8 hours.",
      },
      {
        term: "Incapacitating Episodes",
        definition:
          "For IVDS and some other conditions: periods requiring bed rest PRESCRIBED BY A PHYSICIAN. The VA specifically requires doctor-prescribed bed rest, not just self-imposed rest.",
        example:
          "My doctor has prescribed bed rest for my back 4 times this year, totaling about 5 weeks. This qualifies me for a 40% IVDS rating.",
      },
      {
        term: "Radiculopathy",
        definition:
          "Nerve root damage causing pain, numbness, tingling, or weakness along a nerve pathway. Often radiates from spine down an arm or leg. Can be rated SEPARATELY from the spine condition causing it.",
        example:
          "I have radiculopathy from my lumbar disc herniation - shooting pain and numbness from my lower back down to my right foot, with weakness lifting my toes.",
      },
      {
        term: "Intervertebral Disc Syndrome (IVDS)",
        definition:
          "Condition involving spinal disc damage causing neurological symptoms and incapacitating episodes. Can be rated under ROM criteria OR incapacitating episode criteria - whichever is higher.",
        example:
          "My IVDS is rated at 40% based on incapacitating episodes totaling 4-6 weeks per year, which is higher than my ROM-based rating would be.",
      },
      {
        term: "Peripheral Neuropathy",
        definition:
          "Nerve damage in the extremities causing numbness, tingling, burning, or weakness. Common in diabetics and toxic exposure veterans. Rated based on severity: mild, moderate, or severe incomplete paralysis.",
        example:
          "I have moderate peripheral neuropathy in both feet from Agent Orange exposure - constant numbness and burning that affects my balance.",
      },
      {
        term: "METs (Metabolic Equivalents)",
        definition:
          "Measure of exercise capacity used for heart condition ratings. 1 MET = sitting quietly. 3-5 METs = light housework. 7-10 METs = jogging. Lower METs = higher rating.",
        example:
          "My stress test showed I can only achieve 4 METs before symptoms - I get short of breath just doing light housework. This supports a 60% heart rating.",
      },
    ],
  },
  {
    ...
    terms: [
      {
        ...
        definition:
          'How mental health symptoms affect ability to work. Key factor in ratings. Ranges from "occasional decrease in work efficiency" (10-30%) to "total occupational impairment" (100%).',
        example:
          "My PTSD causes me to miss work 2-3 days per month, reduced productivity, conflicts with supervisors, and I have been demoted twice. This is significant occupational impairment.",
      },
      {
        term: "Social Impairment",
        definition:
          "How mental health symptoms affect relationships and social functioning. Rated alongside occupational impairment. Includes family relationships, friendships, and community involvement.",
        example:
          "I avoid all social gatherings, have no close friends, and my marriage nearly ended due to my irritability and isolation from PTSD.",
      },
      {
        term: "GAF Score (Global Assessment of Functioning)",
        definition:
          "A 0-100 scale previously used to rate overall psychological functioning. Though officially discontinued, still appears in older records. Lower scores = more severe impairment.",
        example:
          "My psychiatrist assigned a GAF of 45, indicating serious symptoms like suicidal ideation and major impairment in work and relationships.",
      },
      {
        term: "Suicidal Ideation",
        definition:
          'Thoughts about suicide, ranging from passive ("I wish I was not alive") to active with a plan. A key symptom in 70% and 100% mental health ratings. Must be documented to count.',
        example:
          "I have passive suicidal ideation - I sometimes think my family would be better off without me, though I have no specific plan.",
      },
      {
        term: "Hypervigilance",
        definition:
          "A state of elevated alertness and scanning for threats, common in PTSD. Causes exhaustion, anxiety, and difficulty relaxing even in safe environments.",
        example:
          "I cannot sit with my back to a door in restaurants. I constantly scan for exits and threats. I have not slept more than 4 hours straight since deployment.",
      },
      {
        term: "Intrusive Thoughts",
        definition:
          "Unwanted, distressing thoughts or memories that enter the mind involuntarily. In PTSD, often involves reliving traumatic events. Different from flashbacks.",
        example:
          "Multiple times daily, I am suddenly overwhelmed by memories of the IED attack. I can be in the middle of a work meeting and suddenly I am back there.",
      },
      {
        term: "Flashbacks",
        definition:
          "Dissociative episodes where the person feels or acts as if the traumatic event is happening again. More severe than intrusive thoughts - involves temporarily losing awareness of present surroundings.",
        example:
          "Loud noises trigger flashbacks where I actually believe I am back in combat. I have hit the ground in public parking lots when cars backfire.",
      },
      {
        term: "Avoidance Behaviors",
        definition:
          "Deliberate efforts to avoid reminders of trauma - places, people, activities, thoughts. A core PTSD symptom that significantly impacts functioning.",
        example:
          "I cannot watch war movies, avoid crowded places, no longer attend fireworks events with my kids, and cut off contact with military friends.",
      },
      {
        term: "Panic Attacks",
        definition:
          "Sudden episodes of intense fear with physical symptoms: racing heart, sweating, trembling, shortness of breath. For 50% or higher MH ratings, should occur weekly or more.",
        example:
          "I have panic attacks 2-3 times per week, usually triggered by crowds. My heart races, I cannot breathe, and I have to leave immediately.",
      },
      {
        term: "Impaired Impulse Control",
        definition:
          "Difficulty controlling behaviors or emotional reactions. Can manifest as unprovoked irritability, angry outbursts, or reckless behavior. Listed in 70% MH rating criteria.",
        example:
          "I have thrown objects during arguments, punched walls, and have received warnings at work for angry outbursts at coworkers over minor issues.",
      },
    ],
  },
  {
    ...
    terms: [
      {
        ...
        definition:
          'The legal standard for service connection - means 50% or greater probability. If medical evidence shows your condition is "at least as likely as not" related to service, the VA must grant service connection.',
        example:
          'My doctor\'s nexus letter states my sleep apnea is "at least as likely as not" caused by my PTSD. This meets the legal standard for secondary service connection.',
      },
      {
        term: "Benefit of the Doubt",
        definition:
          'When evidence is in "approximate balance" (roughly 50-50), the VA must decide in the veteran\'s favor. You do not need to prove your case beyond doubt - just reach equipoise.',
        example:
          "Even though the evidence was not overwhelming, it was about 50-50 on whether my condition was service-related, so the benefit of the doubt rule worked in my favor.",
      },
      {
        term: "Competent Evidence",
        definition:
          "Evidence from a qualified source. Medical diagnoses require medical professionals. But veterans are competent to report symptoms they personally experience (pain, difficulty sleeping, etc.).",
        example:
          "I am competent to state that I have had knee pain since service - I do not need a doctor to say I feel pain. But diagnosing the cause requires medical evidence.",
      },
      {
        term: "Credible Evidence",
        definition:
          "Evidence that is believable and consistent. Your statements must be internally consistent and consistent with other evidence. Inconsistencies harm credibility.",
        example:
          "My buddy statements, service records, and personal testimony all consistently describe my back injury during training - this consistent story is credible evidence.",
      },
      {
        term: "Lay Evidence",
        definition:
          "Non-medical evidence from the veteran or others. Includes your own statements about symptoms, buddy statements from fellow service members, and family observations. Valuable for continuity.",
        example:
          "My wife submitted a lay statement describing how my sleep apnea symptoms started right after I returned from deployment and have continued since.",
      },
      {
        term: "Buddy Statement",
        definition:
          "A written statement from someone who witnessed your condition or injury, usually a fellow service member. Very valuable for documenting in-service events when official records are incomplete.",
        example:
          "My buddy wrote a statement describing how he witnessed me fall during the training exercise and helped carry me to medical - this corroborates my claim.",
      },
      {
        term: "Medical Opinion",
        definition:
          "A healthcare provider's professional judgment about diagnosis, causation, or severity. Should include rationale based on examination, medical records, and accepted medical principles.",
        example:
          "The C&P examiner's medical opinion stated my tinnitus is less likely than not related to service because there was no documented noise exposure - I need to rebut this.",
      },
      {
        term: "Adequate Rationale",
        definition:
          'A medical opinion must explain WHY the examiner reached their conclusion. An opinion without explanation ("it is not related to service") has little value and can be challenged.',
        example:
          'The examiner\'s opinion lacked adequate rationale - he just checked "not related" without explaining why. This gives me grounds to request a new exam.',
      },
    ],
  },
  {
    ...
    terms: [
      {
        ...
        definition:
          "A claim filed after a denial that includes new and relevant evidence not previously considered. Best path when you have new medical evidence or nexus letters.",
        example:
          "After my denial, I got a nexus letter from a private doctor. I filed a supplemental claim with this new evidence and was granted service connection.",
      },
      {
        term: "Higher-Level Review (HLR)",
        definition:
          "Request for a senior VA employee to review a decision for clear and obvious errors. No new evidence allowed - just a fresh look at existing evidence. Good for examiner errors.",
        example:
          "The examiner ignored my documented in-service injury. I requested an HLR arguing there was clear error, and they granted my claim.",
      },
      {
        term: "Board Appeal (BVA)",
        definition:
          "Appeal to the Board of Veterans Appeals. Options: direct review, evidence submission, or hearing with a Veterans Law Judge. Takes longer but can add new evidence.",
        example:
          "After exhausting other options, I appealed to the Board and requested a hearing. The judge understood my case and granted my appeal.",
      },
      {
        term: "Clear and Unmistakable Error (CUE)",
        definition:
          "A very high standard for correcting final decisions. Must show the correct facts were known but the law was incorrectly applied, and the outcome would definitely have been different.",
        example:
          "The VA failed to apply the bilateral factor to my knees in 2018. This is CUE because the law clearly requires it and it definitely changed my rating.",
      },
      {
        term: "Deferred",
        definition:
          "When VA postpones a decision on part of your claim, usually pending additional evidence or examination. Your other claims may be decided while deferred claims wait.",
        example:
          "My PTSD claim was granted, but my sleep apnea claim was deferred pending a nexus opinion from a specialist.",
      },
      {
        term: "Duty to Assist",
        definition:
          "VA's legal obligation to help you develop your claim by obtaining relevant records, scheduling exams, and notifying you of evidence needed. But you must cooperate.",
        example:
          "Under duty to assist, the VA obtained my military personnel records and scheduled a C&P exam. I did not have to pay for any of this.",
      },
      {
        term: "TDIU (Total Disability Individual Unemployability)",
        definition:
          "A 100% payment rate for veterans whose service-connected conditions prevent substantial gainful employment, even if combined rating is less than 100%. Requires one condition at 60% or combined 70%.",
        example:
          "My combined rating is 80%, but my PTSD makes it impossible to hold a job. I applied for TDIU and now receive compensation at the 100% rate.",
      },
      {
        term: "Special Monthly Compensation (SMC)",
        definition:
          "Additional compensation beyond schedular ratings for specific circumstances: loss of limbs, blindness, need for aid and attendance, housebound status, or having multiple 100% ratings.",
        example:
          "I receive SMC(s) because I need my wife's help with bathing and dressing due to my service-connected conditions.",
      },
    ],
  },
  {
    ...
    terms: [
      {
        ...
        definition:
          "For 50% migraine rating: attacks are so severe and frequent they prevent maintaining employment or cause significant income loss. Must show actual work impact, not just severe headaches.",
        example:
          "I have missed so much work due to migraines that I was placed on a performance improvement plan and fear losing my job.",
      },
      {
        term: "Characteristic Prostrating Attacks",
        definition:
          'Migraine term: attacks so severe you must stop all activity and lie down. For 30% rating, need these monthly. For 50%, need them to be "very frequent" AND cause economic inadaptability.',
        example:
          "I have characteristic prostrating attacks about twice per month - each time I must leave work and lie in a dark room for hours.",
      },
      {
        term: "Guarding",
        definition:
          "For spine ratings: muscle spasm or guarding severe enough to result in abnormal gait or abnormal spinal contour. Required for 20% lumbosacral rating with normal ROM.",
        example:
          "My back muscles spasm so badly that my spine curves to one side and I walk bent forward. This guarding affects my posture and gait.",
      },
      {
        term: "Abnormal Spinal Contour",
        definition:
          "Visible abnormality in spinal curvature - scoliosis, reversed lordosis, abnormal kyphosis. Can result from muscle spasm/guarding. Relevant to spine ratings.",
        example:
          "X-rays show I have reversed lordosis in my lumbar spine - it curves the wrong direction due to muscle spasm. This is abnormal spinal contour.",
      },
      {
        term: "Painful Motion",
        definition:
          "Per 38 CFR 4.59, joints that are actually painful, unstable, or misaligned due to healed injury warrant at least the minimum compensable rating. Pain itself can support 10%.",
        example:
          "Even though my knee has full range of motion, I have constant pain with movement. Under 4.59, this painful motion supports at least a 10% rating.",
      },
      {
        term: "Recurrent Subluxation/Instability",
        definition:
          "For knee ratings: knee giving way or partially dislocating. Rated separately from limitation of motion - can have both ratings for the same knee.",
        example:
          "My knee gives out unexpectedly 2-3 times per week, especially on stairs. This instability is rated at 20% separately from my flexion limitation.",
      },
      {
        term: "Degenerative Arthritis (DJD)",
        definition:
          "Arthritis from wear and tear, confirmed by X-ray. If ROM is not compensable, rate at 10% per major joint group with X-ray evidence. Higher if motion is limited.",
        example:
          "My X-rays show bone-on-bone arthritis in my knee. Even with good ROM, the X-ray-confirmed arthritis warrants at least 10%.",
      },
      {
        term: "Complete vs Incomplete Paralysis",
        definition:
          "For nerve conditions: complete paralysis means total loss of function. Incomplete paralysis has grades: mild, moderate, moderately severe, and severe. Rating depends on which nerve and degree.",
        example:
          "I have moderate incomplete paralysis of the sciatic nerve - significant weakness and numbness but not total loss of function. This rates at 20%.",
      },
    ],
  },
  {
    ...
    terms: [
      {
        ...
        definition:
          "Conditions presumed connected to herbicide exposure in Vietnam, Thailand, Korea DMZ, and certain C-123 aircraft. Includes Type 2 diabetes, ischemic heart disease, Parkinson's, and multiple cancers.",
        example:
          "As a Vietnam veteran, my Type 2 diabetes is presumptively service-connected. I did not have to prove exactly when or how I was exposed to Agent Orange.",
      },
      {
        term: "Gulf War Presumptives",
        definition:
          "Undiagnosed illnesses and medically unexplained chronic multi-symptom illnesses in Gulf War veterans. Includes chronic fatigue, fibromyalgia, IBS, and functional GI disorders.",
        example:
          "My chronic fatigue syndrome is presumptively connected to my Gulf War service even though doctors cannot explain what caused it.",
      },
      {
        term: "PACT Act Presumptives",
        definition:
          "Conditions added by the 2022 PACT Act related to toxic exposures: burn pit exposure, radiation, Agent Orange (expanded), and more. Created many new presumptive conditions.",
        example:
          "Under the PACT Act, my constrictive bronchiolitis from burn pit exposure is now presumptive. I deployed to Iraq and have the diagnosis.",
      },
      {
        term: "Camp Lejeune Presumptives",
        definition:
          "Conditions linked to contaminated water at Camp Lejeune (1953-1987). Includes multiple cancers, Parkinson's, kidney disease, and other conditions for those who served 30+ days.",
        example:
          "I was stationed at Camp Lejeune for 2 years in the 1970s. My bladder cancer is presumptively connected to the water contamination.",
      },
      {
        term: "Chronic Disease Presumption",
        definition:
          "Certain chronic diseases (arthritis, heart disease, diabetes, etc.) are presumptively connected if they manifest to 10% degree within one year of discharge.",
        example:
          "My arthritis was diagnosed 8 months after discharge. Because it's a chronic disease that manifested within one year, it's presumptively connected.",
      },
      {
        term: "Combat Presumption (1154(b))",
        definition:
          "For combat veterans, satisfactory lay evidence of injury/disease is accepted even without official records if consistent with combat circumstances. Lowers the evidence burden.",
        example:
          "I have no medical records of my shoulder injury during combat, but my account is consistent with combat circumstances. Under 1154(b), this is accepted as evidence.",
      },
    ],
  },
  {
    ...
    terms: [
      {
        ...
        definition:
          "When evidence is in approximate balance (50/50), VA must decide in the veteran's favor. You do NOT need to prove your case beyond reasonable doubt - just reach equipoise (50% probability).",
        example:
          'My doctor said my condition was "at least as likely as not" related to service. Even though the C&P examiner disagreed, the evidence was roughly equal, so under §3.102, I should win.',
      },
      {
        term: "Duty to Assist (§3.159)",
        definition:
          "VA's legal obligation to help develop your claim: obtain service records, VA records, and private records (with authorization), schedule C&P exams, and notify you of needed evidence.",
        example:
          "VA denied my claim without getting my service treatment records. They violated their duty to assist under §3.159. I appealed citing this failure.",
      },
      {
        term: "Intent to File (§3.155(b))",
        definition:
          "A submission that reserves your effective date for up to 1 year while you gather evidence. Can be filed electronically, on VA Form 21-0966, or verbally to VA. Preserves back-pay rights.",
        example:
          "I filed an Intent to File on January 1st. Even though my full claim wasn't ready until June, my effective date will be January 1st - preserving 5 months of back-pay.",
      },
      {
        term: "Complete Claim (§3.160)",
        definition:
          "A claim with all required elements: claimant's name, service information, signature, benefit sought, and condition description. Incomplete claims cause delays.",
        example:
          "My claim was returned as incomplete because I forgot to sign it. Always double-check you've provided everything required.",
      },
      {
        term: "Favorable Findings (§3.104(c))",
        definition:
          "Any favorable finding by a VA adjudicator is binding on all future adjudicators unless clear and unmistakable error is shown. Past favorable findings carry forward.",
        example:
          "In 2018, VA acknowledged my in-service back injury even though they denied service connection then. That finding is binding - new adjudicators cannot reverse it.",
      },
      {
        term: "Rating Stabilization (§3.344)",
        definition:
          "Ratings in effect 5+ years cannot be reduced unless sustained improvement under ordinary conditions of life is shown. After 20 years, ratings are essentially permanent.",
        example:
          "I've had 70% for PTSD for 8 years. VA can't reduce it just because one exam looked better - they must show sustained improvement in my daily life.",
      },
      {
        term: "Clear and Unmistakable Error (CUE) (§3.105)",
        definition:
          "A high standard for correcting final decisions. Must show correct facts were known but law was incorrectly applied, and outcome would undoubtedly have been different. Can recover decades of back-pay.",
        example:
          "VA failed to apply the bilateral factor to my knees in 2010. This is CUE because the law clearly requires it. I filed a CUE claim and got 15 years of back-pay.",
      },
      {
        term: "Effective Date Rules (§3.400)",
        definition:
          "When benefits start. Usually date VA received claim OR date entitlement arose, whichever is LATER. Can be earlier with Intent to File or claim within 1 year of discharge.",
        example:
          "I filed within 1 year of discharge, so my effective date is the day after discharge. Veterans who wait lose months or years of back-pay.",
      },
      {
        term: "Review Options (§3.2500)",
        definition:
          "After a decision, you have 1 year to choose: Supplemental Claim (add evidence), Higher-Level Review (error review), or Board Appeal. Each preserves different rights.",
        example:
          "I got denied and had new evidence, so I filed a Supplemental Claim within 1 year. My original effective date was preserved when I won.",
      },
      {
        term: "Secondary Service Connection (§3.310)",
        definition:
          "A disability caused OR aggravated by an already service-connected condition. Does NOT need to occur during service. Medical evidence must show the connection.",
        example:
          "My service-connected diabetes caused peripheral neuropathy in my feet. Under §3.310, the neuropathy is secondary service-connected.",
      },
      {
        term: "Special Monthly Compensation (§3.350)",
        definition:
          "Additional compensation beyond schedular ratings for: loss of use of limbs/organs, blindness, deafness, being housebound, or needing aid and attendance.",
        example:
          "I qualify for SMC(s) because I have 100% for PTSD plus 60% for other conditions that are separate and distinct. That's an extra ~$400/month.",
      },
      {
        term: "Procedural Due Process (§3.103)",
        definition:
          "Your rights as a claimant: written notice of decisions, right to a hearing, right to representation. VA must assist you and grant every benefit supported by law.",
        example:
          "VA decided my claim without telling me what evidence I needed. They violated my due process rights under §3.103. I appealed based on this procedural error.",
      },
    ],
  },
  {
    ...
    terms: [
      {
        ...
        definition:
          "The form (VA Form 10182) you file to appeal a VA decision to the Board of Veterans' Appeals. Must be filed within 1 year of the decision. You must choose one of three dockets.",
        example:
          "VA denied my claim, and I believe they misread the evidence. I filed a Notice of Disagreement within the 1-year deadline to take my case to the Board.",
      },
      {
        term: "Direct Review Docket",
        definition:
          "One of three BVA docket options. The Board reviews your existing record without new evidence or a hearing. Fastest option - typically under 1 year wait time.",
        example:
          "My evidence was strong but the regional office misapplied the law. I chose Direct Review because I didn't need to add anything - just needed a fresh look.",
      },
      {
        term: "Evidence Submission Docket",
        definition:
          "One of three BVA docket options. You can submit new evidence within 90 days of filing, but no hearing. Wait time typically 1-2 years.",
        example:
          "I got a new nexus letter after my denial. I chose Evidence Submission docket so I could add this new evidence to my appeal.",
      },
      {
        term: "Hearing Request Docket",
        definition:
          "One of three BVA docket options. You testify before a Veterans Law Judge and can submit evidence at the hearing. Longest wait (2-4 years) but most comprehensive.",
        example:
          "My case is complex and I want to explain my symptoms directly to the judge. I chose Hearing Request even though it takes longer.",
      },
      {
        term: "Veterans Law Judge (VLJ)",
        definition:
          "An attorney appointed by the VA Secretary who decides Board appeals. VLJs are required to be experienced in veterans law and conduct hearings.",
        example:
          "At my Board hearing, the Veterans Law Judge asked clarifying questions and really seemed to understand my condition.",
      },
      {
        term: "Remand",
        definition:
          "When the Board sends your case back to the regional office for additional development (like a new exam) or to fix errors. NOT a denial - often leads to a grant.",
        example:
          "The Board remanded my case because VA never got my private treatment records. After the remand, the regional office got the records and granted my claim.",
      },
      {
        term: "CAVC (Court of Appeals for Veterans Claims)",
        definition:
          "The federal court that reviews BVA decisions. You have ONLY 120 days to appeal to CAVC after a Board denial. This deadline is absolute with almost no exceptions.",
        example:
          "The Board denied me and I believed they made a legal error. I filed a Notice of Appeal to CAVC within the strict 120-day deadline.",
      },
      {
        term: "Joint Motion for Remand (JMR)",
        definition:
          "An agreement between the veteran and VA at CAVC to send the case back to the Board. Most CAVC cases settle this way rather than going to full court decision.",
        example:
          "At CAVC, VA agreed they made an error. We filed a Joint Motion for Remand, and the court sent my case back to the Board with instructions.",
      },
      {
        term: "VA Pension",
        definition:
          "A needs-based benefit for wartime veterans who are 65+ or permanently disabled (not from service) with limited income. Different from disability compensation - does NOT require service connection.",
        example:
          "I'm a Vietnam-era veteran with heart disease that isn't service-connected. Since I'm 70 with limited income, I qualified for VA Pension.",
      },
      {
        term: "Aid and Attendance (A&A)",
        definition:
          "An enhanced pension rate for veterans who need help with daily activities like bathing, dressing, or eating. Can add over $1,000/month to pension.",
        example:
          "My father is a wartime veteran on pension who now needs help bathing and dressing. Adding Aid and Attendance increased his monthly benefit significantly.",
      },
    ],
  },
];
