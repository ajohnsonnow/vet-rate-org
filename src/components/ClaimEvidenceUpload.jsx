...
// Builds the "upload the selected file to VA" handler. Split out of
// useEvidenceUpload purely to keep that hook under the line-count limit.
...
// Drag-and-drop handlers for the file drop zone. Split out of
// useEvidenceUpload purely to keep that hook under the line-count limit.
const useFileDragAndDrop = (setIsDragOver, handleFileSelect) => {
...
// Owns all upload state + handlers so ClaimEvidenceUpload itself stays a
// thin render layer.
const useEvidenceUpload = ({ claimId, accessToken, onUploadSuccess }) => {
  ...
  // Handle drag events
  const { handleDragOver, handleDragLeave, handleDrop } = useFileDragAndDrop(
    setIsDragOver,
    handleFileSelect,
  );
...
const EvidenceUploadFooter = ({ onClose }) => (
...
const EvidenceUploadHeader = ({ onClose, claimDetails, claimId }) => (
  <div className="bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-600 text-white p-6 relative">
    ...
    <div className="flex items-center gap-3 mb-2">
      ...
      <div>
        <h2 id="evidence-upload-title" className="text-2xl font-bold">
          Upload Evidence
        </h2>
    ...
    <div className="flex items-start gap-3">
      ...
      <div>
        ...
        <p className="text-sm text-green-700 dark:text-green-300">
          Documents are uploaded directly to the VA Claims system via their
          official API. Files are encrypted in transit and never stored on our
          servers.
        </p>
      </div>
    </div>
...
const UploadSuccessState = ({ onUploadAnother }) => (
  <div className="bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-700 rounded-xl p-6 mb-6 text-center">
    ...
    <p className="text-green-700 dark:text-green-300 mb-4">
      Your document has been submitted to the VA and attached to your claim.
    </p>
    <button
...
const DocumentTypeSelector = ({ documentType, setDocumentType }) => (
  <div className="mb-6">
...
const SelectedFileCard = ({ selectedFile, onClear }) => (
...
const FileDropZone = ({
  ...
  // eslint-disable-next-line jsx-a11y/no-static-element-interactions -- drag handlers only; the click target is the <label> below
  <div
  ...
  >
    <label htmlFor="evidence-file-input" className="cursor-pointer block">
      ...
      <p className="text-lg font-medium text-gray-700 dark:text-gray-300 mb-2">
        {isDragOver ? "Drop file here" : "Drag & drop your PDF here"}
      </p>
...
const FileUploadSection = ({
  ...
  <div className="mb-6">
    ...
    {!selectedFile ? (
      ...
      <SelectedFileCard selectedFile={selectedFile} onClear={clearFile} />
    )}
  </div>
);
...
const ClaimEvidenceUpload = ({
  ...
  const {
    ...
    handleUpload,
    clearFile,
  } = useEvidenceUpload({ claimId, accessToken, onUploadSuccess });
  ...
  return (
    <ResponsiveModal
      ...
      onClose={onClose}
      size="lg"
      labelledBy="evidence-upload-title"
      footer={<EvidenceUploadFooter onClose={onClose} />}
      header={
        <EvidenceUploadHeader
          onClose={onClose}
