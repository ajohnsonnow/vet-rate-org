...
// Reads the "Big 3" evidence signals for a condition from localStorage /
// the ClaimNavigator integration bridge.
const computeBigThreeStatus = (conditionCode, conditionName) => {
  const storageKey =
  ...
  try {
    ...
    // Corrupt or restricted localStorage just means we skip this signal.
    logger.debug("ClaimProgress: saved-claims lookup failed", {
      error: String(err),
    });
  }
  ...
  return { hasDiagnosis, hasInServiceEvent, hasNexus };
};

// Turns Big-3 status into a completeness score + the list of missing items.
const computeMissingItems = ({ hasDiagnosis, hasInServiceEvent, hasNexus }) => {
  let score = 0;
  const missing = [];
...
// Gauge color scheme based on completeness percentage.
const getGaugeColors = (completeness) => {
  if (completeness === 0)
    return {
...
const ReadinessGauge = ({ completeness, color }) => {
  const size = 200;
...
const ClaimStatusMessage = ({ completeness }) => {
  if (completeness === 100) {
    return (
...
const MissingItemsChecklist = ({ missingItems }) => {
...
const CompletedItemsList = ({ checklist }) => {
  if (
...
const ClaimProgress = ({ conditionCode, conditionName, className = "" }) => {
  ...
  const calculateCompleteness = () => {
    ...
    const bigThree = computeBigThreeStatus(conditionCode, conditionName);
    setChecklist(bigThree);

    const { score, missing } = computeMissingItems(bigThree);
    setCompleteness(score);
    setMissingItems(missing);
  };

  const color = getGaugeColors(completeness);
  ...
  return (
    <div className={`claim-progress ${className}`}>
      ...
      <ReadinessGauge completeness={completeness} color={color} />

      {/* Status Message */}
      <div
        className={`${color.bg} border border-current rounded-lg p-4 mb-4 ${color.stroke}`}
      >
        <ClaimStatusMessage completeness={completeness} />
      </div>

      <MissingItemsChecklist missingItems={missingItems} />

      <CompletedItemsList checklist={checklist} />

      <ClaimProgressHelpText />
    </div>
  );
};
