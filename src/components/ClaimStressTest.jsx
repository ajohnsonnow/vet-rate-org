...
const ClaimStressTest = ({ claimData = {}, onClose }) => {
  ...
  const loadFromPacket = (item, type) => {
    ...
    if (type === 'claim') {
      ...
      text = statement || `Condition: ${item.condition}\nCode: ${item.code}\nRating: ${item.rating || 'N/A'}%\nNotes: ${item.notes || 'None'}`;
  ...
  const hasPacketData = savedClaims.length > 0 || savedStatements.length > 0 || savedForms.length > 0;
  ...
  const analyzeClaimWeaknesses = (claim) => {
    ...
    const years = text.match(yearPattern)?.map(y => parseInt(y)).sort();
    ...
    if (lowerText.includes('hurt') || lowerText.includes('pain') || lowerText.includes('sore')) {
      if (!lowerText.includes('chronic') && !lowerText.includes('acute') && !lowerText.includes('persistent')) {
        weaknesses.push({
    ...
    if (!lowerText.includes('as likely as not') && !lowerText.includes('at least as likely')) {
      weaknesses.push({
    ...
    const servicePatterns = [
      'deployed', 'combat', 'iraq', 'afghanistan', 'vietnam', 'korea',
      'airborne', 'ranger', 'infantry', 'special forces'
    ];
    
    const mentionsService = servicePatterns.some(pattern => lowerText.includes(pattern));
    const mentionsNoRecords = lowerText.includes('no record') || lowerText.includes('lost record') || lowerText.includes('missing record');
    ...
    // Inconsistent Dates
    const hasDateInconsistency = (lowerText.includes('2010') && lowerText.includes('2012') && 
    ...
    if (lowerText.includes('made worse') || lowerText.includes('aggravated')) {
      if (!lowerText.includes('pre-existing') && !lowerText.includes('before service')) {
        weaknesses.push({
          type: 'Aggravation Claim Unclear',
          severity: 'MEDIUM',
          description: 'Mentions aggravation but unclear if condition pre-existed service',
          impact: 'VA distinguishes between service-connected conditions and aggravated conditions'
        });
    ...
    const functionalKeywords = ['unable', 'cannot', 'difficulty', 'limitation', 'restricted', 'prevents me from'];
    const hasFunctionalImpact = functionalKeywords.some(keyword => lowerText.includes(keyword));
    ...
    const currentTreatment = lowerText.includes('currently taking') || 
                            lowerText.includes('see a doctor') || 
                            lowerText.includes('physical therapy') ||
                            lowerText.includes('medication');

    if (!currentTreatment && !lowerText.includes('no longer') && !lowerText.includes('stopped treatment')) {
      weaknesses.push({
    ...
    <div className="bg-gray-900 border border-red-500/30 rounded-lg p-6 relative">
      ...
      <div className="bg-red-900/20 border border-red-500/30 rounded p-4 mb-6">
        ...
        <p className="text-gray-300 text-sm mb-3">
          This tool adopts the persona of a <span className="font-bold text-red-400">Skeptical VA Rater</span>. 
          It will identify logical gaps, timeline issues, and missing evidence in your claim.
        </p>
      ...
      {!testResults && (
        <div className="mb-6">
            ...
            <div className="mb-4">
              ...
              {showPacketSelector && (
                <div className="mt-3 bg-gray-800 border border-blue-500/50 rounded-lg p-4 max-h-64 overflow-y-auto">
                  <p className="text-gray-400 text-sm mb-3">Select an item from your packet to analyze:</p>
                  
                  {/* Saved Claims */}
                  {savedClaims.length > 0 && (
                    <div className="mb-4">
                      <h4 className="text-blue-400 font-semibold text-sm mb-2">📋 Saved Claims</h4>
                      <div className="space-y-2">
                        {savedClaims.map((claim) => (
                          ...
                          >
                            <span className="text-white font-medium">{claim.condition}</span>
                            <span className="text-gray-400 ml-2">({claim.code})</span>
                            {claim.rating && <span className="text-green-400 ml-2">{claim.rating}%</span>}
                          </button>
                        ))}
                      </div>
                    </div>
                  ...
                  {savedForms.length > 0 && (
                    <div className="mb-4">
                      <h4 className="text-yellow-400 font-semibold text-sm mb-2">📝 Saved Forms & Statements</h4>
                      <div className="space-y-2">
                        {savedForms.map((form) => (
                          <button
                            key={form.id}
                            ...
                            className="w-full text-left p-2 bg-gray-700 hover:bg-gray-600 rounded text-sm transition"
                          >
                            <span className="text-white font-medium">{form.title || form.formName}</span>
                            <span className="text-gray-400 ml-2 text-xs">
                              {form.formType?.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase())}
                            </span>
          ...
          {selectedPacketItem && (
            <div className="mb-3 p-2 bg-blue-900/30 border border-blue-500/30 rounded flex items-center justify-between">
              <span className="text-blue-300 text-sm">
                📁 Loaded: <strong>{selectedPacketItem.condition || selectedPacketItem.title || selectedPacketItem.formName}</strong>
              </span>
              <button
                onClick={() => {
          ...
          <button
            ...
            disabled={isAnalyzing || (!selectedClaim && Object.keys(claimData).length === 0)}
      ...
      {testResults && (
        <div className="space-y-6">
              ...
              {testResults.weaknesses.map((weakness, idx) => (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`px-2 py-1 text-xs font-bold rounded ${getSeverityColor(weakness.severity)}`}>
                      {weakness.severity}
                    </span>
                  </div>
                  <p className="text-white text-sm mb-2">{weakness.description}</p>
                ...
                <div key={idx} className="bg-gray-800 border border-gray-700 rounded p-4">
                  ...
                  <div className="mb-3">
                    <div className="flex items-start justify-between mb-2">
                      <span className="text-gray-500 font-bold text-sm">Question {idx + 1}</span>
                      <span className={`px-2 py-1 text-xs font-bold rounded ${getThreatBadge(q.threat_level)}`}>
                        {q.threat_level}
                      </span>
                    </div>
                    <p className="text-white font-semibold mb-1">{q.question}</p>
                    <p className="text-gray-500 text-xs">Category: {q.category}</p>
                  </div>
          ...
          <div className="bg-blue-900/30 border border-blue-500/30 rounded p-4">
            <h4 className="text-blue-400 font-bold mb-2">📋 Recommended Actions:</h4>
            <ul className="text-gray-300 text-sm space-y-2">
              {testResults.weaknesses.some(w => w.severity === 'HIGH') && (
                <li>• <span className="text-red-400 font-semibold">Critical Issues Found:</span> Address high-severity weaknesses before submitting</li>
              )}
              <li>• Review your timeline and ensure all gaps have explanations or supporting evidence</li>
              <li>• Replace vague terms with specific medical terminology (use the Somatic Target tool)</li>
              <li>• Ensure your nexus letter includes "at least as likely as not" language</li>
      ...
      <div className="mt-6 bg-yellow-900/20 border border-yellow-500/30 rounded p-3">
        <p className="text-yellow-400 text-xs font-semibold">
          ⚠️ This is a simulation. Real C&P examiners may ask different questions. Always consult with a VSO or attorney before submitting your claim.
        </p>
      </div>
    </div>
  );
