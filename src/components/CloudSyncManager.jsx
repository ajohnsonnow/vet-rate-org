      ...
      <div>
        ...
        <ul className="space-y-2 text-gray-300 text-sm">
          <li className="flex items-start gap-2">
            ...
            <span>
              Backups are encrypted and saved to <strong>YOUR</strong> Google
              Drive
            </span>
          </li>
          <li className="flex items-start gap-2">
...
const NotInitializedWarning = ({ status, error }) => (
  <div className="bg-red-500/10 border border-red-400/30 rounded-lg p-6 mb-6">
    ...
    <p className="text-gray-400 text-xs mt-3">
      If this persists, try refreshing the page or check browser console for
      errors.
    </p>
  </div>
);

const SignInSection = ({ onSignIn, isLoading }) => (
...
const UserInfoCard = ({ userInfo, onSignOut }) => (
  ...
  <div className="bg-gray-900 border border-gray-700 rounded-lg p-4">
    <div className="flex items-start justify-between mb-2">
      <div className="flex-1">
        <p className="text-white font-medium text-sm mb-1">{backup.name}</p>
...
const SignedInPanel = ({
  ...
  <div className="space-y-6">
    {userInfo && <UserInfoCard userInfo={userInfo} onSignOut={onSignOut} />}
...
// Initializes the Google Drive client on mount, retrying while gapi loads.
// Split out of useCloudSync purely to keep that hook under the line-count
// limit.
const useGoogleDriveInit = ({
  setIsInitialized,
  setIsSignedIn,
  setStatus,
  setError,
...
// Owns all Google Drive connection + backup state so CloudSyncManager
// itself stays a thin render layer.
const useCloudSync = () => {
  ...
  useGoogleDriveInit({ setIsInitialized, setIsSignedIn, setStatus, setError });
  ...
  const { handleCreateBackup, handleRestoreBackup, handleDeleteBackup } =
    useBackupActions({ setIsLoading, setError, setStatus, loadBackups });
...
const CloudSyncManager = ({ onClose }) => {
  ...
  return (
    <ResponsiveModal
      ...
      size="xl"
      zIndex={70}
      labelledBy="cloud-sync-title"
      className="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 border border-green-500/30"
      header={<CloudSyncHeader onClose={onClose} />}
    >
      <div className="-mx-4 -my-4 p-6">
        <CloudSyncExplainer />

        {!isInitialized && (
          <NotInitializedWarning status={status} error={error} />
        )}
