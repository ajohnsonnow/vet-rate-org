...
// Map squashed-bug categories to roadmap categories
const BUG_CATEGORY_MAP = {
  UI: "Community",
  Data: "Tools",
  Feature: "Tools",
  Performance: "Infrastructure",
...
// Build roadmap items from every changelog entry across all versions
const buildChangelogRoadmapItems = () => {
  const items = [];
  let itemIndex = 0;
  ...
  changelogData.updates.forEach((update) => {
    update.changelog.forEach((entry) => {
      items.push({
        ...
        // eslint-disable-next-line no-misleading-character-class, sonarjs/no-misleading-character-class, sonarjs/duplicates-in-character-class -- emoji grapheme clusters share UTF-16 code units (e.g. variation selectors); a code-unit-level dedup would corrupt the surrogate pairs and break the match
        title: entry.title.replace(/^[🎖️🌐🖥️⚡🧹💰🔍📝🎯🤝🛡️]+\s*/g, "").trim(),
        description: entry.description,
        category: CHANGELOG_CATEGORY_MAP[entry.category] || "Tools",
        itemType: CHANGELOG_TYPE_MAP[entry.type] || "Feature",
        status: "live",
        votes: 0,
        isOfficial: true,
...
// Build roadmap items from squashed bugs, skipping any with a very similar
// title already present in the changelog-derived items
const buildSquashedBugRoadmapItems = (changelogTitles) => {
  const items = [];
  ...
  SQUASHED_BUGS.forEach((bug) => {
    ...
    items.push({
      ...
      category: BUG_CATEGORY_MAP[bug.category] || "Tools",
      itemType: "Bug Fix",
      status: "live",
      votes: 0,
      isOfficial: true,
...
const generateRoadmapFromChangelog = () => {
  const items = buildChangelogRoadmapItems();

  // Deduplicate squashed bugs by checking title similarity
  const changelogTitles = new Set(items.map((i) => i.title.toLowerCase()));
  items.push(...buildSquashedBugRoadmapItems(changelogTitles));
...
// ============================================
const RoadmapCardVoteButton = ({ item, onVote, hasVoted, isSubmitting }) => (
  <button
    ...
    className={`
          ...
          `}
    aria-label={hasVoted ? "You voted for this!" : "Vote for this feature"}
  >
    <svg
...
// Sends the new feature request to the developer via FormSubmit (RT1-2:
// only when an endpoint is configured; PII is scrubbed before send)
const sendRoadmapFeatureRequestNotification = async (newItem) => {
  try {
...
const FeatureTitleDescriptionFields = ({ formData, setFormData }) => (
  <>
    {/* Title */}
    <div>
...
const SubmitFormActions = ({ onCancel, onSubmit, canSubmit, submitting }) => (
...
const SubmitFeatureForm = ({ onSubmit, onCancel }) => {
  ...
  const handleSubmit = async () => {
    ...
    await sendRoadmapFeatureRequestNotification(newItem);
...
// Derives the per-column item lists (filtered + sorted) and the header
// summary stats from the current roadmap items.
function useRoadmapDerivedData(roadmapItems, activeFilter, userVotes) {
  ...
  return { getItemsForColumn, stats };
}

// Loads persisted votes/submissions, and exposes voting + submission
// handlers for the roadmap items and filter/form UI state.
function useCommunityRoadmapState() {
...
const RoadmapModalFooter = ({ onClose }) => (
...
const RoadmapSuccessMessage = ({ successMessage }) => (
...
function CommunityRoadmap({ onClose }) {
  ...
  const {
    ...
    handleVote,
    handleSubmit,
  } = useCommunityRoadmapState();

  const { getItemsForColumn, stats } = useRoadmapDerivedData(
    roadmapItems,
    activeFilter,
    userVotes,
  );
  ...
  return (
    <ResponsiveModal
      ...
      footer={<RoadmapModalFooter onClose={onClose} />}
      header={
        <RoadmapModalHeader
          onClose={onClose}
          stats={stats}
    ...
    >
      ...
      <RoadmapActionBar
        ...
        onAddClick={() => setShowSubmitForm(true)}
      />

      {/* Success Message */}
      {successMessage && (
        <RoadmapSuccessMessage successMessage={successMessage} />
      )}
