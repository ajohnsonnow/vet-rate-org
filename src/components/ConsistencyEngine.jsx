...
const HEALTH_HEADER_GRADIENTS = {
  green: "from-green-600 to-green-800",
  red: "from-red-600 to-red-800",
  yellow: "from-yellow-600 to-orange-600",
};

function getHealthHeaderGradient(color) {
  return HEALTH_HEADER_GRADIENTS[color] || "from-orange-600 to-red-600";
}
...
function getSeverityBadge(severity) {
  return SEVERITY_BADGES[severity] || SEVERITY_BADGES.medium;
}

const SEVERITY_CARD_ACCENTS = {
  critical: "border-red-600 bg-red-50 dark:bg-red-900/20",
  high: "border-orange-600 bg-orange-50 dark:bg-orange-900/20",
};

function getSeverityCardAccent(severity) {
  return (
    SEVERITY_CARD_ACCENTS[severity] ||
    "border-yellow-600 bg-yellow-50 dark:bg-yellow-900/20"
  );
}

const HEALTH_BADGE_STYLES = {
  green: "bg-green-500/20 hover:bg-green-500/40",
  red: "bg-red-500/20 hover:bg-red-500/40 animate-pulse",
};
...
// AI Cross-Examination full-screen view
function AIAnalyzerScreen({ activeTab, setActiveTab, onClose }) {
  return (
    <ResponsiveModal
      isOpen
      onClose={onClose}
...
// Header for the Rules-mode modal
function ConsistencyEngineHeader({
  healthStatus,
  activeTab,
  setActiveTab,
  onClose,
  ...
  return (
    <div
      className={`bg-gradient-to-r ${getHealthHeaderGradient(
        healthStatus.color,
      )} text-white p-6`}
    >
...
// Body for the Rules-mode modal
function ConsistencyEngineBody({
  isChecking,
  contradictions,
  criticalCount,
  highCount,
  ...
  return (
    <>
      {isChecking && <CheckingSpinner />}

      {!isChecking && contradictions.length === 0 && (
        <NoContradictionsState refresh={refresh} />
      )}
...
function NoContradictionsState({ refresh }) {
  return (
...
function ContradictionCard({ contradiction }) {
  ...
  return (
    <div
      className={`border-l-4 ${getSeverityCardAccent(
...
export function ConsistencyBadge({ onClick }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`p-2 rounded-lg transition-colors relative ${getHealthBadgeStyle(
        healthStatus.color,
      )}`}
