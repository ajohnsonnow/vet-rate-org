...
// Search state + logic for the DBQ finder (recent searches, active search, results)
function useDbqSearch() {
  const { t } = useLanguage();
...
function DbqFinderHeader({ onClose, t }) {
  return (
    <div className="bg-gradient-to-r from-teal-600 via-emerald-600 to-teal-600 text-white p-6 relative">
      ...
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          ...
          <p className="text-teal-100 text-sm">{t("dbqFinder", "subtitle")}</p>
        </div>
      </div>
    </div>
  );
...
function DbqFinderFooter({ onClose, t }) {
  return (
...
function DbqInfoBanner({ onDismiss, t }) {
  return (
...
function DbqRecentSearches({ recentSearches, onSelect, t }) {
  return (
    <div className="mb-6">
...
function DbqErrorMessage({ error }) {
  return (
...
function DbqResultCard({ form, t }) {
  return (
    <div className="border-2 border-gray-100 dark:border-gray-700 rounded-xl p-4 hover:border-teal-200 dark:hover:border-teal-700 transition-colors bg-white dark:bg-gray-800">
        ...
        <div className="flex-1">
          ...
          {form.lastRevision && (
            <p className="text-xs text-gray-500 dark:text-gray-400">
              {t("dbqFinder", "lastUpdated")}: {form.lastRevision} •{" "}
              {form.pages} {t("dbqFinder", "pages")}
            </p>
          )}
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            <h4 className="font-semibold text-gray-900 dark:text-white">
              {t("dbqFinder", category.labelKey)}
            </h4>
            <p className="text-xs text-gray-500 dark:text-gray-400">
              {t("dbqFinder", category.descKey)}
            </p>
          </div>
        </div>
        {isExpanded ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {DBQ_CATEGORIES.map((category, idx) => (
          <DbqCategoryCard
            ...
            onToggle={() =>
              onToggleCategory(expandedCategory === idx ? null : idx)
            }
...
const DbqFinder = ({ onClose }) => {
  ...
  return (
    <ResponsiveModal
      ...
      onClose={onClose}
      size="xl"
      labelledBy="dbq-finder-title"
      footer={<DbqFinderFooter onClose={onClose} t={t} />}
      header={<DbqFinderHeader onClose={onClose} t={t} />}
    >
      {/* Info Banner */}
      {showInfo && <DbqInfoBanner onDismiss={() => setShowInfo(false)} t={t} />}
      ...
      {/* Error Message */}
      {error && <DbqErrorMessage error={error} />}

      {/* Search Results */}
      {results.length > 0 && <DbqResultsList results={results} t={t} />}
