...
const DemoDashboard = ({ onClose }) => {
  ...
  const [apiStatus, setApiStatus] = useState({
    ...
    serviceHistory: { status: 'pending', data: null, rawData: null, error: null },
    ...
    // Skipped/Future APIs
    appealsStatus: { status: 'skipped', data: null, rawData: null, error: null },
    appealableIssues: { status: 'skipped', data: null, rawData: null, error: null },
  });
  ...
  const hasApiKey = Boolean(VA_FACILITIES_API_KEY && VA_FACILITIES_API_KEY !== 'your_va_api_key_here');
  ...
  const testOpenDataAPIs = useCallback(async () => {
    ...
    try {
      const data = await getFacilities(VA_FACILITIES_API_KEY, { lat: 38.9072, lng: -77.0369, radius: 50 });
      setApiStatus(prev => ({
        ...prev,
        facilities: { status: 'success', data: formatFacilities(data), rawData: data, error: null }
      }));
    } catch (err) {
      setApiStatus(prev => ({
        ...prev,
    ...
    try {
      ...
      setApiStatus(prev => ({
        ...prev,
        forms: { status: 'success', data: formatForms(data), rawData: data, error: null }
      }));
    } catch (err) {
      setApiStatus(prev => ({
        ...prev,
    ...
    try {
      const data = await getBenefitsReferenceDisabilities(VA_BENEFITS_REF_API_KEY);
      setApiStatus(prev => ({
        ...prev,
        benefitsRef: { status: 'success', data: formatBenefitsDisabilities(data), rawData: data, error: null }
      }));
    } catch (err) {
      setApiStatus(prev => ({
        ...prev,
  ...
  const testOAuthAPIs = useCallback(async () => {
    ...
    try {
      ...
      setApiStatus(prev => ({
        ...prev,
        serviceHistory: { status: 'success', data: formatServiceHistory(data), rawData: data, error: null }
      }));
    } catch (err) {
      // Check if this is a sandbox limitation (403/Forbidden)
      const isSandboxLimit = err.code === 'FORBIDDEN' || err.message?.includes('Access denied') || err.message?.includes('403');
      setApiStatus(prev => ({
        ...prev,
        serviceHistory: { 
          ...
          data: null, 
          rawData: null, 
          error: isSandboxLimit ? 'Sandbox test data not available for this user. Production access will enable real veteran data.' : err.message 
        }
      }));
    }
    ...
    try {
      ...
      setApiStatus(prev => ({
        ...prev,
        claims: { status: 'success', data: formatClaims(data), rawData: data, error: null }
      }));
    } catch (err) {
      // Check if this is a sandbox limitation (403/Forbidden)
      const isSandboxLimit = err.code === 'FORBIDDEN' || err.message?.includes('Access denied') || err.message?.includes('403');
      setApiStatus(prev => ({
        ...
        claims: { 
          ...
          data: null, 
          rawData: null, 
          error: isSandboxLimit ? 'Sandbox test data not available for this user. Production access will enable real claims data.' : err.message 
        }
      }));
    }
  }, [accessToken]);
  ...
  const getStatusCounts = () => {
    ...
    return {
      ...
      pending: statuses.filter(s => s.status === 'pending' || s.status === 'loading').length,
  ...
  const StatusBadge = ({ status }) => {
    const configs = {
      success: { icon: CheckCircle, color: 'text-green-500', bg: 'bg-green-100 dark:bg-green-900/30', label: 'Active' },
      error: { icon: XCircle, color: 'text-red-500', bg: 'bg-red-100 dark:bg-red-900/30', label: 'Error' },
      loading: { icon: Loader2, color: 'text-blue-500', bg: 'bg-blue-100 dark:bg-blue-900/30', label: 'Testing', animate: true },
      pending: { icon: Clock, color: 'text-yellow-500', bg: 'bg-yellow-100 dark:bg-yellow-900/30', label: 'Pending' },
      skipped: { icon: Clock, color: 'text-gray-400', bg: 'bg-gray-100 dark:bg-gray-700', label: 'Future Scope' },
      sandbox_limit: { icon: Lock, color: 'text-orange-500', bg: 'bg-orange-100 dark:bg-orange-900/30', label: 'Sandbox Limited' },
    };
  ...
  // API Card Component
  const ApiCard = ({ name, apiKey, icon: Icon, status, data, rawData, error, isOAuth = false }) => {
    const isExpanded = showRawJson[apiKey];
    
    return (
      <div className={`border rounded-xl overflow-hidden transition-all ${
        status.status === 'success' ? 'border-green-200 dark:border-green-800' :        status.status === 'sandbox_limit' ? 'border-orange-200 dark:border-orange-800' :        status.status === 'error' ? 'border-red-200 dark:border-red-800' :
        status.status === 'skipped' ? 'border-gray-200 dark:border-gray-700 opacity-60' :
        'border-gray-200 dark:border-gray-700'
      }`}>
        {/* Card Header */}
        <div className={`px-4 py-3 flex items-center justify-between ${
          status.status === 'skipped' ? 'bg-gray-50 dark:bg-gray-800/50' : 'bg-white dark:bg-gray-800'
        }`}>
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
              status.status === 'success' ? 'bg-green-100 text-green-600 dark:bg-green-900/50 dark:text-green-400' :
              status.status === 'sandbox_limit' ? 'bg-orange-100 text-orange-600 dark:bg-orange-900/50 dark:text-orange-400' :
              status.status === 'error' ? 'bg-red-100 text-red-600 dark:bg-red-900/50 dark:text-red-400' :
              status.status === 'skipped' ? 'bg-gray-100 text-gray-400 dark:bg-gray-700 dark:text-gray-500' :
              'bg-blue-100 text-blue-600 dark:bg-blue-900/50 dark:text-blue-400'
            }`}>
              <Icon className="w-5 h-5" />
            </div>
            <div>
              <h4 className={`font-semibold ${status.status === 'skipped' ? 'text-gray-400 dark:text-gray-500' : 'text-gray-900 dark:text-white'}`}>
                {name}
              </h4>
              <p className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1">
                {isOAuth ? <Lock className="w-3 h-3" /> : <Unlock className="w-3 h-3" />}
      ...
      <div className="min-h-screen px-4 py-8 flex items-start justify-center">
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-5xl w-full animate-modalEnter">
          ...
          <div className="bg-gradient-to-r from-blue-900 via-blue-800 to-indigo-900 text-white p-6 rounded-t-2xl">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-blue-200 text-sm mt-1 flex items-center gap-2">
                    ...
                    Production Access Demo • {VA_AUTH_CONFIG.environment === 'sandbox' ? 'Sandbox' : 'Production'} Environment
                  </p>
                </div>
                ...
                >
                  ...
                  {authLoading ? 'Connecting...' : isAuthenticated ? 'VA Connected' : 'Connect VA Sandbox'}
                </button>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="mt-6 grid grid-cols-5 gap-3">
              ...
              <div className="bg-orange-500/20 border border-orange-400/30 rounded-xl p-3 text-center">
                <p className="text-2xl font-bold text-orange-300">{counts.sandbox_limit}</p>
          ...
          <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className={`w-4 h-4 rounded-full ${isAuthenticated ? 'bg-green-500' : 'bg-gray-400'} animate-pulse`} />
                <div>
                  ...
                  {isAuthenticated && userInfo && (
                    <p className="text-sm text-gray-500 dark:text-gray-400 flex items-center gap-1 mt-0.5">
                      ...
                      {userInfo.given_name || userInfo.name || 'Veteran'} • Sandbox Test User
                    </p>
                  )}
                </div>
              </div>
          ...
          <div className="p-6">
            ...
            <div className="mb-8">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className="font-bold text-gray-900 dark:text-white">Open Data APIs</h3>
                  <p className="text-xs text-gray-500 dark:text-gray-400">API Key Authentication • No Login Required</p>
                </div>
              </div>
            ...
            <div className="mb-8">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className="font-bold text-gray-900 dark:text-white">OAuth Protected APIs</h3>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Requires VA.gov Login • User-Specific Data</p>
                </div>
              </div>
            ...
            <div>
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className="font-bold text-gray-500 dark:text-gray-400">Future Scope</h3>
                  <p className="text-xs text-gray-400 dark:text-gray-500">Planned for Phase 2 Implementation</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-blue-700 dark:text-blue-300 mt-1">
                    All data is processed client-side only. No veteran data is stored on our servers or in any database. 
                    OAuth tokens are stored in browser session storage and cleared on logout.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 rounded-b-2xl">
            <div className="flex items-center justify-between text-sm text-gray-500 dark:text-gray-400">
              ...
              <p className="flex items-center gap-2">
                ...
                {counts.success} of {Object.keys(apiStatus).length - counts.skipped} APIs Active
              </p>
            </div>
          </div>
        </div>
