...
const RECORD_TYPES = [
  {
    ...
    description: 'Your entire VA claims history including all decisions, medical exams, and evidence',
    icon: '📁',
    category: 'Essential',
    formBox: '11.A',
    tips: [
  ...
  },
  {
    ...
    description: 'Your complete service record including assignments, deployments, and awards',
    icon: '🎖️',
    category: 'Essential',
    formBox: '11.B',
    tips: [
...
const generateRequestText = (formData, selectedRecords) => {
  const selectedRecordsList = selectedRecords.map(id => 
    RECORD_TYPES.find(r => r.id === id)
  ).filter(Boolean);
...
export default function FOIAGenerator({ onClose, onReportBug }) {
  ...
  // Record selection
  const [selectedRecords, setSelectedRecords] = useState(['cfile', 'rating_decisions', 'cp_exams']);
  ...
  const selectEssential = () => {
    const essentialIds = RECORD_TYPES.filter(r => r.category === 'Essential').map(r => r.id);
  ...
  const downloadPDF = () => {
    ...
    doc.text('FOIA/Privacy Act Request', pageWidth / 2, 20, { align: 'center' });
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="bg-amber-50 dark:bg-amber-900/30 border-l-4 border-amber-500 p-4 rounded-r-lg">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="font-bold text-amber-800 dark:text-amber-200">Your Data Stays Local</h3>
            <p className="text-amber-700 dark:text-amber-300 text-sm mt-1">
              None of your personal information leaves your device. Everything is processed locally 
              and goes directly into your downloadable document.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-blue-700 dark:text-blue-300 text-sm mt-1">
              Request your <strong>complete C-File</strong> - it contains everything and reveals what VA actually 
              considered when making their decisions. You're entitled to <strong>one free copy</strong> under 38 CFR § 1.555.
            </p>
          </div>
        </div>
      </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <p className="font-semibold text-gray-800 dark:text-gray-100">{record.name}</p>
                    </div>
                    ...
                    {selectedRecords.includes(record.id) && (
                      <div className="mt-2 p-2 bg-green-100 dark:bg-green-900/50 rounded-lg">
                        <p className="text-xs font-semibold text-green-800 dark:text-green-200 mb-1">Why this matters:</p>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
        ...
        <div className="space-y-3">
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            />
            <span className="text-gray-800 dark:text-gray-100">All records (recommended)</span>
          </label>
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            <input
              ...
              className="w-5 h-5 text-amber-500"
            />
            <span className="text-gray-800 dark:text-gray-100">Specific date range</span>
          </label>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
        ...
        <div className="space-y-3">
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            />
            <div>
              <span className="text-gray-800 dark:text-gray-100 font-medium">Electronic (Recommended)</span>
              <p className="text-sm text-gray-500 dark:text-gray-400">Faster and easier to search through</p>
            </div>
          </label>
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            />
            <div>
              <span className="text-gray-800 dark:text-gray-100 font-medium">Physical Copies</span>
              <p className="text-sm text-gray-500 dark:text-gray-400">Mailed to your address</p>
            </div>
          </label>
        </div>
      </div>
      ...
      {selectedRecords.length > 0 && (
        <div className="bg-green-50 dark:bg-green-900/30 border-l-4 border-green-500 p-4 rounded-r-lg">
          <p className="font-semibold text-green-800 dark:text-green-200">
            {selectedRecords.length} record type{selectedRecords.length > 1 ? 's' : ''} selected
          </p>
        </div>
      )}
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          <div>
            ...
            <p className="font-medium text-gray-800 dark:text-gray-100">
              {formData.dateRange === 'all' ? 'All records' : `${formData.startDate} to ${formData.endDate}`}
            </p>
          </div>
          <div>
        ...
        <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
          <p className="text-gray-500 dark:text-gray-400 text-sm mb-2">Records being requested:</p>
          <div className="flex flex-wrap gap-2">
            {selectedRecords.map(id => {
              const record = RECORD_TYPES.find(r => r.id === id);
              return (
                <span key={id} className="px-2 py-1 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-xs rounded">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          >
            ...
            <p className="font-semibold text-gray-800 dark:text-gray-100">Download PDF</p>
            <p className="text-xs text-gray-500 dark:text-gray-400">For printing or submitting</p>
          </button>
          ...
          >
            <span className="text-3xl block mb-2">📝</span>
            <p className="font-semibold text-gray-800 dark:text-gray-100">Download DOCX</p>
            <p className="text-xs text-gray-500 dark:text-gray-400">For editing if needed</p>
          </button>
          ...
          >
            <span className="text-3xl block mb-2">📋</span>
            <p className="font-semibold text-gray-800 dark:text-gray-100">Copy Text</p>
      ...
      {showPreview && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden">
          <div className="p-4 bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-600">
            <h4 className="font-bold text-gray-800 dark:text-gray-100">Request Preview</h4>
          </div>
      ...
      <div className="bg-blue-50 dark:bg-blue-900/30 border-l-4 border-blue-500 p-4 rounded-r-lg">
        <h4 className="font-bold text-blue-800 dark:text-blue-200 mb-3">📤 How to Submit</h4>
        <div className="text-blue-700 dark:text-blue-300 text-sm space-y-3">
          <div>
            ...
            <p>Upload at <a href="https://www.va.gov/documentation" target="_blank" rel="noopener noreferrer" className="underline">va.gov/documentation</a></p>
          </div>
          <div>
            <p className="font-semibold">Option 2: Submit with VA Form 20-10206</p>
            <p>Download official form from <a href="https://www.va.gov/find-forms/about-form-20-10206/" target="_blank" rel="noopener noreferrer" className="underline">VA.gov</a> and attach this document</p>
          </div>
          <div>
            <p className="font-semibold">Option 3: eVetRecs (for Service Records)</p>
            <p>Submit at <a href="https://www.archives.gov/veterans/military-service-records" target="_blank" rel="noopener noreferrer" className="underline">archives.gov/veterans</a></p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4">
        <h4 className="font-bold text-gray-800 dark:text-gray-100 mb-2">⏱️ Expected Timeline</h4>
        <ul className="text-sm text-gray-600 dark:text-gray-300 space-y-1">
          <li>• <strong>VA Regional Office:</strong> 20-30 business days (can take longer)</li>
          <li>• <strong>NPRC (Service Records):</strong> 2-6 months (backlog varies)</li>
          <li>• <strong>Expedited requests:</strong> If you have a pending claim or appeal, mention it!</li>
        </ul>
      </div>
