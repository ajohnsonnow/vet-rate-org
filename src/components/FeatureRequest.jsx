...
async function _copyReport(generatedReport, ctx) {
  const { setCopied } = ctx;
...
function _updateFormField(field, value, ctx) {
  const { setFormData, setEmailError, t } = ctx;
  setFormData((prev) => ({ ...prev, [field]: value }));

  // Validate email when it changes
  if (field === "veteranEmail") {
    if (value && value.trim() !== "" && !_isValidEmail(value)) {
...
function _formatFeatureRequest(formData, featureId, t) {
  const systemInfo = getSystemInfo();
...
function _computeFeatureRequestDerivedState(
  ...
  const canProceedStep2 =
    ...
    _isValidEmail(formData.veteranEmail);

  const backButtonUsesClose = step === 1 || submitted;
...
function FeatureRequestProgressSteps({ t, step }) {
  return (
    <>
...
function FeatureRequestPriorityField({ t, formData, handleInputChange }) {
  return (
    <div>
...
function FeatureRequestStep1({ step, t, formData, handleInputChange }) {
...
function FeatureRequestTitleField({ t, formData, handleInputChange }) {
  return (
    <div>
      <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
...
function FeatureRequestDescriptionField({ t, formData, handleInputChange }) {
  return (
    <div>
      <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
...
function FeatureRequestProblemSolvedField({ t, formData, handleInputChange }) {
  return (
    <div>
      <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
...
function FeatureRequestAlternativesField({ t, formData, handleInputChange }) {
  return (
    <div>
      <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
...
function FeatureRequestStep2({
  ...
  return (
    <div className="space-y-6">
      <FeatureRequestTitleField
        t={t}
        formData={formData}
        handleInputChange={handleInputChange}
      />

      <FeatureRequestDescriptionField
        t={t}
        formData={formData}
        handleInputChange={handleInputChange}
      />

      <FeatureRequestProblemSolvedField
        t={t}
        formData={formData}
        handleInputChange={handleInputChange}
      />

      <FeatureRequestProposedSolutionField
        t={t}
        formData={formData}
        handleInputChange={handleInputChange}
      />

      <FeatureRequestAlternativesField
        t={t}
        formData={formData}
        handleInputChange={handleInputChange}
      />

      <FeatureRequestAdditionalContextField
        t={t}
        formData={formData}
        handleInputChange={handleInputChange}
      />

      <FeatureRequestPrivacySection
        t={t}
        formData={formData}
        handleInputChange={handleInputChange}
...
function FeatureRequestRoadmapCta({ onClose, onOpenRoadmap }) {
...
function FeatureRequestSubmitButton({ t, submitting, handleSubmitRequest }) {
  return (
    <button
...
function FeatureRequestReviewView({
  t,
  generatedReport,
  copied,
  submitting,
...
function FeatureRequestModal({ s }) {
  const {
    onClose,
    onOpenRoadmap,
    step,
    ...
    <ResponsiveModal
      ...
      footer={
        <FeatureRequestFooter
          ...
          step2ValidationMessage={step2ValidationMessage}
      ...
      header={<FeatureRequestHeader t={t} step={step} onClose={onClose} />}
    >
      <FeatureRequestStep1
        step={step}
        t={t}
...
function FeatureRequest({ onClose, appState = {}, onOpenRoadmap }) {
  ...
  const handleInputChange = (field, value) =>
    _updateFormField(field, value, { setFormData, setEmailError, t });

  const handleGenerateReport = () => {
    setGeneratedReport(_formatFeatureRequest(formData, featureId, t));
    setStep(3);
  };

  const handleSubmitRequest = () =>
    _submitFeatureRequest(featureId, formData, generatedReport, t, {
      setSubmitting,
      setSubmitError,
      setSubmitted,
      setCopied,
  ...
  const backButtonHandler = derived.backButtonUsesClose
    ? onClose
    : () => setStep(step - 1);
  ...
  return (
    <FeatureRequestModal
      s={{
        ...
        handleSubmitRequest,
        backButtonHandler,
        backButtonLabel: derived.backButtonLabel,
        canProceedStep2: derived.canProceedStep2,
        step2ValidationMessage: derived.step2ValidationMessage,
        canProceedCurrentStep: derived.canProceedCurrentStep,
      }}
    />
  );
}
