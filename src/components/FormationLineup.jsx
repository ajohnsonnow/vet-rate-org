...
const STATUS_DISPLAY = {
  [FORMATION_STATUS.WAITING]: { icon: '⏸️', label: 'Waiting', color: 'text-gray-500' },
  [FORMATION_STATUS.CALLED]: { icon: '📞', label: 'Called', color: 'text-blue-600' },
  [FORMATION_STATUS.OCR_IN_PROGRESS]: { icon: '🔍', label: 'OCR In Progress', color: 'text-purple-600' },
  [FORMATION_STATUS.INTEL_BRIEFING]: { icon: '🛡️', label: 'Intel Briefing', color: 'text-indigo-600' },
  [FORMATION_STATUS.USER_REVIEW]: { icon: '👀', label: 'Your Review', color: 'text-yellow-600' },
  [FORMATION_STATUS.VERIFIED]: { icon: '✅', label: 'Verified', color: 'text-green-600' },
  [FORMATION_STATUS.SAVED]: { icon: '💾', label: 'Saved', color: 'text-green-700' },
  [FORMATION_STATUS.SKIPPED]: { icon: '⏭️', label: 'Skipped', color: 'text-gray-400' },
  [FORMATION_STATUS.ERROR]: { icon: '❌', label: 'Error', color: 'text-red-600' }
};

/**
 * Individual document entry in formation
 */
const FormationEntry = ({ entry, index, onRemove, isDragging, onDragStart, onDragOver, onDrop }) => {
  const status = STATUS_DISPLAY[entry.status] || STATUS_DISPLAY[FORMATION_STATUS.WAITING];
  const isCurrent = entry.status === FORMATION_STATUS.CALLED ||
    ...
    <div className="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 rounded-lg p-4 mb-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg p-2 text-center">
          ...
          <div className="text-xs text-gray-500 dark:text-gray-400">Critical Docs</div>
        </div>
      </div>
    </div>
  );
...
export default function FormationLineup({
  ...
  const stats = formation.length > 0 ? {
    total: formation.length,
    waiting: formation.filter(e => e.status === FORMATION_STATUS.WAITING).length,
    completed: formation.filter(e => e.status === FORMATION_STATUS.SAVED).length,
    critical: formation.filter(e => e.priority >= 9).length
  } : null;
  
  const progress = formation.length > 0
  ...
  const canStartFormation = formation.some(e => e.status === FORMATION_STATUS.WAITING);
  const isProcessing = formation.some(e => 
    ...
    <div className="space-y-4">
          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              <p className="text-xs text-yellow-700 dark:text-yellow-300">
                These documents are from your previous session. They were saved but the actual files are no longer in memory.
                You can view the lineup and clear it, but you'll need to re-upload the files to process them again.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 text-sm text-blue-800 dark:text-blue-200">
        ...
        <ul className="space-y-1 text-xs">
          <li>• <strong>Critical documents</strong> (DD214, Rating Decisions) process first</li>
