    ...
    <div className="bg-slate-800 dark:bg-slate-900 border border-slate-700 dark:border-slate-600 rounded-lg p-6 max-w-2xl">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl font-bold text-white">Neural Processing Unit</h2>
        </div>
        <button
          onClick={scanSystem}
      ...
      <p className="text-gray-400 text-sm mb-4">
        Select which GPU should handle AI model inference (Gemini Nano, local LLMs, etc.)
      </p>
      ...
      {adapters.length === 0 && !error && (
        <div className="bg-yellow-900/50 border border-yellow-700 text-yellow-200 p-3 rounded mb-4">
          <p className="text-sm">No GPUs detected. WebGPU may not be available or enabled in your browser.</p>
        </div>
      )}
      ...
      {adapters.length === 1 && (
        <div className="mt-6 text-sm text-gray-400 bg-slate-900 dark:bg-slate-800 p-4 rounded border border-slate-700">
          ...
          <p className="mb-3">
            Due to browser privacy protections (<a 
            ...
            Chrome often hides secondary GPUs. Windows also ignores GPU preference settings.
          </p>
          <div className="bg-slate-800/50 dark:bg-slate-700/50 p-3 rounded">
            <p className="font-semibold text-white mb-2">To force a specific GPU:</p>
            <ol className="list-decimal ml-5 space-y-1 text-gray-300">
              <li>Open Windows <strong>Settings</strong> → <strong>System</strong> → <strong>Display</strong></li>
              <li>Scroll down and click <strong>Graphics Settings</strong></li>
              <li>Click <strong>Browse</strong> and find your browser (Chrome/Edge)</li>
              <li>Click <strong>Options</strong> and select <strong>High Performance</strong></li>
              <li>Restart your browser</li>
            </ol>
            <div className="mt-3 p-2 bg-blue-900/20 border border-blue-500/30 rounded">
              <p className="text-xs text-blue-300">
                <strong>Quick Access:</strong> Press <kbd className="px-1.5 py-0.5 bg-slate-700 rounded text-white font-mono text-xs">Win + R</kbd>, 
                type <code className="px-1.5 py-0.5 bg-slate-700 rounded text-yellow-300 font-mono text-xs">ms-settings:display</code>, 
                then scroll to Graphics Settings
              </p>
            </div>
          </div>
          <p className="mt-3 text-xs text-gray-500 italic">
            This is an OS-level limitation - there's no code workaround. The browser only exposes what Windows allows.
          </p>
        </div>
      )}
      ...
      {adapters.length > 1 && (
        <div className="mt-4 text-xs text-gray-500 bg-slate-900/50 dark:bg-slate-800/50 p-3 rounded">
          <p>💡 <strong>Tip:</strong> Multiple GPUs detected! Select "High Performance" for faster AI inference, or "Integrated" to save battery on laptops.</p>
        </div>
      )}
    </div>
  );
