...
const SOURCE_METADATA = {
  ...
  'eCFR_OFFICIAL': {
    ...
    description: 'Code of Federal Regulations - Official VA disability rating schedule',
  ...
  'PACT_ACT_OFFICIAL': {
    ...
    description: 'Promise to Address Comprehensive Toxics Act - Toxic exposure benefits',
  ...
  'U.S. Court of Appeals for Veterans Claims': {
    ...
    description: 'Court of Appeals for Veterans Claims - Binding judicial precedent',
  ...
  'BVA_DECISIONS': {
    ...
    description: 'Board of Veterans Appeals - Administrative precedent decisions',
  ...
  'SECONDARY_CONDITIONS_MATRIX': {
    ...
    description: 'Medical nexus matrix - Service-connected secondary conditions',
    lastUpdated: '2026-01-22',
    authorityLevel: 6,
    icon: '🔗',
    color: 'purple'
...
const getSourceMetadata = (sourceKey) => {
  ...
  for (const [key, metadata] of Object.entries(SOURCE_METADATA)) {
    if (normalizedKey.includes(key) || key.includes(normalizedKey.split('_').slice(0, 3).join('_'))) {
      return metadata;
    }
  }
  ...
  useEffect(() => {
    ...
    const loadKnowledgeBaseStats = async () => {
      try {
        ...
        const fullCount = dkbData.full_database_count || dkbData.fullDatabaseCount || 130508;
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between pb-2 border-b border-gray-200 dark:border-gray-700">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-gray-900 dark:text-white">Diamond Knowledge Base</h4>
                    <p className="text-xs text-gray-500 dark:text-gray-400">Official VA sources for AI training</p>
                  </div>
                </div>
                <div className="text-right">
                  ...
                  <div className="text-xs text-gray-500 dark:text-gray-400">entries loaded</div>
                </div>
              </div>
                ...
                {!kbStatus.loading && (
                  <div className="dkb-scrollbar max-h-48 overflow-y-auto pr-1 space-y-2">
                    {Object.entries(kbStatus.dkbSources)
                      ...
                      .sort((a, b) => a.metadata.authorityLevel - b.metadata.authorityLevel || b.count - a.count)
                        ...
                        >
                          <div className="flex items-start justify-between gap-2">
                            ...
                            <div className="text-right flex-shrink-0">
                              ...
                              <div className={`text-xs ${metadata.lastUpdated ? 'text-gray-500 dark:text-gray-400' : 'text-amber-500 dark:text-amber-400'}`}>
                                {metadata.lastUpdated ? formatDate(metadata.lastUpdated) : 'Planned'}
                              </div>
                            </div>
                          </div>
                        </div>
              ...
              <div className="pt-2 border-t border-gray-200 dark:border-gray-700">
                ...
                <div className="flex flex-wrap gap-1.5 text-xs">
                  <span className="px-2 py-0.5 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 rounded-full">1-Statutory</span>
                  <span className="px-2 py-0.5 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded-full">2-Judicial</span>
                  <span className="px-2 py-0.5 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 rounded-full">3-Admin</span>
                  <span className="px-2 py-0.5 bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400 rounded-full">4-Policy</span>
                  <span className="px-2 py-0.5 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded-full">5-Procedures</span>
                  <span className="px-2 py-0.5 bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 rounded-full">6-Reference</span>
                </div>
              </div>
      ...
      <div className="space-y-5">
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="text-sm text-amber-700 dark:text-amber-400 mt-1">
                  Currently showing <strong>{kbStatus.dkbEntries.toLocaleString()}</strong> high-priority entries 
                  (top 6% by quality score) for faster browser loading.
                </div>
                <div className="mt-3 p-3 bg-white/50 dark:bg-gray-800/50 rounded-lg">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase">Full Database</div>
                    ...
                    <div className="text-center">
                      <span className="text-2xl">🧠</span>
                      <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">Available with</div>
                      <div className="text-xs font-semibold text-purple-600 dark:text-purple-400">Local LLM</div>
                    </div>
                  </div>
                </div>
                <div className="text-xs text-amber-600 dark:text-amber-500 mt-2">
                  💡 Load <strong>Diamond Swarm</strong> (Local AI) for complete 130K+ entry access with offline capability.
                </div>
              </div>
            </div>
          </div>
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-start gap-3 min-w-0">
                        <span className="text-2xl flex-shrink-0 mt-0.5">{metadata.icon}</span>
                        <div className="min-w-0">
        ...
        <div className="pt-3 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between">
          <div className="text-sm text-gray-500 dark:text-gray-400">
            ...
            <span className="ml-2 text-gray-400">as of {formatDate(kbStatus.ecfrDate)}</span>
          </div>
          <a
            href="https://www.ecfr.gov/current/title-38/chapter-I/part-4"
            target="_blank"
