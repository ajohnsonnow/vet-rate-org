...
export const LLMRecommendationBadge = ({ toolId, className = "" }) => {
  ...
  if (aiStatus.effectiveMode === AI_MODES.CLOUD) {
    return (
      <div
        className={`text-xs bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 px-2 py-1 rounded-full ${className}`}
      >
        ☁️ Cloud AI (Gemini)
      </div>
    );
 ...
 */
export const LLMRecommendationPanel = ({
  toolId,
  onModelSelect,
  className = "",
}) => {
  ...
  return (
    <div
      className={`bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 rounded-xl p-4 border border-indigo-200 dark:border-indigo-800 ${className}`}
    >
      {/* Header */}
...
export const LLMHint = ({ toolId, className = "" }) => {
  ...
  return (
    <div
      className={`text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1 ${className}`}
    >
      <span>💡</span>
      <span>
        Best with{" "}
        <strong className="text-indigo-600 dark:text-indigo-400">
          {recommendation.primary?.modelName || "Local AI"}
        </strong>
      </span>
    </div>
  );
