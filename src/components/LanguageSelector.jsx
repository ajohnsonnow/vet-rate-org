...
const LANGUAGE_REGIONS = [
  ...
  { id: 'pacific', name: 'Pacific Islander', icon: '🌊', note: 'Highest military service rates!' },
  { id: 'asian', name: 'Asian', icon: '🌏' },
  { id: 'african', name: 'African', icon: '🌍' },
  { id: 'middle-eastern', name: 'Middle Eastern', icon: '🕌' },
  { id: 'native-american', name: 'Native American', icon: '🪶', note: 'Honoring Code Talker heritage' },
...
const LanguageSelector = ({ 
  ...
  if (variant === 'compact') {
    return (
      <div ref={dropdownRef} className={`relative ${className}`}>
        ...
        {isOpen && (
          <div className="absolute top-full right-0 mt-1 py-1 bg-gray-900 border border-gray-700 
                  ...
                  return (
                    <div key={region.id}>
                      <div className="px-3 py-1.5 bg-gray-800/50 text-xs font-bold text-gray-400 uppercase tracking-wide flex items-center gap-2">
                        ...
                        {region.note && (
                          <span className="text-[10px] text-cyan-400 font-normal normal-case">({region.note})</span>
                        )}
                      </div>
            ...
            <div className="px-3 py-2 border-t border-gray-800 bg-gray-900/50 space-y-2">
              ...
              <button
                onClick={(e) => { e.stopPropagation(); setShowSuggestionModal(true); setIsOpen(false); }}
  ...
  if (variant === 'inline') {
    const topLanguages = ['en', 'es', 'ch', 'sm', 'tl', 'vi', 'ar', 'am'].map(code => SUPPORTED_LANGUAGES[code]).filter(Boolean);
            ...
            {LANGUAGE_REGIONS.map(region => {
              ...
              return (
                <div key={region.id} className="mb-1">
                  <div className="px-4 py-2 bg-gradient-to-r from-gray-800/80 to-gray-800/40 text-sm font-bold text-gray-300 
                                  uppercase tracking-wide flex items-center gap-2 sticky top-0">
                    <span>{region.icon}</span>
                    <span>{region.name}</span>
                    {region.note && (
                      <span className="text-xs text-cyan-400 font-normal normal-case ml-auto">✨ {region.note}</span>
                    )}
                  </div>
                  {regionLangs.map((lang) => (
                    <button
