...
// Load the veteran's combined rating from their saved profile, if any.
// Returns true when a rating was found and applied.
function loadRatingFromProfile(setRating) {
...
// interval cleanup function for use as a useEffect return value.
function animateTotalCounter(target, { setAnimatedTotal, setShowBreakdown }) {
...
// Build the (age, value) points used to plot the value growth chart
function buildChartData(calculation, currentAge) {
  const points = [];
...
// Generate the SVG path "d" attribute for the value growth line/area
function buildChartPath(chartData, chartWidth, chartHeight, padding) {
...
// Modal header with title, badge, bug report link, and close button
const DashboardHeader = ({ onClose, onReportBug }) => (
...
// The big animated lifetime-value total
const TheBigNumber = ({ animatedTotal, currentAge }) => (
...
// VA Rating % range slider, with a "Load" button when saved ratings exist
...
// Spouse yes/no toggle
const SpouseToggleField = ({ hasSpouse, setHasSpouse }) => (
  <div>
    {/* eslint-disable-next-line jsx-a11y/label-has-associated-control */}
...
// Permanent & Total (P&T) yes/no toggle
const PermanentTotalToggleField = ({
  isPermanentTotal,
  setIsPermanentTotal,
}) => (
  <div>
...
// The full "Your Profile" input controls card
const ProfileInputControls = ({
  ageInputValue,
  setAgeInputValue,
  setCurrentAge,
  rating,
  ...
  <div className="bg-gray-800/50 rounded-2xl p-6 border border-gray-700">
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      <AgeInputField
        ageInputValue={ageInputValue}
        setAgeInputValue={setAgeInputValue}
    ...
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
      <SpouseToggleField hasSpouse={hasSpouse} setHasSpouse={setHasSpouse} />
      <ChildrenInputField
        numChildren={numChildren}
        setNumChildren={setNumChildren}
      />
      <PermanentTotalToggleField
...
// Gold gradient definitions used by the value line and area fill
...
// Cumulative value growth SVG chart
const ValueGrowthChart = ({ chartData, chartWidth, chartHeight, padding }) => (
  <div className="bg-gray-800/50 rounded-2xl p-6 border border-gray-700">
    ...
    <div className="overflow-x-auto">
      ...
      >
        ...
        {/* Value line */}
        <path
          d={buildChartPath(chartData, chartWidth, chartHeight, padding)}
          fill="none"
        ...
        {/* Area fill */}
        <path
          d={`${buildChartPath(chartData, chartWidth, chartHeight, padding)} L ${chartWidth - padding},${chartHeight - padding} L ${padding},${chartHeight - padding} Z`}
...
// VA Disability Pay breakdown card
const VAPayCard = ({ calculation }) => (
  <div className="bg-gradient-to-br from-green-900/50 to-emerald-900/30 rounded-2xl p-6 border border-green-700/50">
    ...
    <p className="text-sm text-green-300/70 mt-2">
      {formatCurrency(calculation.monthlyPay)}/mo × {calculation.yearsRemaining}{" "}
      years
    </p>
  </div>
);
  ...
  >
    ...
    >
      {hasSpouse || numChildren > 0
        ? `${formatCurrency(CHAPTER_35_DEA.totalValue)} × ${dependentsCount} dependents`
        : "Add dependents to qualify"}
    </p>
  </div>
);

// CHAMPVA Healthcare breakdown card
const HealthcareCard = ({ rating, hasSpouse, numChildren, calculation }) => (
  <div
    className={`rounded-2xl p-6 border ${
      rating === 100 && (hasSpouse || numChildren > 0)
...
// The four benefit breakdown cards, shown once the counter animation completes
...
// Motivational framing statement
const MotivationalMessage = () => (
  <div className="bg-gradient-to-r from-amber-900/30 via-yellow-900/30 to-amber-900/30 rounded-2xl p-6 border border-amber-700/50 text-center">
    ...
    <p className="text-amber-300/70">
      ...
      <span className="font-bold text-amber-200">generational wealth</span>.
    </p>
  </div>
);

// Notice that SMC/CRSC/CRDP aren't calculated here, with an SMC-S hint
const SMCNotIncludedNotice = ({ smcSCheck }) => (
  <div className="bg-gray-800/50 rounded-2xl p-6 border border-gray-700">
    <h3 className="text-lg font-bold text-white mb-2">
      ℹ️ Not included: SMC, CRSC &amp; CRDP
    </h3>
    <p className="text-sm text-gray-400">
      Special Monthly Compensation (SMC), Combat-Related Special Compensation
...
// Contextual "did this open your eyes" support CTA
const SupportCTA = () => (
...
// Luna's Treat Fund CTA, shown after the total has been viewed
const LunaTreatFund = ({ animatedTotal, grandTotal }) => {
  if (animatedTotal <= 0) return null;
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-purple-200 text-xs leading-relaxed mb-2">
              Saved ${Math.round(grandTotal).toLocaleString()}? That&apos;s a
              LOT of treats! Help keep this tool free (and Luna fed).
            </p>
            <a
...
// Encapsulates all the Million Dollar Dashboard's input/animation state,
// load handlers, and derived calculations, so the component body stays
// focused on composing the extracted section components.
function useMillionDollarDashboardState() {
  // Input state
  ...
  useEffect(
    () =>
      animateTotalCounter(calculation.grandTotal, {
        setAnimatedTotal,
        setShowBreakdown,
      }),
  ...
  const chartData = useMemo(
    () => buildChartData(calculation, currentAge),
    [calculation, currentAge],
  );

  // Chapter 35 DEA eligible dependents for the education value display
  const dependentsCount = hasSpouse ? numChildren + 1 : numChildren;
  ...
  return (
    <>
      ...
      >
        ...
        <div className="space-y-6">
          ...
          <ProfileInputControls
            ...
            setAgeInputValue={d.setAgeInputValue}
            setCurrentAge={d.setCurrentAge}
            rating={d.rating}
            setRating={d.setRating}
            handleLoadMyRatings={d.handleLoadMyRatings}
            state={d.state}
            setState={d.setState}
            hasSpouse={d.hasSpouse}
            setHasSpouse={d.setHasSpouse}
            numChildren={d.numChildren}
            setNumChildren={d.setNumChildren}
            isPermanentTotal={d.isPermanentTotal}
            setIsPermanentTotal={d.setIsPermanentTotal}
          />
          ...
          <SMCNotIncludedNotice smcSCheck={d.smcSCheck} />
      ...
      <LunaTreatFund
        ...
        grandTotal={d.calculation.grandTotal}
      />
    </>
  );
}
