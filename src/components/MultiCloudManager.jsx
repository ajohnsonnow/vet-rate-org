      ...
      try {
        if (typeof window.gapi !== 'undefined') {
          ...
          setProviderStates(prev => ({
            ...prev,
            google_drive: { ...prev.google_drive, initialized: true, connected: signedIn }
          }));
        }
      } catch (err) {
  ...
  const handleRestore = async (backup) => {
    ...
    try {
      ...
      if (selectedProvider === 'google_drive') {
        ...
        try {
          data = await restoreBackup(selectedProvider, backup.path || backup.id);
        } catch (err) {
      ...
      if (isEncryptedBackup(data)) {
        ...
        if (localKey) {
          ...
          throw new Error('Encryption key not found. This backup cannot be restored on this device.');
        }
      }
    ...
    try {
      ...
      if (!data) {
        if (selectedProvider === 'google_drive') {
          ...
          data = await restoreBackup(selectedProvider, pendingRestore.path || pendingRestore.id, restorePassphrase);
      ...
      const decryptedData = await decryptFromCloud(data, restorePassphrase, true);
      ...
      <div className="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 rounded-xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden border border-cyan-500/30">
        ...
        <div className="bg-gradient-to-r from-cyan-600 to-blue-600 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-white">The Redundant Bunker Network</h1>
              <p className="text-cyan-100 text-sm">Multi-cloud backup with military-grade encryption</p>
            </div>
          </div>
          <button onClick={onClose} className="text-white hover:text-cyan-200 transition-colors text-2xl font-bold">×</button>
        </div>
        
        {/* Tabs */}
        <div className="flex border-b border-gray-700">
          {[
            { id: 'providers', label: '☁️ Cloud Providers', desc: 'Connect accounts' },
            { id: 'backups', label: '💾 Backups', desc: 'Manage backups' },
            { id: 'settings', label: '🔒 Security', desc: 'Encryption settings' }
          ].map(tab => (
            <button
              key={tab.id}
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
          ...
          {/* Status Messages */}
          {(status || error) && (
            <div className={`mb-4 p-3 rounded-lg ${error ? 'bg-red-500/10 border border-red-400/30' : 'bg-blue-500/10 border border-blue-400/30'}`}>
              <p className={error ? 'text-red-300 text-sm' : 'text-blue-300 text-sm'}>
          ...
          {activeTab === 'providers' && (
            <div className="space-y-4">
              <div className="bg-gradient-to-r from-green-500/10 to-cyan-500/10 border border-green-400/30 rounded-lg p-4 mb-4">
                <h3 className="text-green-300 font-bold mb-2">🛡️ Your Data, Your Cloud</h3>
                <p className="text-gray-300 text-sm">
                  Connect your personal cloud storage. Your data is encrypted <strong>before</strong> it leaves your browser 
                  and stored in <strong>your own</strong> cloud account. We never have access to your data.
                </p>
              </div>
              ...
              <div className="bg-gradient-to-r from-amber-500/10 to-orange-500/10 border border-amber-400/30 rounded-lg p-4 mb-6">
                ...
                <div className="text-gray-300 text-sm space-y-2">
                  ...
                  <p>
                    <strong>Cloud storage disclaimer:</strong> When you backup to cloud providers, YOUR data goes to YOUR personal 
                    account. The security of that data depends on:
                  </p>
                  ...
                  <p className="text-amber-300 text-xs mt-2">
                    💡 <strong>Recommendation:</strong> Always enable encryption with a passphrase for sensitive medical data. 
                    Consider using Dropbox Business or OneDrive for Business if you need HIPAA-compliant storage.
                  </p>
                </div>
              </div>
              ...
              <div className="grid gap-4">
                ...
                <div className={`border rounded-lg p-4 transition-all ${
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="text-white font-semibold">Google Drive</h4>
                        <p className="text-gray-400 text-xs">Most veterans have Gmail - easiest option</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <span className="px-2 py-1 bg-green-500/20 text-green-300 text-xs rounded">🔐 TLS Encryption</span>
                    <span className="px-2 py-1 bg-green-500/20 text-green-300 text-xs rounded">🗄️ At-Rest Encryption</span>
                    <span className="px-2 py-1 bg-blue-500/20 text-blue-300 text-xs rounded">SOC 2</span>
                    <span className="px-2 py-1 bg-blue-500/20 text-blue-300 text-xs rounded">ISO 27001</span>
                  </div>
                </div>
                ...
                <div className={`border rounded-lg p-4 transition-all ${
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-gray-400 text-xs">Strong security, great mobile apps</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      {providerStates.dropbox.connected ? (
                        ...
                        >
                          {!import.meta.env.VITE_DROPBOX_APP_KEY ? '🔧 Not Configured' : 'Connect'}
                        </button>
                      )}
                    </div>
                  </div>
                  <div className="mt-3 flex flex-wrap gap-2">
                    <span className="px-2 py-1 bg-green-500/20 text-green-300 text-xs rounded">🔐 TLS Encryption</span>
                    <span className="px-2 py-1 bg-green-500/20 text-green-300 text-xs rounded">🗄️ AES-256 At-Rest</span>
                    <span className="px-2 py-1 bg-purple-500/20 text-purple-300 text-xs rounded">HIPAA Eligible</span>
                    <span className="px-2 py-1 bg-blue-500/20 text-blue-300 text-xs rounded">SOC 2/3</span>
                  </div>
                </div>
                ...
                <div className={`border rounded-lg p-4 transition-all ${
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="text-white font-semibold">Microsoft OneDrive</h4>
                        <p className="text-gray-400 text-xs">Great for Microsoft/VA computer users</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      {providerStates.onedrive.connected ? (
                        ...
                        >
                          {!import.meta.env.VITE_ONEDRIVE_CLIENT_ID ? '🔧 Not Configured' : 'Connect'}
                        </button>
                      )}
                    </div>
                  </div>
                  <div className="mt-3 flex flex-wrap gap-2">
                    <span className="px-2 py-1 bg-green-500/20 text-green-300 text-xs rounded">🔐 TLS Encryption</span>
                    <span className="px-2 py-1 bg-green-500/20 text-green-300 text-xs rounded">🗄️ BitLocker At-Rest</span>
                    <span className="px-2 py-1 bg-purple-500/20 text-purple-300 text-xs rounded">HIPAA Eligible</span>
                    <span className="px-2 py-1 bg-yellow-500/20 text-yellow-300 text-xs rounded">FedRAMP</span>
                  </div>
                </div>
              </div>
            </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {providerStates[selectedProvider]?.user && (
                      <p className="text-gray-400 text-sm">{providerStates[selectedProvider].user.email}</p>
                    )}
                  </div>
                </div>
              ...
              {providerStates[selectedProvider]?.connected && (
                <>
                  ...
                  <div className="bg-gray-800 border border-gray-700 rounded-lg p-4">
                    <h3 className="text-white font-semibold mb-3">Create New Backup</h3>
                    ...
                    <div className="mb-4 space-y-3">
                      <label className="flex items-center gap-2 cursor-pointer">
                        ...
                        />
                        <span className="text-gray-300">🔐 Encrypt backup (AES-256-GCM)</span>
                      ...
                      {encryptionEnabled && (
                        <label className="flex items-center gap-2 cursor-pointer ml-6">
                          <input
                            ...
                            className="rounded text-cyan-500"
                          />
                          <span className="text-gray-300">🔑 Use passphrase protection</span>
                          <span className="text-yellow-400 text-xs">Extra security</span>
                        </label>
                      )}
                      ...
                      {encryptionEnabled && !usePassphrase && (
                        <p className="text-gray-400 text-xs ml-6">
                          ℹ️ Encryption key stored locally. Backup can only be restored on this device.
                        </p>
                      )}
                    </div>
                  ...
                  <div className="bg-gray-800 border border-gray-700 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-white font-semibold">Your Backups ({backups.length})</h3>
                      <button
                        onClick={loadBackups}
                        disabled={isLoading}
                    ...
                    {backups.length === 0 ? (
                      <p className="text-gray-400 text-center py-6">No backups found. Create your first backup above.</p>
                    ) : (
                      <div className="space-y-2 max-h-64 overflow-y-auto">
                        {backups.map((backup) => (
                          <div key={backup.id} className="bg-gray-900 border border-gray-700 rounded p-3 flex items-center justify-between">
                            <div>
                              <p className="text-white text-sm font-medium">{backup.name}</p>
                              <p className="text-gray-400 text-xs">
                                {formatDate(backup.createdTime || backup.modified)} • {formatSize(backup.size)}
                              </p>
                            </div>
          ...
          {activeTab === 'settings' && (
            <div className="space-y-6">
              ...
              <div className="bg-gradient-to-r from-green-500/10 to-cyan-500/10 border border-green-400/30 rounded-lg p-6">
                <h3 className="text-green-300 font-bold text-lg mb-3">🔐 Military-Grade Encryption</h3>
                <div className="space-y-3 text-gray-300">
                  <p>Your backups are protected with:</p>
                  <ul className="space-y-2 ml-4">
                    <li className="flex items-start gap-2">
                      <span className="text-green-400">✓</span>
                      <span><strong>AES-256-GCM</strong> - Same encryption used by the US military for classified data</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-green-400">✓</span>
                      <span><strong>PBKDF2</strong> - 100,000 iterations for passphrase-based keys (OWASP recommended)</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-green-400">✓</span>
                      <span><strong>Client-Side Only</strong> - Encryption happens in YOUR browser, never on a server</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-green-400">✓</span>
                      <span><strong>Zero Knowledge</strong> - We can't read your data even if we wanted to</span>
                    </li>
                  </ul>
                </div>
              </div>
              ...
              {/* Crypto API Status */}
              <div className="bg-gray-800 border border-gray-700 rounded-lg p-4">
                <h4 className="text-white font-semibold mb-3">Security Status</h4>
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-gray-300">Web Crypto API</span>
                    <span className={isCryptoAvailable() ? 'text-green-400' : 'text-red-400'}>
                      {isCryptoAvailable() ? '✓ Available' : '✗ Not Available'}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-gray-300">HTTPS Connection</span>
                    <span className={location.protocol === 'https:' ? 'text-green-400' : 'text-yellow-400'}>
              ...
              <div className="bg-gray-800 border border-gray-700 rounded-lg p-4">
                <h4 className="text-white font-semibold mb-3">Cloud Provider Security</h4>
                ...
                <p className="text-gray-400 text-xs mt-3">
                  ~ Google Drive can be HIPAA compliant with Workspace + BAA. Personal accounts are not.
                </p>
              </div>
              ...
              <div className="bg-gradient-to-r from-red-500/10 to-orange-500/10 border border-red-400/30 rounded-lg p-4">
                ...
                <div className="text-gray-300 text-sm space-y-3">
                  ...
                  <div className="bg-gray-900/50 p-3 rounded-lg">
                    <p className="text-amber-300 font-medium mb-2">For HIPAA-eligible cloud storage:</p>
                    <ul className="list-disc list-inside text-gray-400 text-xs space-y-1">
                      <li><strong>Google:</strong> Requires Google Workspace + Business Associate Agreement (BAA)</li>
                      <li><strong>Dropbox:</strong> Dropbox Business with signed BAA</li>
                      <li><strong>OneDrive:</strong> Microsoft 365 Business/Enterprise with BAA</li>
                    </ul>
                  ...
                  <p className="text-gray-400 text-xs">
                    <strong>Your responsibility:</strong> If you are a healthcare provider or VSO handling 
                    veteran PHI, ensure you use appropriate enterprise accounts with signed BAAs.
                  </p>
                </div>
              </div>
            </div>
      ...
      {showPassphraseModal && (
        <div className="fixed inset-0 z-60 flex items-center justify-center p-4 bg-black/90">
          <div className="bg-gray-800 rounded-xl p-6 max-w-md w-full border border-cyan-500/30">
            <h3 className="text-xl font-bold text-white mb-4">🔑 Enter Passphrase</h3>
            ...
            <div className="flex gap-3">
              <button
                onClick={() => { setShowPassphraseModal(false); setPendingRestore(null); setRestorePassphrase(''); }}
