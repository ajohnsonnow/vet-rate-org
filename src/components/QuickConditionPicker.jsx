...
const conditionMatchesFilter = (condition, filter) =>
...
/**
 * Builds conditions by body system from disability data (static, computed once)
 */
const useConditionCatalog = () => {
  ...
  const sortedSystemNames = useMemo(() => {
    return Object.keys(conditionsBySystem).sort((a, b) => {
      const indexA = SYSTEM_ORDER.findIndex((prefix) => a.startsWith(prefix));
      const indexB = SYSTEM_ORDER.findIndex((prefix) => b.startsWith(prefix));
  ...
  const totalConditions = useMemo(() => {
    return Object.values(conditionsBySystem).reduce(
      (sum, arr) => sum + arr.length,
      0,
    );
  }, [conditionsBySystem]);

  return { conditionsBySystem, sortedSystemNames, totalConditions };
};

/**
 * Search filter over the condition catalog
 */
const useConditionSearch = (conditionsBySystem) => {
  ...
  const filteredSystems = useMemo(() => {
    ...
    Object.entries(conditionsBySystem).forEach(([system, conditions]) => {
      const matchingConditions = conditions.filter((condition) =>
        conditionMatchesFilter(condition, filter),
      );
  ...
  const filteredCount = useMemo(() => {
    return Object.values(filteredSystems).reduce(
      (sum, arr) => sum + arr.length,
      0,
    );
  }, [filteredSystems]);

  return { searchFilter, setSearchFilter, filteredSystems, filteredCount };
};

/**
 * Selected conditions + expanded body system sections
 */
const useConditionSelection = (filteredSystems) => {
  ...
  const selectAllInSystem = (system) => {
    ...
    const allSelected = conditionNames.every((name) =>
      selectedConditions.includes(name),
    );

    if (allSelected) {
      setSelectedConditions((prev) =>
        prev.filter((c) => !conditionNames.includes(c)),
      );
    } else {
      setSelectedConditions((prev) => [
        ...new Set([...prev, ...conditionNames]),
      ]);
    }
  };
...
/**
 * Saves selected conditions to My Packet and tracks the success banner
 */
const useAddToPacket = ({
  selectedConditions,
  conditionsBySystem,
  setSelectedConditions,
  onAddToPacket,
  ...
  return { addedCount, showSuccess, handleAddToPacket };
};

/**
 * Icon, title, and description
 */
const PickerHeader = ({ totalConditions }) => (
  <>
    ...
    <p className="text-sm text-gray-600 dark:text-gray-300 mb-4">
      Select your service-connected conditions to add them to{" "}
      <strong>My Packet</strong> for evidence building. Browse all{" "}
      {totalConditions} VA-rated conditions organized by body system.
    </p>
  </>
);

/**
 * Show/Hide the condition list
 */
const ExpandToggleButton = ({ isExpanded, onToggle, totalConditions }) => (
  <button
    onClick={onToggle}
 ...
 */
const SearchBox = ({ searchFilter, setSearchFilter, filteredCount }) => (
...
/**
 * A single condition checkbox row
 */
const ConditionCheckbox = ({ condition, isSelected, onToggle }) => (
  <label className="flex items-start text-xs text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700/50 px-1 py-0.5 rounded cursor-pointer">
    <input
      type="checkbox"
      checked={isSelected}
      onChange={onToggle}
...
/**
 * One collapsible body system with its conditions
 */
const BodySystemSection = ({
  system,
  conditions,
  isExpanded,
  selectedConditions,
    ...
    <div className="border border-gray-200 dark:border-gray-600 rounded-lg overflow-hidden">
      ...
      {isExpanded && (
        <div className="px-3 py-2 bg-white dark:bg-gray-800">
          ...
          <button
            onClick={() => onSelectAllInSystem(system)}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-3 gap-y-1">
            {conditions.map((condition) => (
              <ConditionCheckbox
                ...
                condition={condition}
                isSelected={selectedConditions.includes(condition.name)}
                onToggle={() => onToggleCondition(condition.name)}
              />
            ))}
          </div>
        </div>
...
/**
 * Scrollable list of body systems, or an empty-search state
 */
const BodySystemsList = ({
  sortedSystemNames,
  filteredSystems,
  expandedSystems,
  selectedConditions,
...
/**
 * Search box, body systems, and selected summary shown when expanded
 */
const ExpandedConditionList = ({
  searchFilter,
  setSearchFilter,
  filteredCount,
  sortedSystemNames,
  ...
  <div className="space-y-3">
    ...
    <BodySystemsList
      sortedSystemNames={sortedSystemNames}
...
/**
 * Confirms conditions were added to My Packet
 */
const SuccessMessage = ({ addedCount, onViewPacket }) => (
    ...
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm text-green-800 dark:text-green-200">
          {addedCount} condition{addedCount !== 1 ? "s" : ""} added to My
          Packet!
        </span>
      </div>
      {onViewPacket && (
...
/**
 * Add selected conditions to My Packet
 */
const AddToPacketButton = ({ selectedCount, onClick }) => (
  <button
    onClick={onClick}
  ...
  >
    {selectedCount > 0
      ? `📁 Add ${selectedCount} to My Packet`
...
const QuickConditionPicker = ({ onAddToPacket, onViewPacket }) => {
  ...
  const { conditionsBySystem, sortedSystemNames, totalConditions } =
    useConditionCatalog();
  const { searchFilter, setSearchFilter, filteredSystems, filteredCount } =
    useConditionSearch(conditionsBySystem);
  const {
    selectedConditions,
    setSelectedConditions,
    expandedSystems,
    ...
    selectAllInSystem,
    clearAll,
  } = useConditionSelection(filteredSystems);
  const { addedCount, showSuccess, handleAddToPacket } = useAddToPacket({
    selectedConditions,
    conditionsBySystem,
    setSelectedConditions,
    onAddToPacket,
  ...
  return (
    <div className="rounded-xl border border-blue-200 dark:border-blue-700 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/40 dark:to-indigo-900/40 p-5 transition-shadow hover:shadow-md">
      <PickerHeader totalConditions={totalConditions} />
      ...
      {isExpanded && (
        <ExpandedConditionList
          ...
          onSelectAllInSystem={selectAllInSystem}
      ...
      {showSuccess && (
        <SuccessMessage addedCount={addedCount} onViewPacket={onViewPacket} />
      )}

      <AddToPacketButton
        selectedCount={selectedConditions.length}
