...
const REGULATIONS_TABS = [
  { id: "regulations", label: "?? Claims (Part 3)" },
  { id: "appeals", label: "?? BVA Appeals" },
...
const RegulationsHeader = ({ onClose, activeTab, setActiveTab }) => (
...
const RegulationItem = ({ reg }) => (
  <div className="border-2 border-blue-100 dark:border-blue-900 rounded-lg p-4 bg-blue-50/50 dark:bg-blue-900/20">
    ...
    <p className="text-gray-700 dark:text-gray-300 mb-3">{reg.summary}</p>
...
const RegulationCategoryCard = ({
  category,
  expanded,
  onToggle,
  matchesSearch,
}) => {
  if (!category.regulations.some(matchesSearch)) return null;
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-amber-700 dark:text-amber-300">
            38 CFR Part 3 contains the rules VA MUST follow when deciding your
            claim. Knowing these regulations helps you file stronger claims,
            catch errors in denials, and effectively appeal unfavorable
            decisions. These are YOUR rights - use them!
          </p>
        </div>
      </div>
    </div>
...
const AppealRegulationItem = ({ reg }) => (
  <div className="border-2 border-purple-100 dark:border-purple-900 rounded-lg p-4 bg-purple-50/50 dark:bg-purple-900/20">
...
const AppealTimelines = () => {
  if (!title38Regulations.bvaAppeals?.appealTimelines) return null;
  return (
    <div className="mt-6">
...
const AppealForms = () => {
  if (!title38Regulations.bvaAppeals?.keyForms) return null;
  return (
    <div className="mt-6">
      <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100 mb-4">
        ?? Appeal Forms
...
const PensionRegulationItem = ({ reg }) => (
  <div className="border-2 border-amber-100 dark:border-amber-900 rounded-lg p-4 bg-amber-50/50 dark:bg-amber-900/20">
...
const PensionTab = ({ expandedCategories, toggleCategory, matchesSearch }) => (
  <>
    {/* Pension Overview */}
    <div className="bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-700 rounded-lg p-4 mb-6">
      <div className="flex items-start gap-3">
...
const expandAllCategories = (setExpandedCategories) => {
  const allExpanded = {};
...
const RegulationsReference = ({ onClose }) => {
  ...
  const expandAll = () => expandAllCategories(setExpandedCategories);
    ...
    >
      ...
      {activeTab === "regulations" && (
        <RegulationsTab
          ...
          matchesSearch={_regMatchesSearch}
        />
      )}
