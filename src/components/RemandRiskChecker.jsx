...
const RISK_LEVEL_STYLES = {
  low: {
    ...
    message: "✓ LOW Remand Risk",
  },
  moderate: {
    bg: "bg-yellow-100 dark:bg-yellow-900/30",
    percentage: "text-yellow-600",
    label: "text-yellow-700 dark:text-yellow-400",
    message: "⚠ MODERATE Remand Risk",
  },
  high: {
    bg: "bg-orange-100 dark:bg-orange-900/30",
    percentage: "text-orange-600",
    label: "text-orange-700 dark:text-orange-400",
    message: "⚠ HIGH Remand Risk",
  },
  critical: {
    bg: "bg-red-100 dark:bg-red-900/30",
    percentage: "text-red-600",
    label: "text-red-700 dark:text-red-400",
    message: "🚨 CRITICAL Risk - Address Gaps First",
  },
};

/**
 * Scores answered questions into a 0-100 remand-risk percentage and
 * collects the unmet evidence gaps driving that score.
 */
function useRemandRiskAnalysis(answers) {
  return useMemo(() => {
...
function PriorDenialsInput({ priorDenials, setPriorDenials, persistenceMsg }) {
  return (
...
function EvidenceChecklist({ answers, onAnswer }) {
  return (
    <div className="space-y-4">
      ...
      {questions.map((q, idx) => {
        ...
        let cardClassName = "border-gray-200 dark:border-gray-600";
        if (answerValue === "yes") {
          cardClassName =
        ...
        return (
          <div
            key={q.id}
            className={`p-4 rounded-lg border-2 transition-colors ${cardClassName}`}
          >
...
function AnalysisResults({ analysis }) {
  if (analysis.answeredCount === 0) return null;

  const riskStyle =
    RISK_LEVEL_STYLES[analysis.riskLevel] ?? RISK_LEVEL_STYLES.critical;

  return (
    <div className="space-y-4">
      {/* Risk Score */}
      <div className={`p-5 rounded-lg ${riskStyle.bg}`}>
        <div className="flex items-center gap-4">
          <div className={`text-5xl font-bold ${riskStyle.percentage}`}>
            {analysis.riskPercentage}%
          </div>
          <div>
            <div className={`text-xl font-bold ${riskStyle.label}`}>
              {riskStyle.message}
            </div>
      ...
      <IdentifiedGaps gaps={analysis.gaps} />
      ...
      {analysis.isComplete && analysis.riskLevel === "low" && (
        <div className="bg-green-100 dark:bg-green-900/30 border border-green-300 dark:border-green-700 rounded-lg p-4 text-center">
          ...
          <p className="text-sm text-green-600 dark:text-green-500 mt-1">
            Based on BVA decision patterns, your claim appears well-documented.
            You&apos;ve addressed the most common remand reasons.
          </p>
        </div>
      )}
    </div>
...
function RemandReasonsReference({ showDetails, setShowDetails }) {
  return (
    <div>
      <button
...
const RemandRiskChecker = ({ onClose }) => {
  ...
  const analysis = useRemandRiskAnalysis(answers);
    ...
    >
      <div className="space-y-6">
        ...
        <PriorDenialsInput
          ...
          setPriorDenials={setPriorDenials}
          persistenceMsg={persistenceMsg}
        />
        <EvidenceChecklist answers={answers} onAnswer={handleAnswer} />
        <AnalysisResults analysis={analysis} />
        <RemandReasonsReference
