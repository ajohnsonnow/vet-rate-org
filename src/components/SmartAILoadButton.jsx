...
import { getRecommendedModelForDevice, checkModelMatch, smartLoadAI, getDeviceType } from '../utils/smartAILoader';
import { getAIStatus } from '../utils/unifiedAIService';

const SmartAILoadButton = ({ toolId, compact = false, onLoadComplete = null }) => {
    ...
    if (compact) {
      return (
        <div className="flex items-center gap-2 px-3 py-2 bg-green-500/10 border border-green-500/30 rounded-lg">
          <span className="text-green-400 text-xs">✅ {check.recommendedModel.name}</span>
        </div>
      );
    }
      ...
      <div className="p-4 bg-green-900/20 border border-green-700 rounded-xl">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-green-500/70 text-xs mt-1">{check.recommendedModel.reason}</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <h4 className="text-cyan-300 font-semibold text-sm">
            {check.action === 'switch' ? 'Recommended: Switch Model' : 'Load AI for This Tool'}
          </h4>
          <p className="text-cyan-400 text-xs mt-1">
            Device: <span className="font-medium capitalize">{deviceType}</span>
          </p>
          <p className="text-cyan-400 text-xs">
            Recommended: <span className="font-medium">{check.recommendedModel.name}</span>
          </p>
          <p className="text-cyan-500/70 text-xs mt-1">{check.recommendedModel.reason}</p>
          ...
          {check.action === 'switch' && check.currentModel && (
            <p className="text-xs text-gray-400 mt-2">
              Currently: {check.currentModel.split('-')[0]} (will be unloaded first)
            </p>
          )}
        </div>
      </div>
