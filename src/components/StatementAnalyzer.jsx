...
const fetchToneSuggestions = async (text) => {
  ...
  return parseToneAnalysisResponse(responseStr);
};

const saveAppliedSuggestion = (suggestion) => {
  saveAnalysisResults({
...
const parseToneAnalysisResponse = (responseStr) => {
  try {
...
const SuggestionCard = ({ suggestion, index, isApplied, onApply }) => (
  <div
  ...
  >
    ...
    <div className="flex items-start gap-2 mb-2">
      ...
      {isApplied && <Check className="w-5 h-5 text-green-600" />}
    </div>
...
const AnalyzerHeader = ({ isAnalyzing, suggestionCount }) => (
...
const AnalyzerEmptyState = ({ isAnalyzing, suggestionsLength, text }) => {
  if (isAnalyzing || suggestionsLength > 0) {
    return null;
  }
  ...
  return (
    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
      <p className="text-sm text-blue-800">
        <strong>The Diplomat</strong> helps you write clinical, professional
        statements that resonate with VA raters. Start typing your statement,
        and I&apos;ll analyze the tone automatically.
      </p>
      <p className="text-xs text-blue-700 mt-2">
        💡 Tip: Avoid emotional language. Focus on facts, measurements, and
        specific impacts.
      </p>
    </div>
  );
...
const StatementAnalyzer = ({ text, onApplySuggestion, className = "" }) => {
  ...
  const analyzeStatement = async () => {
    ...
    try {
      const parsedSuggestions = await fetchToneSuggestions(text);
  ...
  const handleApplySuggestion = (suggestion, index) => {
    if (onApplySuggestion) {
      ...
      saveAppliedSuggestion(suggestion);
    }
  };
