...
const SYMPTOM_TYPES = {
  migraine: {
    ...
    ratingCriteria: [
      {
        rating: 50,
        description: "Prostrating attacks occurring on average once a month",
      },
      {
        rating: 30,
        description: "Prostrating attacks averaging once every 2 months",
      },
      {
        rating: 10,
        description: "Prostrating attacks averaging one in 2 months",
      },
    ...
    ratingCriteria: [
      {
        rating: 30,
        description:
          "Severe; diarrhea or alternating with constipation, with more or less constant abdominal distress",
      },
      {
        rating: 10,
        description:
    ...
    ratingCriteria: [
      {
        rating: 40,
        description:
          "Constant or near-constant pain with severe functional impairment",
      },
      {
        rating: 20,
        description: "Frequent episodes with moderate functional impairment",
      },
      {
        rating: 10,
        description: "Intermittent pain with mild functional impairment",
      },
      {
        rating: 0,
        description: "Occasional pain not requiring frequent treatment",
      },
    ],
    durationOptions: [
      "< 1 hour",
  ...
  fatigue: {
    ...
    ratingCriteria: [
      {
        rating: 100,
        description: "Bed/couch-bound, unable to perform basic self-care",
      },
      {
        rating: 60,
        description:
          "Severely limited activity, frequent rest periods required",
      },
    ...
    ratingCriteria: [
      {
        rating: 50,
        description:
          "Chronic sleep impairment with total occupational and social impairment",
      },
      {
        rating: 30,
        description:
          "Persistent difficulty falling or staying asleep affecting daily function",
      },
      {
        rating: 10,
        description: "Occasional sleep disturbance with mild impairment",
      },
    ...
    ratingCriteria: [
      {
        rating: 30,
        description:
          "Severe/constant distress requiring frequent medical management",
      },
      {
        rating: 10,
        description: "Frequent episodes requiring dietary restrictions",
      },
  ...
  mental: {
    ...
    ratingCriteria: [
      ...
      {
        rating: 50,
        description: "Significant impairment in work and social functioning",
      },
      {
        rating: 30,
        description: "Moderate symptoms with occasional impairment",
      },
    ...
    ratingCriteria: [
      {
        rating: "varies",
        description:
          "Track any symptoms to document frequency and severity patterns",
      },
    ],
    durationOptions: [
      "Brief (< 1 hour)",
...
const addPDFTitleSection = (doc, config, totalEntries, margin, startY) => {
  ...
  doc.text(
    `Generated: ${new Date().toLocaleDateString()} | Total Entries: ${totalEntries}`,
    margin,
    y,
  );
  y += 15;
...
const addPDFStatsSummary = (doc, stats, pageWidth, margin, startY) => {
  let y = startY;
  ...
  doc.setFont(undefined, "normal");
  doc.text(
    `• Prostrating attacks in last 30 days: ${stats.prostratingLast30}`,
    margin + 5,
    y,
  );
  y += 6;
  doc.text(
    `• Prostrating attacks in last 90 days: ${stats.prostratingLast90}`,
    margin + 5,
    y,
  );
  y += 6;
  doc.text(
    `• Average prostrating attacks per month (90-day): ${stats.avgPerMonth}`,
    margin + 5,
    y,
  );
  y += 6;
...
const addPDFRatingCriteriaSection = (doc, config, margin, startY) => {
  let y = startY;
...
const addPDFLogEntry = (doc, log, index, pageWidth, margin, startY) => {
  let y = startY;
  ...
  doc.setFont(undefined, "normal");
  doc.text(
    `   Severity: ${log.severity}/10 | Duration: ${log.duration} | Prostrating: ${log.prostrating ? "YES" : "No"}`,
    margin,
    y,
  );
  y += 5;
  ...
  if (log.weather || log.stressLevel !== undefined) {
    ...
    if (log.stressLevel !== undefined)
      envInfo += `Stress Level: ${log.stressLevel}/10`;
...
const addPDFFooter = (doc, margin) => {
  // Footer
  doc.setFontSize(8);
  doc.setTextColor(150);
  doc.text("Generated by Vet-Rate.org Symptom Logger", margin, 285);
  doc.text(
    "This log is veteran-generated evidence for VA disability claims.",
    margin,
    289,
  );
};
...
const SymptomTypeSelector = ({ symptomType, setSymptomType }) => (
...
const TabNavigation = ({ activeTab, setActiveTab, colors, logs, symptomType }) => (
        <div className="px-6 pt-4 border-b dark:border-gray-700">
          <nav className="flex gap-1">
            {[
              ...
              {
                id: "history",
                label: `📋 History (${logs.filter((l) => l.type === symptomType).length})`,
                icon: "📋",
              },
...
const TrackFrequencyBanner = ({ config, colors }) => (
              <div
                className={`p-4 ${colors.bgLight} ${colors.border} border rounded-xl`}
              >
...
const DateTimeFields = ({ newLog, setNewLog }) => (
  <>
                {/* Date & Time */}
                <div>
...
const SeverityDurationFields = ({ newLog, setNewLog, colors, config }) => (
  <>
                {/* Severity */}
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {config.durationOptions.map((option) => (
                      <button
                        key={option}
                        onClick={() =>
                          setNewLog((prev) => ({ ...prev, duration: option }))
                        }
...
const BodyLocationPainScaleFields = ({ newLog, setNewLog, symptomType }) => (
  <>
                ...
                <div className="md:col-span-2">
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    💡 Tip: Use the Somatic Target tool to select precise body
                    locations
                  </p>
                </div>
...
const StressProstratingMedicationFields = ({ newLog, setNewLog, config }) => (
  <>
...
const LogAttackTab = ({
  ...
  <div className="space-y-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <SeverityDurationFields
        newLog={newLog}
        setNewLog={setNewLog}
        colors={colors}
        config={config}
      />
      <BodyLocationPainScaleFields
        newLog={newLog}
        setNewLog={setNewLog}
        symptomType={symptomType}
      />
      <ActivityWeatherFields
        newLog={newLog}
        setNewLog={setNewLog}
        aiStatus={aiStatus}
        isAIGenerating={isAIGenerating}
        generateAISuggestion={generateAISuggestion}
      />
      <StressProstratingMedicationFields
        newLog={newLog}
        setNewLog={setNewLog}
        config={config}
      />
...
const EmptyHistoryState = ({ config, colors, setActiveTab }) => (
                <div className="text-center py-12 text-gray-500 dark:text-gray-400">
                  <div className="text-6xl mb-4">{config.emoji}</div>
                  <p className="text-lg font-medium">No entries yet</p>
                  <p className="text-sm mt-2">
                    Start logging your {config.label.toLowerCase()} attacks to
...
const HistoryTab = ({
  ...
  <div className="space-y-4">
    {logs.filter((log) => log.type === symptomType).length === 0 ? (
      <EmptyHistoryState config={config} colors={colors} setActiveTab={setActiveTab} />
    ) : (
...
const StatsGrid = ({ stats }) => (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div
                  className={`rounded-xl p-4 text-center ${stats.suggestedRating >= 30 ? "bg-green-100 dark:bg-green-900/50" : "bg-gray-50 dark:bg-gray-900"}`}
                >
                  <div
                    className={`text-3xl font-bold ${stats.suggestedRating >= 30 ? "text-green-700 dark:text-green-300" : "text-gray-800 dark:text-gray-200"}`}
                  >
                    {stats.suggestedRating}%
                  </div>
                  <div
                    className={`text-xs mt-1 ${stats.suggestedRating >= 30 ? "text-green-600 dark:text-green-400" : "text-gray-500 dark:text-gray-400"}`}
                  >
                    Suggested
                    <br />
                    VA Rating
...
const RatingCriteriaSection = ({ stats, colors, config }) => (
  <>
              {/* Rating Criteria Reference */}
              <div
                className={`p-4 ${colors.bgLight} ${colors.border} border rounded-xl`}
...
const ExportPDFButtonSection = ({ logs, symptomType, handleExportPDF }) => (
  <>
              ...
              <button
                ...
                disabled={
                  logs.filter((l) => l.type === symptomType).length === 0
                }
              ...
              <p className="text-xs text-center text-gray-500 dark:text-gray-400">
                Print this and bring it to your C&P exam as documented frequency
                evidence
              </p>
  </>
);

const ExportTab = ({ stats, colors, config, logs, symptomType, handleExportPDF }) => (
  <div className="space-y-6">
    <StatsGrid stats={stats} />
    <RatingCriteriaSection stats={stats} colors={colors} config={config} />
    <ExportPDFButtonSection
      logs={logs}
      symptomType={symptomType}
