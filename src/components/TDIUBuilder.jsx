...
const DISABILITY_CATEGORIES = [
  {
    ...
    icon: '🧠',
    conditions: [
      { name: 'PTSD', commonSymptoms: ['Hypervigilance', 'Flashbacks', 'Difficulty concentrating', 'Anger outbursts', 'Sleep disturbances', 'Avoidance of triggers'] },
      { name: 'Major Depression', commonSymptoms: ['Fatigue', 'Lack of motivation', 'Difficulty concentrating', 'Hopelessness', 'Sleep problems', 'Social withdrawal'] },
      { name: 'Anxiety Disorder', commonSymptoms: ['Panic attacks', 'Racing thoughts', 'Difficulty focusing', 'Irritability', 'Physical tension', 'Avoidance behaviors'] },
      { name: 'Bipolar Disorder', commonSymptoms: ['Mood swings', 'Manic episodes', 'Depressive episodes', 'Impulsivity', 'Sleep disturbances', 'Concentration issues'] },
    ]
  },
  {
    ...
    icon: '🦴',
    conditions: [
      { name: 'Lumbar Spine / Back', commonSymptoms: ['Cannot sit >15-30 mins', 'Cannot stand >15-30 mins', 'Cannot lift >10-20 lbs', 'Requires frequent position changes', 'Pain with bending/twisting', 'Radiating leg pain'] },
      { name: 'Cervical Spine / Neck', commonSymptoms: ['Limited head rotation', 'Pain looking up/down', 'Headaches from strain', 'Numbness in arms', 'Cannot work overhead', 'Pain from prolonged reading'] },
      { name: 'Knee Condition', commonSymptoms: ['Cannot stand >15-30 mins', 'Cannot walk >1 block', 'Difficulty with stairs', 'Instability/giving way', 'Cannot squat or kneel', 'Pain with weather changes'] },
      { name: 'Shoulder Condition', commonSymptoms: ['Cannot reach overhead', 'Cannot lift >5-10 lbs', 'Pain with repetitive motion', 'Limited range of motion', 'Weakness in arm', 'Difficulty dressing'] },
    ]
  },
  {
    ...
    icon: '⚡',
    conditions: [
      { name: 'Migraines', commonSymptoms: ['2+ prostrating attacks/month', 'Requires dark room rest', 'Light/sound sensitivity', 'Nausea/vomiting', 'Cannot drive during episode', 'Last 4-72 hours each'] },
      { name: 'TBI Residuals', commonSymptoms: ['Memory problems', 'Difficulty processing information', 'Headaches', 'Dizziness/balance issues', 'Mood changes', 'Light sensitivity'] },
      { name: 'Peripheral Neuropathy', commonSymptoms: ['Numbness in hands/feet', 'Burning/tingling pain', 'Loss of fine motor control', 'Difficulty with buttons/typing', 'Balance problems', 'Sensitivity to touch'] },
    ]
  },
  {
    ...
    icon: '🫀',
    conditions: [
      { name: 'Sleep Apnea', commonSymptoms: ['Daytime fatigue', 'Cannot stay awake during work', 'Requires CPAP', 'Memory/concentration issues', 'Morning headaches', 'Cannot work night shifts'] },
      { name: 'Heart Condition', commonSymptoms: ['Shortness of breath', 'Cannot exert >3 METs', 'Fatigue with minimal activity', 'Chest pain with stress', 'Requires frequent rest', 'Cannot lift >20 lbs'] },
      { name: 'Respiratory (Asthma/COPD)', commonSymptoms: ['Shortness of breath', 'Cannot work in dust/fumes', 'Frequent breathing treatments', 'Limited physical exertion', 'Coughing spells', 'Weather/allergy triggers'] },
      { name: 'Diabetes', commonSymptoms: ['Requires insulin', 'Blood sugar fluctuations', 'Cannot skip meals', 'Fatigue', 'Vision problems', 'Neuropathy symptoms'] },
    ]
  }
];
...
const getTemplateImpact = (condition, symptom) => {
  ...
  // Concentration/focus
  if (lowerSymptom.includes('concentrat') || lowerSymptom.includes('focus') || lowerSymptom.includes('memory')) {
  ...
  // Fatigue
  if (lowerSymptom.includes('fatigue') || lowerSymptom.includes('tired') || lowerSymptom.includes('sleep')) {
  ...
  if (lowerSymptom.includes('anger') || lowerSymptom.includes('irritab') || lowerSymptom.includes('social')) {
  ...
  // Attacks/episodes
  if (lowerSymptom.includes('attack') || lowerSymptom.includes('episode') || lowerSymptom.includes('flare')) {
...
export default function TDIUBuilder({ onClose, onReportBug, onOpenAISettings }) {
  ...
  const [currentDisability, setCurrentDisability] = useState({ condition: '', symptoms: [] });
  ...
  const getCommonSymptoms = () => {
    for (const category of DISABILITY_CATEGORIES) {
      const condition = category.conditions.find(c => c.name === currentDisability.condition);
  ...
  const getFullStatement = () => {
    ...
    statement += vocationalAnalysis.job_types_precluded.map(j => `• ${j} Work`).join('\n');
    statement += `\n\n`;
    ...
    if (workHistory.lastWorked || workHistory.reasonLeft) {
      ...
      if (workHistory.lastWorked) statement += `Last Worked: ${workHistory.lastWorked}\n`;
      if (workHistory.lastOccupation) statement += `Last Occupation: ${workHistory.lastOccupation}\n`;
      if (workHistory.reasonLeft) statement += `Reason Left: ${workHistory.reasonLeft}\n`;
      if (workHistory.education) statement += `Education Level: ${workHistory.education}\n`;
      if (workHistory.triedToWork) statement += `Attempts to Work: ${workHistory.triedToWork}\n`;
    }
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="bg-green-50 dark:bg-green-900/30 border-l-4 border-green-500 p-4 rounded-r-lg">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="font-bold text-green-800 dark:text-green-200">What is TDIU?</h3>
            <p className="text-green-700 dark:text-green-300 text-sm mt-1">
              <strong>Total Disability Individual Unemployability</strong> pays you at the 100% rate even if your combined rating is only 60-70%. 
              You qualify if you <strong>cannot maintain substantially gainful employment</strong> due to your service-connected disabilities.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
          ...
          <div className="space-y-3">
            {disabilities.map((d, i) => (
              <div key={i} className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4 relative">
                <button
                ...
                <p className="font-semibold text-gray-800 dark:text-gray-100">{d.condition}</p>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
        ...
        <div className="mb-4">
          ...
          <select
            ...
            onChange={(e) => setCurrentDisability({ condition: e.target.value, symptoms: [] })}
            ...
            {getCommonSymptoms().length > 0 && (
              <div className="flex flex-wrap gap-2 mb-3">
                {getCommonSymptoms().map(symptom => (
                  ...
                  >
                    {currentDisability.symptoms.includes(symptom) ? '✓ ' : ''}{symptom}
                  </button>
                ))}
              </div>
            )}
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              onChange={(e) => setWorkHistory(prev => ({ ...prev, lastWorked: e.target.value }))}
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setWorkHistory(prev => ({ ...prev, lastOccupation: e.target.value }))}
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setWorkHistory(prev => ({ ...prev, education: e.target.value }))}
              className="w-full p-3 border-2 border-gray-200 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100"
            >
  ...
  const renderResultsStep = () => (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
        ...
        <div className="flex flex-wrap gap-3">
          {vocationalAnalysis?.job_types_precluded?.map(job => (
            <span key={job} className="px-4 py-2 bg-red-100 dark:bg-red-900/50 text-red-700 dark:text-red-300 rounded-lg font-medium">
              ✕ {job} Work
            </span>
          ))}
        </div>
          ...
          {vocationalAnalysis?.limitations?.map((lim, i) => (
            <div key={i} className="p-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-medium text-gray-800 dark:text-gray-100">{lim.symptom}</p>
                  <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">{lim.vocational_impact}</p>
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="bg-amber-50 dark:bg-amber-900/30 border-l-4 border-amber-500 p-4 rounded-r-lg">
        <h3 className="font-bold text-amber-800 dark:text-amber-200 mb-2">🔗 Combined Effect</h3>
        <p className="text-amber-700 dark:text-amber-300">{vocationalAnalysis?.combined_effect}</p>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h3 className="font-bold text-gray-800 dark:text-gray-100">Download Full Report</h3>
      ...
      <div className="bg-blue-50 dark:bg-blue-900/30 border-l-4 border-blue-500 p-4 rounded-r-lg">
        <h3 className="font-bold text-blue-800 dark:text-blue-200 mb-2">📋 Next Steps</h3>
        <ol className="text-blue-700 dark:text-blue-300 text-sm space-y-1 list-decimal list-inside">
          <li>Complete <strong>VA Form 21-8940</strong> (Application for Unemployability)</li>
          <li>Paste the Box 18 statement into the employment limitation section</li>
          <li>Complete <strong>VA Form 21-4192</strong> (Request for Employment Information) - have your last employer fill this out</li>
      ...
      <button
        onClick={() => {
          ...
          setWorkHistory({ lastWorked: '', lastOccupation: '', reasonLeft: '', education: '', triedToWork: '' });
        }}
              ...
              {[1, 2, 3].map(s => (
                <div key={s} className="flex items-center">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold ${
                    step >= s ? 'bg-green-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400'
                  }`}>
                    {step > s ? '✓' : s}
                  </div>
                  {s < 3 && (
                    <div className={`w-16 h-1 ${step > s ? 'bg-green-600' : 'bg-gray-200 dark:bg-gray-700'}`}></div>
                  )}
                </div>
              ))}
            </div>
            <div className="flex justify-center gap-8 text-sm text-gray-500 dark:text-gray-400 mb-6">
              <span className={step === 1 ? 'font-bold text-green-600' : ''}>Disabilities</span>
              ...
              <div className="bg-gradient-to-r from-amber-900/40 to-yellow-900/40 rounded-2xl p-6 border border-amber-700/50 mt-6">\n                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-green-300/70 text-sm">
                      TDIU claims are complex. Most veterans hire expensive consultants just to translate 
                      their symptoms into "occupational limitations." You just did it for free. 
                      Help keep this tool available for every veteran fighting for 100%.
                    </p>
                  </div>
                </div>
              </div>
