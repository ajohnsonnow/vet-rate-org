...
// Find the value for the highest threshold key that tokenLimit meets or exceeds
const findClosestThresholdValue = (thresholdMap, tokenLimit) => {
...
const getWarningForModel = (currentModel, tokenLimit) => {
  ...
  return findClosestThresholdValue(currentModel.warnings, tokenLimit);
};

const getVRAMImpactForModel = (currentModel, aiStatus, tokenLimit) => {
  if (aiStatus.effectiveMode !== "local") {
  ...
  return findClosestThresholdValue(currentModel.vramImpact, tokenLimit);
};

const getWarningPanelClasses = (exceedsCapacity, exceedsRecommended) => {
  if (exceedsCapacity) {
    return "bg-red-50 dark:bg-red-900/20 border-red-500 dark:border-red-700";
  }
  if (exceedsRecommended) {
    return "bg-yellow-50 dark:bg-yellow-900/20 border-yellow-500 dark:border-yellow-700";
  }
  return "bg-blue-50 dark:bg-blue-900/20 border-blue-500 dark:border-blue-700";
};

const getWarningIcon = (exceedsCapacity, exceedsRecommended) => {
  if (exceedsCapacity) return "🚫";
  if (exceedsRecommended) return "⚠️";
  return "ℹ️";
};

const ConfigHeader = ({ showAdvanced, onToggleAdvanced }) => (
...
const CurrentModelInfo = ({ currentModel, tokenLimit }) => (
...
const PresetButtons = ({ tokenLimit, isCustom, onSelect }) => (
...
const CustomInput = ({ customValue, onChange, onApply, currentModel }) => (
...
const WarningsPanel = ({
  ...
  if (!warning && !exceedsRecommended && !exceedsCapacity) return null;

  return (
    <div
      className={`rounded-lg p-4 border-2 ${getWarningPanelClasses(exceedsCapacity, exceedsRecommended)}`}
    >
      <div className="flex items-start gap-3">
        <span className="text-2xl">
          {getWarningIcon(exceedsCapacity, exceedsRecommended)}
        </span>
        <div>
          {exceedsCapacity && (
          ...
          {aiStatus.effectiveMode === "cloud" && tokenLimit >= 4096 && (
            <div className="mt-2 pt-2 border-t border-gray-300 dark:border-gray-700">
              <p className="text-sm text-gray-700 dark:text-gray-300">
                💰 Higher token limits increase API costs and response times
                with Google Gemini.
              </p>
            </div>
          )}
...
const AdvancedDetails = ({ currentModel, aiStatus }) => (
  <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4 border border-gray-300 dark:border-gray-700 space-y-3">
    ...
    <div className="pt-3 border-t border-gray-300 dark:border-gray-700">
      ...
      <ul className="list-disc ml-5 space-y-1 text-sm text-gray-700 dark:text-gray-300">
        <li>
          <strong>Recommended Maximum:</strong>{" "}
          {currentModel.recommendedMax.toLocaleString()} tokens
        </li>
        <li>
          <strong>Context Window:</strong>{" "}
          {currentModel.maxContext.toLocaleString()} tokens total
        </li>
...
const TokenLimitConfig = () => {
  ...
  return (
    <div className="space-y-4">
      ...
      <CurrentModelInfo currentModel={currentModel} tokenLimit={tokenLimit} />
      ...
      {/* Advanced Details */}
      {showAdvanced && (
        <AdvancedDetails currentModel={currentModel} aiStatus={aiStatus} />
      )}
