...
function UnityTutorHeader({ onClose }) {
  return (
...
function LanguageAndCategorySelectors({
  targetLang,
  onLanguageChange,
  phraseCategory,
  onCategoryChange,
  ...
  return (
    <div className="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
      ...
      <div>
        ...
        >
          {Object.entries(LANGUAGE_TRANSLATIONS).map(([code, lang]) => (
...
function PhraseListView({
  ...
  const module = LEARNING_MODULES[phraseCategory];
    ...
    <div className="space-y-2">
      ...
      {module.phrases.map((phrase) => {
        ...
        return (
          <button
            key={phrase.key}
            onClick={() => onSelectPhrase(phrase.key, phrase.en)}
...
function LearningCard({
  ...
  const lang = LANGUAGE_TRANSLATIONS[targetLang];
...
const UnityLanguageTutor = ({ isOpen = false, onClose, className = "" }) => {
  ...
  const {
    ...
    markLearned,
    getPhraseProgress,
  } = useUnityPhraseLearning(targetLang);
    ...
    >
      ...
      {!currentPhrase ? (
        ...
        <LearningCard
          ...
          targetLang={targetLang}
          feedback={feedback}
          onPronounceSlowly={pronounceSlowly}
          onPronounceNormal={pronounceNormal}
          onBack={() => setCurrentPhrase(null)}
