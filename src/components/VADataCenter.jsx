...
import { isVaIntegrationConfigured, getVaConfigStatus, VA_FACILITIES_API_KEY, VA_FORMS_API_KEY, VA_BENEFITS_REF_API_KEY } from '../config/vaAuth';
import { saveVADataWithConsent, loadVARecords, clearVARecords } from '../utils/vaDataPersistence';
import {
  getServiceHistory,
  getClaims,
  getAppealableIssues,
  ...
  oauth: [
    {
      ...
      icon: Medal,
      description: 'Your military service record including branches, dates, deployments, and discharge status.',
      useCase: 'Pre-fill claim forms with accurate service dates. Verify eligibility for benefits.',
      dataPoints: ['Branch of Service', 'Service Dates', 'Discharge Status', 'Deployments', 'Character of Service'],
    ...
    },
    {
      ...
      icon: FileText,
      description: 'Your current and past VA disability claims with status updates.',
      useCase: 'Track claim progress. Identify which conditions are rated. Plan supplemental claims.',
      dataPoints: ['Claim Status', 'Submission Date', 'Rated Conditions', 'Phase Dates', 'Supporting Documents'],
      scope: 'claim.read',
    },
    {
      id: 'appealsStatus',
      name: 'Appeals Status',
      icon: Gavel,
      description: 'Status of any appeals you have filed (Board Appeals, Higher Level Reviews).',
      useCase: 'Monitor appeal progress. Prepare for hearings. Track decision timelines.',
      dataPoints: ['Appeal Type', 'Status', 'Docket Number', 'Events Timeline', 'Hearing Info'],
      scope: 'appeals.read',
    },
    {
      id: 'appealableIssues',
      ...
      useCase: 'Identify issues eligible for Supplemental Claim or Higher Level Review.',
      dataPoints: ['Decision Date', 'Issue Description', 'Rating Decision ID', 'Appeal Deadline'],
      scope: 'appeals.read',
    },
  ],
  ...
  apiKey: [
    {
      ...
      name: 'VA Facilities',
      icon: Building2,
      description: 'Find VA Medical Centers, Clinics, and regional offices near you.',
      useCase: 'Locate C&P exam sites. Find nearest VA healthcare. Get facility contact info.',
      dataPoints: ['Facility Name', 'Address', 'Phone', 'Services', 'Hours', 'Wait Times'],
      requiresInput: true,
    ...
    },
    {
      ...
      name: 'VA Forms',
      icon: ListChecks,
      description: 'Search the entire VA forms library - DBQs, claim forms, appeals forms.',
      useCase: 'Find the right form for your claim. Download DBQs for medical evidence.',
      dataPoints: ['Form Number', 'Title', 'Description', 'Last Updated', 'PDF Link'],
      requiresInput: true,
    ...
    },
    {
      ...
      icon: BookOpen,
      description: 'Official list of ratable disabilities from the VA schedule.',
      useCase: 'Find diagnostic codes. Understand rating criteria. Research conditions.',
      dataPoints: ['Condition Name', 'Diagnostic Code', 'Rating Criteria', 'Body System'],
      requiresInput: false,
    },
  ],
};
...
const VADataCenter = ({ onClose, embeddedMode = false }) => {
  ...
  // FETCH FUNCTIONS
  // =========================================================================
  const fetchApi = useCallback(async (apiId) => {
    ...
    try {
      ...
      switch (apiId) {
        ...
        case 'forms':
          ...
          break;
        case 'disabilities':
          data = await getBenefitsReferenceDisabilities(VA_BENEFITS_REF_API_KEY);
  ...
  const handleSaveSelected = async () => {
    ...
    try {
      const dataToSave = {
        serviceHistory: selectedApis.serviceHistory && apiData.serviceHistory?.formatted?.[0] ? apiData.serviceHistory.formatted[0] : null,
        claims: selectedApis.claims && apiData.claims?.formatted ? apiData.claims.formatted : [],
        appeals: selectedApis.appealsStatus && apiData.appealsStatus?.formatted ? apiData.appealsStatus.formatted : [],
        appealableIssues: selectedApis.appealableIssues && apiData.appealableIssues?.formatted ? apiData.appealableIssues.formatted : [],
        rawServiceHistory: selectedApis.serviceHistory ? apiData.serviceHistory?.raw : null,
        ...
        rawAppealableIssues: selectedApis.appealableIssues ? apiData.appealableIssues?.raw : null,
      };

      await saveVADataWithConsent(dataToSave, { saveToPacket: true, saveToVKB: true });
      loadSavedRecords();
  ...
  const handleClearSavedData = () => {
    if (window.confirm('Clear all saved VA records? This cannot be undone. Your data on VA.gov is not affected.')) {
      clearVARecords();
      loadSavedRecords();
      setSaveStatus({ loading: false, success: true, message: 'Saved VA records cleared.' });
      setTimeout(() => setSaveStatus(null), 3000);
    }
  };

  const handleDisconnect = () => {
    if (window.confirm('Disconnect from VA.gov? Your saved data will be preserved.')) {
      logout();
    }
  };
    ...
    return (
      <div className="bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-700 rounded-lg p-4 mb-6">
        <div className="flex gap-3">
          ...
          <div>
            <h3 className="font-semibold text-amber-800 dark:text-amber-200">VA Integration Not Configured</h3>
            ...
            <div className="mt-2 text-xs space-y-1 text-amber-600 dark:text-amber-400">
              <div className="flex items-center gap-2">
                {configStatus.oauthConfigured ? <Check className="w-3 h-3 text-green-500" /> : <XCircle className="w-3 h-3" />}
                <span>OAuth (VA.gov Sign-In)</span>
              </div>
              <div className="flex items-center gap-2">
                {configStatus.facilitiesConfigured ? <Check className="w-3 h-3 text-green-500" /> : <XCircle className="w-3 h-3" />}
                <span>Facilities API</span>
              </div>
              <div className="flex items-center gap-2">
                {configStatus.formsConfigured ? <Check className="w-3 h-3 text-green-500" /> : <XCircle className="w-3 h-3" />}
                <span>Forms API</span>
              </div>
              <div className="flex items-center gap-2">
                {configStatus.benefitsConfigured ? <Check className="w-3 h-3 text-green-500" /> : <XCircle className="w-3 h-3" />}
    ...
    <div className={`rounded-xl p-4 mb-6 ${
      ...
      {rateLimitStatus && (
        <div className="mt-3 flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
          ...
          API calls remaining: {rateLimitStatus.remaining}/{rateLimitStatus.total}
          {rateLimitStatus.resetsIn > 0 && ` (resets in ${rateLimitStatus.resetsIn}s)`}
        </div>
      )}
    </div>
  );
    ...
    <div className="flex border-b border-gray-200 dark:border-gray-700 mb-6">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          Saved Data
          {savedRecords && (savedRecords.claims?.length > 0 || savedRecords.serviceHistory) && (
  ...
  const renderOAuthApiCard = (api) => {
    ...
    return (
      <div key={api.id} className={`border rounded-xl overflow-hidden transition-all ${
        isSelected 
        ...
        <div className="p-4">
          <div className="flex items-start gap-3">
            ...
            <div className={`p-2 rounded-lg ${
              data ? 'bg-green-100 dark:bg-green-900/50' : 'bg-gray-100 dark:bg-gray-700'
            }`}>
              <Icon className={`w-5 h-5 ${
                data ? 'text-green-600 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'
              }`} />
            </div>
        ...
        {data && isExpanded && (
          <div className="border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 p-4">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Data Preview</span>
              <button
  ...
  const renderApiKeyCard = (api) => {
    ...
    const apiKeyConfigured = api.id === 'facilities' 
    ...
    return (
      <div key={api.id} className="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden bg-white dark:bg-gray-800">
        {/* Card Header */}
        <div className="p-4">
          <div className="flex items-start gap-3">
            <div className={`p-2 rounded-lg ${
              data ? 'bg-purple-100 dark:bg-purple-900/50' : 'bg-gray-100 dark:bg-gray-700'
            }`}>
              <Icon className={`w-5 h-5 ${
                data ? 'text-purple-600 dark:text-purple-400' : 'text-gray-500 dark:text-gray-400'
              }`} />
            </div>
          ...
          <div className="mt-3 ml-11 flex gap-2 flex-wrap">
            {api.requiresInput && (
              <input
                ...
                onChange={(e) => setSearchInputs(prev => ({ ...prev, [api.id]: e.target.value }))}
    ...
    switch (apiId) {
      case 'serviceHistory':
        return data.map((service, idx) => (
          <div key={idx} className="bg-white dark:bg-gray-800 p-3 rounded-lg border border-gray-200 dark:border-gray-700">
            <div className="font-medium text-gray-900 dark:text-white">{service.branchOfService}</div>
            <div className="text-sm text-gray-600 dark:text-gray-400">
              {service.startDate} - {service.endDate}
            </div>
            <div className="text-sm text-green-600 dark:text-green-400">{service.dischargeStatus}</div>
          </div>
        ));
        ...
        return data.slice(0, 5).map((claim, idx) => (
          <div key={idx} className="bg-white dark:bg-gray-800 p-3 rounded-lg border border-gray-200 dark:border-gray-700">
            <div className="flex justify-between items-start">
              <div className="font-medium text-gray-900 dark:text-white">{claim.claimType}</div>
            ...
            <div className="text-sm text-gray-600 dark:text-gray-400">Filed: {claim.claimDate}</div>
          </div>
        ));
      ...
      case 'appealsStatus':
        return data.slice(0, 5).map((appeal, idx) => (
          <div key={idx} className="bg-white dark:bg-gray-800 p-3 rounded-lg border border-gray-200 dark:border-gray-700">
            <div className="font-medium text-gray-900 dark:text-white">{appeal.type}</div>
            <div className="text-sm text-gray-600 dark:text-gray-400">Status: {appeal.status}</div>
            {appeal.docketNumber && (
              <div className="text-xs text-gray-500">Docket: {appeal.docketNumber}</div>
            )}
          </div>
        ));
      ...
      case 'appealableIssues':
        return data.slice(0, 5).map((issue, idx) => (
          <div key={idx} className="bg-white dark:bg-gray-800 p-3 rounded-lg border border-gray-200 dark:border-gray-700">
            <div className="font-medium text-gray-900 dark:text-white">{issue.description}</div>
            <div className="text-sm text-gray-600 dark:text-gray-400">Decision: {issue.decisionDate}</div>
          </div>
        ));
      ...
      case 'facilities':
        return data.slice(0, 5).map((facility, idx) => (
          <div key={idx} className="bg-white dark:bg-gray-800 p-3 rounded-lg border border-gray-200 dark:border-gray-700">
            ...
            <div className="text-sm text-gray-600 dark:text-gray-400">{facility.address}</div>
            {facility.phone && <div className="text-sm text-blue-600 dark:text-blue-400">{facility.phone}</div>}
          </div>
        ));
      ...
      case 'forms':
        return data.slice(0, 5).map((form, idx) => (
          <div key={idx} className="bg-white dark:bg-gray-800 p-3 rounded-lg border border-gray-200 dark:border-gray-700">
            <div className="font-semibold text-gray-900 dark:text-white">{form.formName}</div>
            <div className="text-sm text-gray-600 dark:text-gray-400">{form.title}</div>
            {form.url && (
              <a href={form.url} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-600 hover:underline">
                Download PDF ↗
              </a>
            )}
          </div>
      ...
      case 'disabilities':
        return data.slice(0, 10).map((disability, idx) => (
          <div key={idx} className="bg-white dark:bg-gray-800 p-2 rounded border border-gray-200 dark:border-gray-700 text-sm">
            <span className="font-medium">{disability.name}</span>
            {disability.diagnosticCode && (
              <span className="text-gray-500 ml-2">({disability.diagnosticCode})</span>
            )}
          </div>
        ));
  ...
  const renderSavedTab = () => (
    <div className="space-y-4">
      {!savedRecords || (!savedRecords.claims?.length && !savedRecords.serviceHistory && !savedRecords.appeals?.length) ? (
        <div className="text-center py-12 bg-gray-50 dark:bg-gray-800 rounded-xl">
          <Download className="w-12 h-12 mx-auto text-gray-400 mb-4" />
          <h3 className="text-lg font-medium text-gray-700 dark:text-gray-300">No Saved VA Data</h3>
          <p className="text-gray-500 dark:text-gray-400 mt-2 max-w-md mx-auto">
            Connect to VA.gov and import your records. Data is saved locally on your device, 
            available to AI tools, and persists after you disconnect.
          </p>
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4 text-center">
              ...
              <div className="text-sm text-blue-700 dark:text-blue-300">Service Record</div>
            </div>
            ...
            <div className="bg-purple-50 dark:bg-purple-900/20 rounded-xl p-4 text-center">
              ...
              <div className="text-sm text-purple-700 dark:text-purple-300">Appeals</div>
            </div>
            <div className="bg-amber-50 dark:bg-amber-900/20 rounded-xl p-4 text-center">
              <div className="text-2xl font-bold text-amber-600 dark:text-amber-400">
                {savedRecords.appealableIssues?.length || 0}
              </div>
              <div className="text-sm text-amber-700 dark:text-amber-300">Appealable Issues</div>
            </div>
          </div>
          ...
          <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="font-medium text-green-800 dark:text-green-200">Data Privacy</h4>
                <p className="text-sm text-green-700 dark:text-green-300 mt-1">
                  Your VA data is stored locally on this device only. It is never uploaded to any server. 
                  AI tools in this app can use this data to provide personalized assistance.
                </p>
              </div>
            </div>
          </div>
  ...
  // =========================================================================
  return (
    <div className={embeddedMode ? '' : 'fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4'}>
      <div className={`bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full ${
        embeddedMode ? '' : 'max-w-4xl max-h-[90vh] overflow-hidden flex flex-col'
      }`}>
        {/* Header */}
        {!embeddedMode && (
          <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-gray-900 dark:text-white">VA Data Center</h2>
                <p className="text-sm text-gray-500 dark:text-gray-400">
            ...
            <div className="space-y-4">
              {!isAuthenticated ? (
                <div className="text-center py-12 bg-gray-50 dark:bg-gray-800 rounded-xl">
                  ...
                  <h3 className="text-lg font-medium text-gray-700 dark:text-gray-300">Sign in to Access Your Records</h3>
                  <p className="text-gray-500 dark:text-gray-400 mt-2 max-w-md mx-auto">
                    Connect to VA.gov to securely access your service history, claims, and appeals information.
                  </p>
                </div>
              ) : (
                <>
                  ...
                  <div className="flex items-center justify-between flex-wrap gap-4 mb-4">
                    ...
                    <button
                      ...
                      disabled={saveStatus?.loading || !Object.values(selectedApis).some(Boolean)}
                      className="flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-colors disabled:opacity-50"
                    >
                      {saveStatus?.loading ? (
                        <Loader2 className="w-4 h-4 animate-spin" />
