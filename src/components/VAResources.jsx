...
const buildExposuresCategory = (t) => ({
  id: "exposures",
...
const buildMentalHealthCategory = (t) => ({
  id: "mentalHealth",
...
const buildSpecializedProgramsCategory = (t) => ({
...
const buildHealthCareCategory = (t) => ({
  id: "healthCare",
...
const buildRegulationsCategory = (t) => ({
  id: "regulations",
...
const buildResourceCategories = (t) => [
  buildPactActCategory(t),
  buildExposuresCategory(t),
  buildMentalHealthCategory(t),
  buildSpecializedProgramsCategory(t),
  buildHealthCareCategory(t),
  buildBenefitsCategory(t),
  buildRegulationsCategory(t),
];

const resourceCardClassName = (resource) => {
  const base = "block p-3 rounded-lg border transition-all hover:shadow-md";
  if (resource.urgent) {
    return `${base} bg-red-50 dark:bg-red-900/20 border-red-300 dark:border-red-700 hover:bg-red-100 dark:hover:bg-red-900/30`;
  }
  if (resource.important) {
    return `${base} bg-green-50 dark:bg-green-900/20 border-green-300 dark:border-green-700 hover:bg-green-100 dark:hover:bg-green-900/30`;
  }
  return `${base} bg-gray-50 dark:bg-gray-700/50 border-gray-200 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700`;
};
...
const ResourceLinkCard = ({ resource, onInternalClick }) => {
...
const VAResources = ({ onClose, onReportBug }) => {
  ...
  const resourceCategories = buildResourceCategories(t);
        ...
        <div className="space-y-4">
          ...
          {resourceCategories.map((category) => (
            <ResourceCategorySection
              ...
              onInternalClick={() => setShowRegulationsReference(true)}
              t={t}
            />
          ))}

          {/* Footer Info */}
          <VAResourcesFooterInfo t={t} />
        </div>
      </ResponsiveModal>
