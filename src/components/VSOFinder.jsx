...
const OrganizationCard = ({ org, t }) => {
  const gradientClass =
    ORG_TYPE_GRADIENTS[org.type] || "from-gray-500 to-slate-600";
  const bgClass =
    ORG_TYPE_BG_CLASSES[org.type] ||
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        <div
          className={`w-10 h-10 rounded-lg bg-gradient-to-br ${gradientClass} flex items-center justify-center text-white shadow-sm flex-shrink-0`}
        >
...
const VSOFinderHeader = ({ t, onClose, onReportBug, onToggleAISettings }) => (
...
const VSOFinderFooter = ({ t, hasResults, onClose }) => (
    ...
    <div className="flex items-start gap-3">
      ...
      <div>
        <h3 className="font-bold text-red-800 dark:text-red-200 text-lg">
          {t("vsoFinder", "bewareClaimSharks")}
        </h3>
        <div className="text-sm text-red-700 dark:text-red-300 mt-2 space-y-2">
          <p>
            <strong>🚫 {t("vsoFinder", "neverPayFees")}</strong>
          </p>
          <p>{t("vsoFinder", "legitimateVSOsFree")}</p>
          <p className="font-medium">{t("vsoFinder", "alwaysVerify")}</p>
        </div>
      </div>
    </div>
  </div>
...
const LocalOrganizationsSection = ({ t, results }) => (
  <div>
...
const NationalResourceCard = ({ resource, t }) => (
...
const NationalResourcesSection = ({ t, resources }) => (
  <div>
    <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
      <span className="text-xl">🇺🇸</span>
...
const VerificationSection = ({ t, verificationLink }) => (
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="font-medium text-green-800 dark:text-green-200">
            {t("vsoFinder", "verifyAccreditation")}
          </p>
          <p className="text-sm text-green-600 dark:text-green-300">
            {t("vsoFinder", "alwaysConfirmVSO")}
          </p>
        </div>
      </div>
      <a
...
const ResultsDisplay = ({ t, results }) => (
  <div className="space-y-6">
    ...
    {results.organizations && results.organizations.length > 0 && (
      <LocalOrganizationsSection t={t} results={results} />
    )}

    {/* National Resources */}
    {results.nationalResources && results.nationalResources.length > 0 && (
      <NationalResourcesSection t={t} resources={results.nationalResources} />
    )}

    {/* Verification Link */}
    <VerificationSection t={t} verificationLink={results.verificationLink} />

    {/* Disclaimer */}
    <div className="mt-4 p-4 bg-gray-100 dark:bg-gray-900 rounded-lg">
      <p className="text-xs text-gray-500 dark:text-gray-400">
        <strong>⚠️ {t("vsoFinder", "disclaimer")}</strong>{" "}
        {t("vsoFinder", "disclaimerText")}
      </p>
    </div>
  </div>
);

const VSOLoadingState = ({ t, zipCode }) => (
  <div className="text-center py-12">
    ...
    <div className="mt-4 space-y-2 text-xs text-gray-500 dark:text-gray-500 max-w-xs mx-auto">
      <p className="flex items-center justify-center gap-2">
        ...
        {t("vsoFinder", "checkingCountyVSOs")}
      </p>
      <p className="flex items-center justify-center gap-2">
        <span className="animate-pulse">🏢</span>{" "}
        {t("vsoFinder", "findingStateVA")}
      </p>
      <p className="flex items-center justify-center gap-2">
        <span className="animate-pulse">🎖️</span>{" "}
        {t("vsoFinder", "locatingNationalOrgs")}
      </p>
    </div>
...
const VSONoResultsState = ({ t }) => (
  <div className="text-center py-12 text-gray-500 dark:text-gray-400">
    <div className="text-6xl mb-4">🔍</div>
    <p className="text-lg">{t("vsoFinder", "enterZipToFind")}</p>
    <p className="text-sm mt-2">{t("vsoFinder", "willHelpFind")}</p>
  </div>
);

const VSOInfoSection = ({ t }) => (
  <div className="mt-6 bg-blue-50 dark:bg-blue-900/30 rounded-xl p-5 border border-blue-200 dark:border-blue-700">
    ...
    <div className="text-sm text-blue-700 dark:text-blue-300 space-y-2">
      ...
      <p>
        <strong>{t("vsoFinder", "commonVSOs")}</strong>{" "}
        {t("vsoFinder", "commonVSOsList")}
      </p>
      <p>
        <strong>{t("vsoFinder", "whyUseVSO")}</strong>{" "}
        {t("vsoFinder", "whyUseVSOText")}
      </p>
    </div>
  </div>
);
...
const VSOFinderBody = ({
  ...
  <div>
    <ClaimSharkWarning t={t} />

    <VSOSearchForm
      t={t}
      zipCode={zipCode}
      onZipCodeChange={onZipCodeChange}
      onOfficialSearch={onOfficialSearch}
    ...
    {error && <VSOErrorDisplay error={error} />}

    {results && <ResultsDisplay t={t} results={results} />}

    {isLoading && <VSOLoadingState t={t} zipCode={zipCode} />}

    {!results && !isLoading && !error && <VSONoResultsState t={t} />}

    <VSOInfoSection t={t} />
  </div>
);

const buildVSOAdvisorPrompt = (
  aiQuestion,
) => `You are a knowledgeable Veterans Service Officer (VSO) advisor helping veterans understand their representation options.
...
/**
 * ZIP-code search state (official VA link + AI-powered local search).
 * Extracted from VSOFinder to keep that component under the line limit.
 */
function useVSOZipSearch(t) {
...
/**
 * Free-text AI consultation state. Extracted from VSOFinder to keep that
 * component under the line limit.
 */
function useVSOAIConsultation(t, setError) {
...
const VSOFinder = ({ onClose, onReportBug }) => {
  ...
  const {
    ...
    isAIThinking,
    handleAIQuestion,
  } = useVSOAIConsultation(t, setError);
  ...
  return (
    ...
    >
      <VSOFinderBody
        ...
        onOfficialSearch={handleOfficialSearch}
        onAISearch={handleAISearch}
        onAskAI={handleAIQuestion}
      />
    </ResponsiveModal>
  );
};
