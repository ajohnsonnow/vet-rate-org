...
export function VaApiStatusBanner({ onDismiss }) {
  ...
  return (
    <div
      className={`${bgColor} ${textColor} px-4 py-3 border-b-2 shadow-lg relative`}
    >
          ...
          <div className="flex-1">
            ...
            <p className="text-xs sm:text-sm opacity-90">
              ...
              <span className="hidden sm:inline">
                {" "}
                - This is a VA system issue, not Vet-Rate.org.
              </span>
            </p>
          </div>
        </div>
...
export function VaApiStatusIndicator({ showLabel = true, size = "md" }) {
  ...
  return (
    ...
    >
      <div
        className={`w-2 h-2 ${dotColor} rounded-full ${hasIssues ? "animate-pulse" : ""}`}
      />
      {showLabel && (
        <span
          className={
...
export function VaFeatureStatusBadge({ feature, showDetails = false }) {
  ...
  return (
    <div className="relative inline-block">
      ...
      {showTooltip &&
        hasIssues &&
        (incidents.length > 0 || maintenance.length > 0) && (
        ...
        <div className="flex-1">
          ...
          >
            {isVaIssue
              ...
              : sanitizeErrorMessage(error) ||
                "An error occurred while fetching data from the VA."}
          </p>
