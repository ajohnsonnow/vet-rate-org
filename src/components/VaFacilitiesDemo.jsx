    ...
    <div className={containerClass}>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-yellow-700 dark:text-yellow-300 mt-1">
                To use the VA Facilities API, add your API key to the <code className="bg-yellow-100 dark:bg-yellow-800 px-1 rounded">.env</code> file:
              </p>
              ...
              >
                Get your free API key at developer.va.gov <ExternalLink className="w-3 h-3" />
              </a>
            </div>
          </div>
        </div>
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                            {Object.entries(facility.hours).map(([day, hours]) => (
                              <div key={day} className="text-gray-600 dark:text-gray-400">
                                <span className="font-medium capitalize">{day}:</span> {hours || 'Closed'}
                              </div>
                            ))}
                          </div>
                        </div>
      ...
      {!loading && !error && facilities.length === 0 && isConfigured && (
        <div className="text-center py-12 text-gray-500 dark:text-gray-400">
          ...
          <p className="text-lg">Enter a ZIP code to find nearby VA facilities</p>
          <p className="text-sm mt-2">
            Search for medical centers, benefits offices, cemeteries, and Vet Centers
          </p>
        </div>
      )}
      ...
      <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-700 rounded-lg">
        <div className="flex items-start gap-3">
          ...
          <div className="text-sm text-blue-800 dark:text-blue-200">
            ...
            <p className="mt-1">
              This component uses the VA Facilities API with API Key authentication (no OAuth required).
              The default ZIP code (97217) is set for demo purposes - Portland, OR area.
            </p>
            <p className="mt-2 text-xs text-blue-600 dark:text-blue-400">
              Endpoint: <code className="bg-blue-100 dark:bg-blue-800 px-1 rounded">GET /services/va_facilities/v1/facilities</code>
            </p>
          </div>
        </div>
      </div>
