...
/**
 * Builds the SpeechRecognition event handlers for a recognition session.
 * Extracted from useSpeechRecognition to keep that hook under the line limit.
 */
function buildRecognitionHandlers({ setIsListening, setTranscript, setError }) {
  const onstart = () => {
    setIsListening(true);
  };
  ...
  return { onstart, onresult, onerror, onend };
}
...
export const useSpeechRecognition = () => {
  ...
  const startListening = useCallback(() => {
    ...
    const handlers = buildRecognitionHandlers({
      setIsListening,
      setTranscript,
      setError,
    });
 ...
 */
const VOICE_BUTTON_SIZE_CLASSES = { sm: "p-1.5", md: "p-2", lg: "p-3" };
const VOICE_ICON_SIZES = { sm: "w-4 h-4", md: "w-5 h-5", lg: "w-6 h-6" };
...
const VoiceMicButton = ({
  ...
  <button
    ...
    disabled={disabled}
    className={`
      ${VOICE_BUTTON_SIZE_CLASSES[size]}
      rounded-full
      transition-all
      duration-200
      focus:outline-none
  ...
  >
    {isListening ? (
      ...
      <svg
        className={VOICE_ICON_SIZES[size]}
        viewBox="0 0 24 24"
        fill="currentColor"
      >
...
const VoiceSafetyPromptFooter = ({ onConfirm, onDecline }) => (
  <>
...
const VoiceSafetyPromptModal = ({ onConfirm, onDecline }) => (
  <ResponsiveModal
    isOpen
    onClose={onDecline}
    size="sm"
...
/**
 * Combines speech recognition with the tooltip/safety-prompt UI state for
 * VoiceInputButton. Extracted to keep that component under the line limit.
 */
function useVoiceInputController(onTranscript) {
...
const VoiceInputButton = ({
  ...
  const {
    ...
    handleSafetyDecline,
    handleClick,
  } = useVoiceInputController(onTranscript);
  ...
  return (
    <div className="relative inline-flex items-center group">
      ...
      {/* Error tooltip */}
      {error && <VoiceErrorTooltip error={error} />}
 ...
 */
const VoiceTextAreaLabel = ({ id, label, required }) => (
  <label
    htmlFor={id}
...
const VoiceTextAreaFooter = ({ helpText, maxLength, value }) => (
