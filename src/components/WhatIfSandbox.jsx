...
const runScenarioCalculation = (
  ...
  const { finalRating, pay } = computeScenarioResult(
    currentConditions,
    compensationRates,
  );
          ...
          <div className="space-y-1">
            {categoryConditions.map((condition) => (
              <button
                ...
                onClick={() => onAddCondition(condition)}
...
const ScenarioConditions = ({
  ...
  if (currentConditions.length === 0) {
    return (
      // Drop target only — dragging is an enhancement; the equivalent
      // action (tap a condition in the library) is fully keyboard operable
    ...
    <div className="space-y-2">
      {currentConditions.map((condition, index) => (
        <div key={condition.id}>
          {hoveredIndex === index && (
            // Drop target only — dragging is an enhancement; the
            // equivalent action (tap a condition in the library) is
          ...
          >
            <div className="min-w-0">
              <span className="text-lg font-bold">{condition.name}</span>
...
const ScenarioInfoBox = ({ ratesYear }) => (
...
function useScenarioSandbox() {
  ...
  useEffect(() => {
    ...
    loadCurrentClaims(setCurrentConditions);
