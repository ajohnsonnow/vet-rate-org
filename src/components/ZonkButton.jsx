...
const ZonkButton = ({ className = '' }) => {
  ...
  const handleZonkClick = () => {
    ...
    const randomZonk = ZONK_MESSAGES[Math.floor(Math.random() * ZONK_MESSAGES.length)];
    ...
    <div className={className}>
      ...
      {showZonk && zonkData && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 animate-fadeIn">
          <div className={`max-w-md w-full border-4 rounded-2xl p-8 shadow-2xl transform animate-bounce ${getColorClasses(zonkData.color)}`}>
