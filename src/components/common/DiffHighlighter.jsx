...
const DiffHighlighter = ({ text, issues = [], className = '' }) => {
  ...
  if (!issues || issues.length === 0) {
    return (
      <div className={`whitespace-pre-wrap text-gray-300 leading-relaxed ${className}`}>
        {text}
      </div>
    );
  }
  ...
  const sortedIssues = [...issues]
    ...
    .sort((a, b) => (b.quote_target?.length || 0) - (a.quote_target?.length || 0));
  ...
  return (
    <div className={`whitespace-pre-wrap text-gray-300 leading-relaxed font-mono text-sm ${className}`}>
      {nodes}
      
      {/* Legend */}
      <div className="mt-6 pt-4 border-t border-gray-700">
        <p className="text-xs text-gray-500 uppercase tracking-wider mb-2">Issue Legend</p>
        <div className="flex flex-wrap gap-3 text-xs">
          {Object.entries(ISSUE_COLORS).filter(([key]) => key !== 'default').map(([type, colors]) => (
            <span 
              key={type} 
...
export const IssueCard = ({ issue, index }) => {
  ...
  return (
    <div 
      className="bg-gray-900/80 border border-gray-700 rounded-lg p-4 hover:border-orange-500 transition-colors"
    >
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 space-y-2">
          ...
          <div className="flex justify-between items-start">
            ...
            {issue.severity && (
              <span className={`text-xs font-mono uppercase px-2 py-1 rounded ${
                issue.severity === 'High' ? 'bg-red-900/50 text-red-200' :
                issue.severity === 'Medium' ? 'bg-yellow-900/50 text-yellow-200' :
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm bg-black/20 p-3 rounded">
            {issue.quote_target && (
              <div>
                <span className="text-xs text-gray-500 uppercase">You wrote:</span>
                <p className="text-red-200 italic">"{issue.quote_target}"</p>
              </div>
            )}
            {issue.quote_reference && (
              <div>
                <span className="text-xs text-gray-500 uppercase">Evidence says:</span>
                <p className="text-blue-200 italic">"{issue.quote_reference}"</p>
              </div>
            )}
          </div>
          ...
          <div className="mt-2 space-y-1">
            {issue.explanation && (
              <p className="text-gray-300 text-sm">
                <span className="font-bold text-orange-400">Rater View:</span> {issue.explanation}
              </p>
            )}
            {issue.fix_suggestion && (
              <p className="text-green-400 text-sm">
                <span className="font-bold">💡 Suggestion:</span> {issue.fix_suggestion}
              </p>
            )}
          </div>
        </div>
