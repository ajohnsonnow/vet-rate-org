...
export const APP_TRANSLATIONS = {
  ...
  common: {
    ...
    dropIn: { en: 'Drop In', es: 'Soltar', tl: 'I-drop', vi: 'Thả vào', ko: '드롭인' },
  ...
  splash: {
    ...
    secondaryScoutDesc: { en: 'discover 65+ linked conditions to maximize your rating', es: 'descubre más de 65 condiciones vinculadas para maximizar tu calificación', tl: 'tuklasin ang 65+ na linked conditions para ma-maximize ang rating mo', vi: 'khám phá hơn 65 tình trạng liên kết để tối đa hóa xếp hạng của bạn', ko: '등급을 최대화하기 위해 65개 이상의 연결된 상태 발견' },
  ...
  tools: {
    ...
    secondaryScout: { en: 'Secondary Scout', es: 'Explorador Secundario', tl: 'Secondary Scout', vi: 'Trinh Sát Thứ Cấp', ko: '2차 스카우트' },
    secondaryScoutDesc: { en: 'Find 65+ linked conditions', es: 'Encuentra más de 65 condiciones vinculadas', tl: 'Hanapin ang 65+ na linked conditions', vi: 'Tìm hơn 65 tình trạng liên kết', ko: '65개 이상의 연결된 상태 찾기' },
  ...
  header: {
    ...
    userManual: { en: 'Field Manual', es: 'Manual de Campo', tl: 'Field Manual', vi: 'Sổ Tay Thực Địa', ko: '현장 매뉴얼' },
  ...
  privacy: {
    ...
    zeroDataCollection: {
      en: 'Zero personal data collection. Tiny bit of privacy-first analytics (GoatCounter). Your privacy is our honor.',
      es: 'Cero recolección de datos personales. Un poco de análisis respetuoso (GoatCounter). Tu privacidad es nuestro honor.',
      tl: 'Zero personal data collection. Kaunting privacy-first analytics (GoatCounter). Ang privacy mo ay karangalan namin.',
      vi: 'Không thu thập dữ liệu cá nhân. Phân tích tối thiểu (GoatCounter). Sự riêng tư của bạn là danh dự của chúng tôi.',
      ko: '개인 데이터 수집 없음. 최소한의 개인정보 보호 분석 (GoatCounter). 당신의 프라이버시는 우리의 명예입니다.',
    },
  },
...
**무엇이든 물어보세요!** VA 규정(38 CFR)을 사용하여 정확한 답변을 드립니다.`
  ...
  about: {
    ...
    noTrackingDesc: { en: 'Minimal analytics (GoatCounter - see stats). No selling data.', es: 'Análisis mínimo (GoatCounter - ver stats). Sin venta de datos.', tl: 'Minimal analytics (GoatCounter - tingnan stats). Walang pagbebenta ng data.', vi: 'Phân tích tối thiểu (GoatCounter - xem thống kê). Không bán dữ liệu.', ko: '최소 분석 (GoatCounter - 통계 보기). 데이터 판매 없음.' },
    ...
    commitment2: { en: 'Protecting your privacy - no ads, no personal data collection, no claim sharks (just tiny GoatCounter page views)', es: 'Proteger tu privacidad - sin anuncios, sin recopilación de datos personales, sin tiburones de reclamaciones (solo pequeñas vistas de página GoatCounter)', tl: 'Protektahan ang iyong privacy - walang ads, walang personal data collection, walang claim sharks (kaunting GoatCounter page views lang)', vi: 'Bảo vệ quyền riêng tư của bạn - không quảng cáo, không thu thập dữ liệu cá nhân, không cá mập yêu cầu (chỉ lượt xem trang GoatCounter nhỏ)', ko: '귀하의 개인 정보 보호 - 광고 없음, 개인 데이터 수집 없음, 청구 상어 없음 (작은 GoatCounter 페이지 조회만)' },
  ...
  pathfinder: {
    ...
    // File Drop In Modal (files stay in your browser's local storage - nothing leaves your device)
    dropInDocument: { en: 'Drop In Document', es: 'Soltar Documento', tl: 'I-drop ang Dokumento', vi: 'Thả Tài Liệu Vào', ko: '문서 드롭인' },
    dropInDocumentDesc: { en: 'Drop in VA decision letter, rating sheet, or notes (stays in your browser)', es: 'Suelta carta de decisión del VA, hoja de calificación o notas (se queda en tu navegador)', tl: 'I-drop ang VA decision letter, rating sheet, o mga notes (mananatili sa iyong browser)', vi: 'Thả vào thư quyết định VA, bảng xếp hạng hoặc ghi chú (lưu trong trình duyệt)', ko: 'VA 결정서, 등급표 또는 메모 드롭인 (브라우저에 저장됨)' },
  ...
  dd214Analyzer: {
    ...
    // Local File Drop Input (files stay in your browser - never sent to any server)
    dropPdfFiles: { en: 'Drop PDF files here (stays local)', es: 'Suelta archivos PDF aquí (se queda local)', tl: 'I-drop ang PDF files dito (lokal lang)', vi: 'Thả tệp PDF vào đây (lưu cục bộ)', ko: 'PDF 파일을 여기에 놓으세요 (로컬 저장)' },
  ...
  cfileAnalyzer: {
    ...
    // Errors
    pleaseDropPdf: { en: 'Please drop in a PDF file (stays in your browser).', es: 'Por favor suelta un archivo PDF (se queda en tu navegador).', tl: 'Mangyaring mag-drop ng PDF file (mananatili sa iyong browser).', vi: 'Vui lòng thả tệp PDF (lưu trong trình duyệt).', ko: 'PDF 파일을 놓으세요 (브라우저에 저장됨).' },
    pleaseDropInPdf: { en: 'Please drop in a PDF file.', es: 'Por favor suelta un archivo PDF.', tl: 'Mangyaring mag-drop ng PDF file.', vi: 'Vui lòng thả tệp PDF.', ko: 'PDF 파일을 놓으세요.' },
  ...
  denialDecoder: {
    ...
    // Local Image Selection (images stay in your browser - never sent to any server)
    takePhoto: { en: 'Take Photo', es: 'Tomar Foto', tl: 'Kumuha ng Litrato', vi: 'Chụp Ảnh', ko: '사진 찍기' },
    useYourCamera: { en: 'Use your camera (photo stays local)', es: 'Usa tu cámara (foto se queda local)', tl: 'Gamitin ang camera mo (lokal ang litrato)', vi: 'Sử dụng camera của bạn (ảnh lưu cục bộ)', ko: '카메라 사용 (사진 로컬 저장)' },
    selectImage: { en: 'Select Image', es: 'Seleccionar Imagen', tl: 'Pumili ng Larawan', vi: 'Chọn Hình Ảnh', ko: '이미지 선택' },
    selectFromFiles: { en: 'Select from files (stays in browser)', es: 'Seleccionar de archivos (se queda en navegador)', tl: 'Pumili mula sa files (mananatili sa browser)', vi: 'Chọn từ tệp (lưu trong trình duyệt)', ko: '파일에서 선택 (브라우저에 저장)' },
  ...
  bootCampTour: {
    ...
    helpFriend: { en: 'The Field Manual is your friend.', es: 'El Manual de Campo es tu amigo.', tl: 'Ang Field Manual ay kaibigan mo.', vi: 'Sổ Tay Thực Địa là bạn của bạn.', ko: '현장 매뉴얼이 도움이 됩니다.' },
  ...
  // Field Manual Component
  userManual: {
    // Header & Navigation
    title: { en: '📖 Field Manual', es: '📖 Manual de Campo', tl: '📖 Field Manual', vi: '📖 Sổ Tay Thực Địa', ko: '📖 현장 매뉴얼' },
  ...
  privacyPolicy: {
    ...
    section7Title: { en: '7. AI Processing & Privacy (BYOK Model)', es: '7. Procesamiento de IA y Privacidad (Modelo BYOK)', tl: '7. AI Processing at Privacy (BYOK Model)', vi: '7. Xử Lý AI & Quyền Riêng Tư (Mô Hình BYOK)', ko: '7. AI 처리 및 개인정보 보호 (BYOK 모델)' },
    section7OptionalAI: { en: 'Bring Your Own Key (BYOK) Model:', es: 'Modelo Traiga Su Propia Clave (BYOK):', tl: 'Bring Your Own Key (BYOK) Model:', vi: 'Mô Hình Mang Khóa Của Bạn (BYOK):', ko: '자체 키 가져오기(BYOK) 모델:' },
    section7OptionalAIText: { en: 'Vet-Rate.org does NOT provide AI services directly. To use AI-powered features (such as statement enhancement, document analysis, or recommendations), you must supply your own Google Gemini API key. No other AI providers are enabled by default. This BYOK approach ensures that: (1) You maintain direct control over your data, (2) Your API key and usage are solely between you and Google, (3) Vet-Rate.org never sees, stores, or transmits your AI interactions.', es: 'Vet-Rate.org NO proporciona servicios de IA directamente. Para usar funciones impulsadas por IA (como mejora de declaraciones, análisis de documentos o recomendaciones), debes proporcionar tu propia clave API de Google Gemini. Ningún otro proveedor de IA está habilitado de forma predeterminada. Este enfoque BYOK garantiza que: (1) Mantienes control directo sobre tus datos, (2) Tu clave API y uso son únicamente entre tú y Google, (3) Vet-Rate.org nunca ve, almacena ni transmite tus interacciones de IA.', tl: 'Ang Vet-Rate.org ay HINDI nag-provide ng AI services directly. Para gamitin ang AI-powered features (tulad ng statement enhancement, document analysis, o recommendations), dapat mong i-supply ang sarili mong Google Gemini API key. Walang ibang AI providers na enabled by default. Ang BYOK approach na ito ay nagsisiguro na: (1) Ikaw ay may direktang kontrol sa data mo, (2) Ang API key at paggamit mo ay exclusively sa pagitan mo at ng Google, (3) Ang Vet-Rate.org ay hindi nakakakita, nag-store, o nag-transmit ng AI interactions mo.', vi: 'Vet-Rate.org KHÔNG cung cấp dịch vụ AI trực tiếp. Để sử dụng các tính năng được hỗ trợ bởi AI (chẳng hạn như cải thiện tuyên bố, phân tích tài liệu hoặc đề xuất), bạn phải cung cấp khóa API Google Gemini của riêng mình. Không có nhà cung cấp AI nào khác được kích hoạt theo mặc định. Cách tiếp cận BYOK này đảm bảo rằng: (1) Bạn duy trì quyền kiểm soát trực tiếp đối với dữ liệu của mình, (2) Khóa API và việc sử dụng của bạn chỉ giữa bạn và Google, (3) Vet-Rate.org không bao giờ nhìn thấy, lưu trữ hoặc truyền các tương tác AI của bạn.', ko: 'Vet-Rate.org는 AI 서비스를 직접 제공하지 않습니다. AI 기반 기능(예: 진술 개선, 문서 분석 또는 권장 사항)을 사용하려면 자체 Google Gemini API 키를 제공해야 합니다. 다른 AI 제공업체는 기본적으로 활성화되어 있지 않습니다. 이 BYOK 접근 방식은 다음을 보장합니다: (1) 귀하가 데이터에 대한 직접적인 통제권을 유지합니다, (2) API 키와 사용은 귀하와 Google 간에만 이루어집니다, (3) Vet-Rate.org는 귀하의 AI 상호작용을 보거나 저장하거나 전송하지 않습니다.' },
    section7WhatThisMeans: { en: 'Your Privacy Options:', es: 'Sus Opciones de Privacidad:', tl: 'Iyong Privacy Options:', vi: 'Các Tùy Chọn Quyền Riêng Tư Của Bạn:', ko: '귀하의 개인정보 보호 옵션:' },
    section7AIItem1: { en: 'Local AI Models: Run AI entirely on your device using WebGPU/WebLLM for maximum privacy (no internet connection required for processing)', es: 'Modelos de IA Locales: Ejecuta IA completamente en tu dispositivo usando WebGPU/WebLLM para máxima privacidad (no se requiere conexión a internet para procesar)', tl: 'Local AI Models: Patakbuhin ang AI entirely sa device mo gamit ang WebGPU/WebLLM para sa maximum privacy (walang internet connection na kailangan para sa processing)', vi: 'Mô Hình AI Cục Bộ: Chạy AI hoàn toàn trên thiết bị của bạn bằng WebGPU/WebLLM để đảm bảo quyền riêng tư tối đa (không cần kết nối internet để xử lý)', ko: '로컬 AI 모델: WebGPU/WebLLM을 사용하여 기기에서 AI를 완전히 실행하여 최대 개인정보 보호 (처리에 인터넷 연결 필요 없음)' },
    section7AIItem2: { en: 'Google Gemini (BYOK Required): If you choose to use cloud AI, you must provide your own API key. Your data goes directly from your browser to Google - Vet-Rate.org is not involved in the transmission', es: 'Google Gemini (BYOK Requerido): Si eliges usar IA en la nube, debes proporcionar tu propia clave API. Tus datos van directamente de tu navegador a Google - Vet-Rate.org no está involucrado en la transmisión', tl: 'Google Gemini (BYOK Required): Kung pipiliin mong gumamit ng cloud AI, dapat kang magbigay ng sarili mong API key. Ang data mo ay direktang pumupunta mula sa browser mo papunta sa Google - Ang Vet-Rate.org ay hindi involved sa transmission', vi: 'Google Gemini (Yêu Cầu BYOK): Nếu bạn chọn sử dụng AI đám mây, bạn phải cung cấp khóa API của riêng mình. Dữ liệu của bạn đi trực tiếp từ trình duyệt của bạn đến Google - Vet-Rate.org không tham gia vào việc truyền tải', ko: 'Google Gemini (BYOK 필수): 클라우드 AI를 사용하려면 자체 API 키를 제공해야 합니다. 귀하의 데이터는 브라우저에서 Google로 직접 전송됩니다 - Vet-Rate.org는 전송에 관여하지 않습니다' },
    section7AIItem3: { en: 'No Third-Party AI by Default: Other AI providers (Anthropic, OpenAI, etc.) are NOT enabled unless you explicitly choose to enable them and provide your own API keys', es: 'Sin IA de Terceros por Defecto: Otros proveedores de IA (Anthropic, OpenAI, etc.) NO están habilitados a menos que elijas explícitamente habilitarlos y proporcionar tus propias claves API', tl: 'Walang Third-Party AI by Default: Ang ibang AI providers (Anthropic, OpenAI, etc.) ay HINDI enabled maliban kung explicitly mong piliin na i-enable sila at magbigay ng sarili mong API keys', vi: 'Không Có AI Bên Thứ Ba Theo Mặc Định: Các nhà cung cấp AI khác (Anthropic, OpenAI, v.v.) KHÔNG được kích hoạt trừ khi bạn chọn rõ ràng để kích hoạt chúng và cung cấp khóa API của riêng bạn', ko: '기본적으로 제3자 AI 없음: 다른 AI 제공업체(Anthropic, OpenAI 등)는 명시적으로 활성화하고 자체 API 키를 제공하지 않는 한 활성화되지 않습니다' },
    section7AIItem4: { en: 'You Have Complete Control: You decide which AI model to use (local or cloud), what data to process, and can disable all AI features at any time', es: 'Tienes Control Completo: Tú decides qué modelo de IA usar (local o en la nube), qué datos procesar y puedes desactivar todas las funciones de IA en cualquier momento', tl: 'Mayroon Kang Complete Control: Ikaw ang nagpapasya kung aling AI model ang gagamitin (local o cloud), kung anong data ang i-process, at maaari mong i-disable ang lahat ng AI features anumang oras', vi: 'Bạn Có Toàn Quyền Kiểm Soát: Bạn quyết định mô hình AI nào sẽ sử dụng (cục bộ hoặc đám mây), dữ liệu nào cần xử lý và có thể tắt tất cả các tính năng AI bất kỳ lúc nào', ko: '완전한 통제권: 사용할 AI 모델(로컬 또는 클라우드), 처리할 데이터를 결정하고 언제든지 모든 AI 기능을 비활성화할 수 있습니다' },
    section7TransparencyNote: { en: "Maximum Privacy Design: For the highest level of privacy, use Local AI models that run entirely on your device. If you choose to use cloud AI with your own Google Gemini API key, please review Google's Gemini API Terms and Privacy Policy. When using your own API keys, data handling is governed by your direct agreement with Google, not by Vet-Rate.org. We recommend using local models whenever possible for maximum privacy and data security.", es: 'Diseño de Máxima Privacidad: Para el más alto nivel de privacidad, usa modelos de IA Locales que se ejecutan completamente en tu dispositivo. Si eliges usar IA en la nube con tu propia clave API de Google Gemini, revisa los Términos de la API de Gemini de Google y la Política de Privacidad. Al usar tus propias claves API, el manejo de datos se rige por tu acuerdo directo con Google, no por Vet-Rate.org. Recomendamos usar modelos locales siempre que sea posible para máxima privacidad y seguridad de datos.', tl: 'Maximum Privacy Design: Para sa pinakamataas na antas ng privacy, gumamit ng Local AI models na tumatakbo entirely sa device mo. Kung pipiliin mong gumamit ng cloud AI gamit ang sarili mong Google Gemini API key, mangyaring suriin ang Google\'s Gemini API Terms at Privacy Policy. Kapag gumagamit ng sarili mong API keys, ang data handling ay pinamamahalaan ng direktang agreement mo sa Google, hindi ng Vet-Rate.org. Inirerekomenda namin na gumamit ng local models kung possible para sa maximum privacy at data security.', vi: 'Thiết Kế Quyền Riêng Tư Tối Đa: Để có mức độ bảo mật cao nhất, hãy sử dụng các mô hình AI cục bộ chạy hoàn toàn trên thiết bị của bạn. Nếu bạn chọn sử dụng AI đám mây với khóa API Google Gemini của riêng bạn, vui lòng xem lại Điều khoản API Gemini và Chính sách Bảo mật của Google. Khi sử dụng khóa API của riêng bạn, việc xử lý dữ liệu được điều chỉnh bởi thỏa thuận trực tiếp của bạn với Google, không phải bởi Vet-Rate.org. Chúng tôi khuyên bạn nên sử dụng các mô hình cục bộ bất cứ khi nào có thể để đảm bảo quyền riêng tư và bảo mật dữ liệu tối đa.', ko: '최대 개인정보 보호 설계: 최고 수준의 개인정보 보호를 위해 기기에서 완전히 실행되는 로컬 AI 모델을 사용하십시오. 자체 Google Gemini API 키로 클라우드 AI를 사용하려면 Google의 Gemini API 약관 및 개인정보 처리방침을 검토하십시오. 자체 API 키를 사용할 때 데이터 처리는 Vet-Rate.org가 아닌 Google과의 직접 계약에 의해 관리됩니다. 최대 개인정보 보호 및 데이터 보안을 위해 가능한 한 로컬 모델을 사용하는 것이 좋습니다.' },
