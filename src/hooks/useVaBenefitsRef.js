...
async function fetchBenefitsRef(endpoint, apiKey) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Benefits Ref API error: ${response.status} - ${errorBody}`);
  }
...
export const CLAIM_PHASES = {
  'CLAIM_RECEIVED': {
    ...
    longDesc: 'Your claim has been received and is waiting to be assigned to a claims processor. This is the starting point for all claims.',
    avgDays: '1-7 days',
  ...
  'INITIAL_REVIEW': {
    phase: 2,
    name: 'Initial Review',
    shortDesc: 'A Veterans Service Representative (VSR) is reviewing your claim',
    longDesc: 'A claims processor is reviewing your submitted evidence and determining what additional information may be needed.',
    avgDays: '7-30 days',
  ...
  'EVIDENCE_GATHERING': {
    ...
    longDesc: 'The VA is collecting medical records, scheduling C&P exams, and reviewing all evidence. This is typically the longest phase.',
    avgDays: '30-90+ days',
    whatNext: 'Attend any scheduled C&P exams. Submit any additional evidence promptly.',
    tips: [
  ...
  'PREPARATION_FOR_DECISION': {
    ...
    shortDesc: 'All evidence has been collected and a decision is being prepared',
    longDesc: 'A Rating Veterans Service Representative (RVSR) is reviewing all evidence and preparing the rating decision.',
    avgDays: '7-30 days',
    whatNext: 'Wait for the decision. You can still submit evidence, but it may delay the process.',
    tips: [
  ...
  'PENDING_DECISION_APPROVAL': {
    ...
    longDesc: 'The decision has been made and is awaiting final quality review and approval before being sent to you.',
    avgDays: '3-14 days',
  ...
  'PREPARATION_FOR_NOTIFICATION': {
    ...
    longDesc: 'The decision has been approved and your notification letter is being generated and prepared for mailing.',
    avgDays: '1-7 days',
  ...
  'COMPLETE': {
    ...
    longDesc: 'A decision has been made on your claim. Review your decision letter carefully for the effective date, rating, and appeal rights.',
    avgDays: 'N/A',
    whatNext: 'Review your decision letter. If you disagree, you have appeal options.',
    tips: [
  ...
  'ERRORED': {
    ...
    longDesc: 'Something unexpected happened with your claim. This is rare and usually requires VA staff to intervene.',
    avgDays: 'Varies',
...
export function useVaBenefitsRef() {
  ...
  const isConfigured = Boolean(apiKey && apiKey !== 'your_va_benefits_ref_api_key_here');
  ...
  const fetchDisabilities = useCallback(async () => {
    if (!isConfigured) {
      setError('VA Benefits Reference API is not configured. Add VITE_VA_BENEFITS_REF_API_KEY to .env');
      return { success: false, error: 'API not configured' };
    }

    // Check cache
    const now = Date.now();
    if (cache.disabilities && (now - cache.disabilities_timestamp) < CACHE_DURATION) {
    ...
    try {
      const data = await fetchBenefitsRef('/services/benefits-reference-data/v1/disabilities', apiKey);
  ...
  const fetchContentionTypes = useCallback(async () => {
    ...
    const now = Date.now();
    if (cache.contention_types && (now - cache.contention_types_timestamp) < CACHE_DURATION) {
    ...
    try {
      const data = await fetchBenefitsRef('/services/benefits-reference-data/v1/contention-types', apiKey);
  ...
  const fetchServiceBranches = useCallback(async () => {
    ...
    const now = Date.now();
    if (cache.service_branches && (now - cache.service_branches_timestamp) < CACHE_DURATION) {
    ...
    try {
      const data = await fetchBenefitsRef('/services/benefits-reference-data/v1/service-branches', apiKey);
   ...
   */
  const searchDisabilities = useCallback((query) => {
    if (!query || query.length < 2) return [];
    
    const lowerQuery = query.toLowerCase();
    return disabilities.filter(d => 
