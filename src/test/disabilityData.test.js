...
describe('disabilityData.json structure', () => {
  ...
  it('has disabilities array', () => {
    const data = Array.isArray(disabilityData) ? disabilityData : disabilityData.disabilities;
  ...
  it('each entry has a name or title', () => {
    ...
    sample.forEach(entry => {
      const hasName = entry.name || entry.title || entry.condition;
...
describe('disabilityData content quality', () => {
  ...
  it('includes common conditions like PTSD', () => {
    ...
    const hasPTSD = names.some(n => n.includes('ptsd') || n.includes('post-traumatic') || n.includes('posttraumatic'));
