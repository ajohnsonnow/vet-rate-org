 ...
 */
function _migraineNotProstrating(result, q1) {
  if (q1 === "yes_always" || q1 === "yes_sometimes") return null;

  result.predictedRating = 0;
  result.ratingRationale =
    'Your answers indicate attacks that do not require stopping activities and lying down. The VA requires attacks to be "prostrating" (completely incapacitating) for a compensable rating.';
  result.gaps.push(
    'Your migraines must be "prostrating" - meaning you must stop all activity and lie down, usually in a dark, quiet room.',
  );
...
function _migraine50Percent(result, q1, q2, q3, q4) {
...
function _migraine30Percent(result, q2, q3, q4) {
  // Requires: prostrating + averaging once a month
  if (!(q2 === "monthly" || q2 === "very_frequent")) return null;

  result.predictedRating = 30;
  result.ratingRationale =
    "Your answers align with the 30% rating criteria: characteristic prostrating attacks occurring on average once a month or more over the last several months.";
  ...
  if (q3 !== "yes_severe") {
    result.gaps.push(
      '• The 50% rating requires evidence of "severe economic inadaptability" - meaning the attacks prevent you from maintaining employment or cause significant income loss.',
    );
    result.actionItems.push(
  ...
  if (q4 !== "days" && q4 !== "many_hours") {
    result.gaps.push(
      '• The 50% rating requires attacks to be "prolonged" (lasting many hours or days).',
    );
    result.actionItems.push(
      "Track how long your prostrating attacks last - if they last 4+ hours, document this.",
    );
  }

  if (q2 !== "very_frequent") {
    result.gaps.push(
      '• The 50% rating requires "very frequent" attacks (typically interpreted as 2+ per month).',
    );
    result.actionItems.push(
      "Keep a detailed headache diary showing frequency.",
    );
  }
...
function _migraine10Percent(result, q2) {
  // Requires: prostrating + averaging 1 in 2 months
  if (q2 !== "bi_monthly") return null;
...
function _migraine0PercentFallback(result) {
  result.predictedRating = 0;
  result.ratingRationale =
    "Your answers indicate prostrating attacks less frequently than once every 2 months. The minimum compensable rating (10%) requires attacks averaging one in 2 months.";
  result.gaps.push(
    "To qualify for a 10% rating, you need prostrating attacks at least once every 2 months on average.",
  );
  result.actionItems.push(
...
export function calculateMigraineRating(answers) {
  ...
  return (
    _migraineNotProstrating(result, q1) ||
    _migraine50Percent(result, q1, q2, q3, q4) ||
    _migraine30Percent(result, q2, q3, q4) ||
    _migraine10Percent(result, q2) ||
    _migraine0PercentFallback(result)
  );
}
 ...
 */
function _ptsdSeverityScore(q1, q2, q3, q4) {
  const q1Weight =
    { near_continuous: 3, weekly: 2, occasional: 1, rarely: 0 }[q1] || 0;
  const q2Weight =
    { severe: 3, reduced_reliability: 2, intermittent: 1, occasional: 0.5 }[
      q2
    ] || 0;
  const q3Weight = { total: 3, severe: 2, moderate: 1, mild: 0.5 }[q3] || 0;
  const q4Weight =
    { yes_frequent: 3, yes_occasional: 2, past_not_current: 1, no: 0 }[q4] || 0;

  return q1Weight + q2Weight + q3Weight + q4Weight;
}

function _addSuicidalIdeationWarning(result, q4) {
  if (q4 !== "yes_frequent" && q4 !== "yes_occasional") return;

  result.warnings.push(
    "⚠️ CRITICAL: You indicated suicidal thoughts. This is a key symptom for high ratings (70-100%). You MUST mention this to the examiner.",
  );
  result.actionItems.push(
...
function _applyPtsd70Or100(result, q2, q3) {
  ...
  if (q2 !== "severe") {
    result.gaps.push(
      "• You would need to demonstrate complete inability to maintain employment.",
    );
  }
  if (q3 !== "total") {
    result.gaps.push(
      "• You would need to demonstrate complete social isolation and inability to maintain any relationships.",
    );
  }
}

function _applyPtsd50(result) {
  result.predictedRating = 50;
  result.ratingRationale =
...
function _applyPtsd0(result) {
  result.predictedRating = 0;
  result.ratingRationale =
    "Your answers indicate symptoms that do not interfere with work or social functioning. This aligns with a 0% (non-compensable) rating.";
  result.gaps.push(
    "To qualify for a compensable rating (10% or higher), symptoms must cause occupational or social impairment.",
  );
  result.actionItems.push(
    "If your symptoms worsen, document this and file for an increase.",
  );
}

function _applyPtsdRatingTier(result, totalScore, q2, q3, q4) {
  if (
    totalScore >= 9 ||
    q2 === "severe" ||
    q3 === "total" ||
    ...
    _applyPtsd70Or100(result, q2, q3);
  } else if (
    totalScore >= 6 ||
    q2 === "reduced_reliability" ||
    q3 === "severe"
  ) {
    _applyPtsd50(result);
...
export function calculatePTSDRating(answers) {
  ...
  const totalScore = _ptsdSeverityScore(q1, q2, q3, q4);

  _addSuicidalIdeationWarning(result, q4);
  _applyPtsdRatingTier(result, totalScore, q2, q3, q4);
 ...
 */
function _backPainRomRating(result, q1) {
  if (q1 === "30_or_less") {
    result.ratingRationale =
      "Forward flexion limited to 30 degrees or less qualifies for a 40% rating under the General Rating Formula for Diseases of the Spine (38 CFR § 4.71a).";
    return 40;
  }
  ...
  if (q1 === "30_to_60") {
    ...
    result.warnings.push(
      "⚠️ CRITICAL EXAM TIP: Many veterans bend too far during ROM testing because they want to show effort. STOP at the point of pain. The examiner measures to the point where pain stops you, not where you can force yourself to go.",
    );
    return 20;
  }
...
function _backPainIvdsCheck(result, q2) {
  if (q2 === "yes_frequent_episodes") {
    result.warnings.push(
      "⚠️ IMPORTANT: You indicated frequent incapacitating episodes. This may qualify for an additional rating under DC 5243 (Intervertebral Disc Syndrome) of up to 60% if episodes total 6+ weeks per year.",
    );
    result.actionItems.push(
    ...
    result.warnings.push(
      "⚠️ IMPORTANT: You indicated some incapacitating episodes. This may qualify for an additional rating under DC 5243 (Intervertebral Disc Syndrome) of 20-40% depending on frequency.",
    );
    result.actionItems.push(
      "Document incapacitating episodes - when, how long, what triggered them.",
    );
  }
}

function _backPainRadiculopathyCheck(result, q3) {
  if (q3 === "no") return;

  result.warnings.push(
    "⚠️ IMPORTANT: You indicated radiculopathy (nerve symptoms). This can be rated SEPARATELY under DC 5243 (Intervertebral Disc Syndrome) or DC 8520 (Paralysis of the Sciatic Nerve).",
  );
  result.actionItems.push(
  ...
  if (q3 === "yes_severe") {
    result.warnings.push(
      "• Severe radiculopathy with weakness can be rated at 40-60% or higher depending on severity.",
    );
  }
}

function _backPainGaitCheck(result, q4) {
  if (q4 !== "wheelchair" && q4 !== "cane_walker") return;
...
export function calculateBackPainRating(answers) {
  ...
  result.predictedRating = _backPainRomRating(result, q1);

  _backPainIvdsCheck(result, q2);
  _backPainRadiculopathyCheck(result, q3);
  _backPainGaitCheck(result, q4);
...
export function calculateTinnitusRating(answers) {
  ...
  if (
    ...
    result.predictedRating = 10;
    result.ratingRationale =
      "Recurrent tinnitus is rated at a flat 10% under 38 CFR § 4.87a, Diagnostic Code 6260. There is no higher or lower rating for tinnitus alone.";
    ...
    result.predictedRating = 0;
    result.ratingRationale =
      'Occasional tinnitus (less than weekly) does not meet the "recurrent" criteria for a compensable rating.';

    result.gaps.push(
      'To qualify for a 10% rating, tinnitus must be "recurrent" - occurring regularly (typically weekly or more).',
    );
    result.actionItems.push(
      "If your tinnitus frequency increases, file for service connection or an increase.",
    );
  }
 ...
 */
function _kneeFlexionRating(result, q1) {
  if (q1 === "45_or_less") {
    result.ratingRationale =
      "Knee flexion limited to 45 degrees or less qualifies for a 30% rating under DC 5260 (Limitation of Flexion of the Knee).";
    return 30;
  }
...
function _kneeInstabilityCheck(result, q2) {
  if (q2 === "severe_recurrent") {
    result.warnings.push(
      "⚠️ IMPORTANT: Severe recurrent instability requiring a brace can qualify for a SEPARATE rating of 10-20% under DC 5257 (Recurrent Subluxation or Lateral Instability).",
    );
    result.actionItems.push(
    ...
    result.warnings.push(
      "⚠️ IMPORTANT: Knee instability can qualify for a SEPARATE rating under DC 5257. You may be able to get both a flexion limitation rating AND an instability rating.",
    );
    result.actionItems.push(
      "Document instability episodes - when they occur, what causes them.",
    );
  }
}

function _kneeSwellingCheck(result, q3) {
  if (q3 === "persistent") {
    result.warnings.push(
    ...
    result.warnings.push(
      "ℹ️ Recurrent swelling should be documented. Mention when it occurs and what causes it.",
    );
  }
}
...
export function calculateKneePainRating(answers) {
  ...
  result.predictedRating = _kneeFlexionRating(result, q1);

  _kneeInstabilityCheck(result, q2);
  _kneeSwellingCheck(result, q3);
