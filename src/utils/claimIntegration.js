...
export const getBigThreeStatus = (condition) => {
  if (!condition)
    return { diagnosis: false, event: false, nexus: false, complete: false };

  const slug = condition.toLowerCase().replace(/[^a-z0-9]/g, "_");

  const diagnosis =
    localStorage.getItem(BIG_THREE_KEYS.DIAGNOSIS(slug)) === "true";
...
export const setBigThreeStatus = (condition, type, value) => {
  ...
  if (key) {
    ...
    window.dispatchEvent(
      new CustomEvent("bigThreeUpdate", {
...
export const bigThreeToNavigatorEvidence = (condition) => {
  ...
  return {
    medical_diagnosis: status.diagnosis
      ? { status: "obtained" }
      : { status: "needed" },
    in_service_event: status.event
      ? { status: "obtained" }
      : { status: "needed" },
...
export const markMedicalRecordsReviewed = () => {
  localStorage.setItem(
    MILESTONE_KEYS.MEDICAL_RECORDS,
    JSON.stringify({
      reviewed: true,
...
export const markServiceConnectionIdentified = () => {
  localStorage.setItem(
    MILESTONE_KEYS.SERVICE_CONNECTION,
    JSON.stringify({
      completed: true,
      timestamp: new Date().toISOString(),
      source: "claim_navigator",
...
export const recordPhaseAdvanced = (claimId, fromPhase, toPhase) => {
  const history = JSON.parse(
    localStorage.getItem(MILESTONE_KEYS.PHASE_ADVANCED) || "[]",
  );
  history.push({
    claimId,
    fromPhase,
...
export const getOverallMilestoneProgress = () => {
  const milestoneChecks = [
    ...
    { id: "medical_records", key: MILESTONE_KEYS.MEDICAL_RECORDS, weight: 10 },
    {
      ...
      key: MILESTONE_KEYS.SERVICE_CONNECTION,
      weight: 12,
    },
...
export const dispatchNavigatorUpdate = (eventType, data) => {
  window.dispatchEvent(
    new CustomEvent("claimNavigatorUpdate", {
