...
export const getClaimStats = () => {
  ...
  return {
    ...
    statementGenerated: claims.filter(c => c.status === 'Statement Generated').length,
...
export const importClaims = (claims, mergeMode = 'replace') => {
  try {
    if (mergeMode === 'replace') {
      ...
      const existingIds = new Set(existingClaims.map(c => c.conditionName + '|' + (c.parentCondition || '')));
...
export const importStatements = (statements, mergeMode = 'replace') => {
  try {
    if (mergeMode === 'replace') {
      ...
      const existingStatements = JSON.parse(localStorage.getItem(STATEMENTS_KEY) || '{}');
