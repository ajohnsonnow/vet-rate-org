...
export const isFullDKBCached = async () => {
  ...
  return metadata?.fullDatabaseLoaded === true && metadata?.entryCount >= FULL_DATABASE_COUNT * 0.9;
};
...
export const downloadFullDKB = async (onProgress = () => {}) => {
  try {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn('[DKB] Full database not available, using web-optimized version');
    ...
    while (true) {
      ...
      if (totalSize > 0) {
        const downloadProgress = 20 + Math.round((receivedLength / totalSize) * 50);
    ...
    console.log(`[DKB] Parsed ${entries.length} entries (${isFullDB ? 'FULL' : 'web-optimized'})`);
    ...
    window.dispatchEvent(new CustomEvent('dkb-cache-updated', {
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        const entries = (request.result || []).filter(e => e.id !== METADATA_KEY);
        resolve(entries);
      };
