...
function assessPotentialImpact(evidence, decisionData) {
  ...
  const deniedConditions = decisionData.conditions.filter(
    (c) => c.status === "DENIED",
  );
...
function generateAdversarialReport(gapReport, decisionData) {
  return {
    ...
    vkbIntegration: {
      ...
      needsRedTeamReview: gapReport.potentialViolations.some(
        (v) => v.severity === "HIGH",
      ),
    },
  };
...
export function quickGapCheck(decisionText, cFileSummary) {
  ...
  if (
    scan.hasDBQs &&
    !decision.evidenceConsidered?.some((e) => e.match(/exam|dbq|c&p/i))
  ) {
  ...
  if (scan.hasBVA) {
    quickReport.flagsRaised.push(
      "Prior BVA decision exists - check for compliance",
    );
  }
...
export function compareDecisions(beforeText, afterText) {
  ...
  return {
    ...
    newEvidence:
      after.evidenceConsidered?.filter(
        (e) => !before.evidenceConsidered?.includes(e),
      ) || [],
  };
}
