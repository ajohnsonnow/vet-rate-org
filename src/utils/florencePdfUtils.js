 ...
 */
export async function convertPdfPageToBlob(pdfInput, pageNum = 1, scale = PDF_CONFIG.SCALE_HIGH) {
  ...
  if (viewport.width > PDF_CONFIG.MAX_DIMENSION || viewport.height > PDF_CONFIG.MAX_DIMENSION) {
    const reductionFactor = PDF_CONFIG.MAX_DIMENSION / Math.max(viewport.width, viewport.height);
    const adjustedScale = scale * reductionFactor;
    viewport = page.getViewport({ scale: adjustedScale });
    console.warn(`[PDF] Reduced scale to ${adjustedScale.toFixed(2)} to fit canvas limits`);
  }
