...
export const migrateUserData = () => {
  ...
  try {
    ...
    if (!userSchemaVersion) {
      console.log('🆕 First-time user detected. Initializing at current schema version.');
    ...
    console.log(`🔄 Migrating user data from v${userSchemaVersion} to v${SCHEMA_VERSION}`);

    // Find all migrations that need to run
    const migrationsToRun = MIGRATIONS.filter(migration => {
      const fromOK = compareVersions(userSchemaVersion, migration.fromVersion) <= 0;
...
/**
 * Force a clean slate (nuclear option for corrupted data)
 * Only use this if a user's data is completely broken
 */
export const resetToCleanSlate = () => {
  const confirmation = window.confirm(
    '⚠️ EMERGENCY RESET ⚠️\n\n' +
    'This will clear ALL saved data and start fresh.\n' +
    'Use this ONLY if your data is corrupted.\n\n' +
  ...
  try {
    ...
    console.log('✅ Clean slate initialized. Please reload the page.');
    return true;
  } catch (error) {
...
export const getVersionInfo = () => {
  return {
    ...
    userAppVersion: localStorage.getItem(VERSION_STORAGE_KEY) || 'not set',
    userSchemaVersion: localStorage.getItem(SCHEMA_STORAGE_KEY) || 'not set',
