...
export const getProviderState = (providerId) => {
  return connectionState[providerId] || { connected: false, user: null, token: null };
};
...
export const connectDropbox = async () => {
  if (!DROPBOX_CONFIG.clientId) {
    throw new Error('Dropbox App Key not configured. Set VITE_DROPBOX_APP_KEY in .env');
  }
...
export const saveToDropbox = async (data, filename, passphrase = null) => {
  ...
  // Encrypt the data
  const { encryptedPackage, keyExport } = await encryptForCloud(data, passphrase);
...
export const restoreFromDropbox = async (filePath, passphraseOrKey = null) => {
  ...
  if (isEncryptedBackup(encryptedPackage)) {
    ...
    return await decryptFromCloud(encryptedPackage, key, encryptedPackage.hasPassphrase);
  }
...
export const connectOneDrive = async () => {
  if (!ONEDRIVE_CONFIG.clientId) {
    throw new Error('OneDrive Client ID not configured. Set VITE_ONEDRIVE_CLIENT_ID in .env');
  }
  ...
  const popup = window.open(authUrl.toString(), 'onedrive_auth', 'width=600,height=700');
...
const exchangeOneDriveCode = async (code) => {
  ...
  connectionState.onedrive = {
    connected: true,
    token,
    user: { email: user.mail || user.userPrincipalName, name: user.displayName }
  };
 ...
 */
export const restoreBackup = async (providerId, fileIdOrPath, passphraseOrKey = null) => {
  switch (providerId) {
    case 'dropbox':
