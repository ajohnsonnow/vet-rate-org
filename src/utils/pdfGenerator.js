...
const CLAIMS_TOOLKIT = {
  fourPillars: [
    {
      name: 'Frequency',
      description: 'How often does it happen? (e.g., "My migraines occur 3 times a week.")',
    },
    {
      name: 'Severity',
      description: 'How bad is it? (e.g., "I must lie down in a dark room for 4 hours.")',
    },
    {
      name: 'Duration',
      description: 'How long has it lasted? (e.g., "This has been getting worse since 2018.")',
    },
    {
      name: 'Impact',
      description:
  ...
  doctorGuidance: {
    ...
    introduction: 'If obtaining a private Nexus Letter or Disability Benefits Questionnaire (DBQ), ensure your provider addresses these critical elements:',
    requirements: [
      {
        name: 'Probability Standard',
        description: 'The medical opinion must state the nexus is "at least as likely as not" (50% or greater probability) that your current condition is related to your military service, a service-connected injury, or service-connected treatment.',
      },
      {
        name: 'Functional Impact',
        description: 'The provider must document specific functional limitations (e.g., range of motion, ability to stand/walk, cognitive impairment, frequency/severity of flare-ups) that align with the VA rating criteria under 38 CFR Part 4.',
      },
      {
        name: 'Review of Records',
        description: 'The doctor should reference your service treatment records (STRs), VA medical records, and any relevant civilian medical history to establish continuity of symptoms from service to present.',
      },
      {
        name: 'Rationale',
...
export const generatePDF = (result, searchTerm) => {
  try {
    ...
    doc.setProperties({
      title: `SupplyLocker.org - ${result.conditionName} (DC ${result.diagnosticCode})`,
      subject: 'VA Disability Rating Information',
      author: 'SupplyLocker.org',
      keywords: 'VA, disability, rating, veterans, 38 CFR Part 4',
      creator: 'SupplyLocker.org'
    });
    ...
    const addText = (text, size = 10, indent = 0) => {
      ...
      yPosition += lines.length * (size * 0.5) + 3;
      doc.setTextColor(0, 0, 0);
    };

    // Helper function for info box
    const addInfoBox = (text, bgColor = colors.lightGray, borderColor = colors.mediumGray) => {
      checkPageBreak(25);
      ...
      doc.roundedRect(margin, yPosition - 3, contentWidth, boxHeight, 1.5, 1.5, 'FD');
    ...
    try {
      ...
      logoImg.src = '/images/supply-locker-org-logo.png';
    ...
    doc.setFontSize(9);
    doc.text('38 CFR Part 4 - Schedule for Rating Disabilities', pageWidth / 2, 38, {
      align: 'center',
    });
    ...
    doc.text(`Generated: ${new Date().toLocaleString('en-US', { 
      dateStyle: 'long', 
      timeStyle: 'short' 
    })}`, margin + 3, yPosition + 5);
    doc.text(`Search Query: "${searchTerm}"`, margin + 3, yPosition + 10);
    doc.setTextColor(0, 0, 0);
    yPosition += 22;
    ...
    doc.text(result.conditionName, margin + 5, yPosition);
    doc.setTextColor(0, 0, 0);
    yPosition += 8;
    ...
    doc.setFont(undefined, 'normal');
    doc.setTextColor(...colors.mediumGray);
    doc.text(`Diagnostic Code: ${result.diagnosticCode}  |  Rating Schedule: ${result.ratingSchedule}`, margin + 5, yPosition);
    doc.setTextColor(0, 0, 0);
    yPosition += 5;
    ...
    if (result.ecfrUrl) {
      ...
      doc.text(`eCFR Reference: ${result.ecfrUrl}`, margin + 5, yPosition);
      doc.setTextColor(0, 0, 0);
    }
    ...
    if (result.ratingCriteria && (result.ratingCriteria.ratings || result.ratingCriteria.ratedUnder)) {
      checkPageBreak(60);
      ...
      doc.text(`RATING TYPE: ${result.ratingCriteria.type.replace(/-/g, ' ').toUpperCase()}`, margin, yPosition);
      yPosition += 6;
      doc.setFont(undefined, 'normal');
      doc.setTextColor(0, 0, 0);
      ...
      if (result.ratingCriteria.ratings && Object.keys(result.ratingCriteria.ratings).length > 0) {
        checkPageBreak(40);
        ...
        doc.setFontSize(10);
        doc.setFont(undefined, 'bold');
        doc.text('Rating %', margin + 3, yPosition + 5);
        doc.text('Criteria', margin + 35, yPosition + 5);
        yPosition += 10;
        doc.setFont(undefined, 'normal');
        doc.setTextColor(0, 0, 0);

        // Sort ratings in descending order
        const sortedRatings = Object.entries(result.ratingCriteria.ratings)
        ...
        sortedRatings.forEach(([percentage, criteria], index) => {
          ...
          if (index % 2 === 0) {
            ...
            doc.rect(margin, yPosition - 3, contentWidth, rowHeight + 3, 'F');
          }

          // Percentage badge
          doc.setFillColor(253, 224, 71);
          doc.roundedRect(margin + 2, yPosition - 2, 25, 7, 1, 1, 'F');
          doc.setFontSize(11);
          doc.setFont(undefined, 'bold');
          doc.setTextColor(17, 63, 123);
          doc.text(`${percentage}%`, margin + 5, yPosition + 3);
          
          // Criteria text
          doc.setFont(undefined, 'normal');
          doc.setFontSize(9);
          doc.setTextColor(55, 65, 81);
          doc.text(criteriaLines, margin + 35, yPosition + 3);
      ...
      if (result.ratingCriteria.notes) {
        ...
        doc.roundedRect(margin, yPosition - 3, contentWidth, totalNotesHeight, 1, 1, 'F');
        doc.setFontSize(9);
        doc.setFont(undefined, 'bold');
        doc.setTextColor(5, 122, 85);
        doc.text('IMPORTANT NOTES:', margin + 3, yPosition);
        doc.setFont(undefined, 'normal');
        yPosition += 6;
        ...
        allNoteLines.forEach((noteLines) => {
          checkPageBreak(15);
          doc.setTextColor(75, 85, 99);
          doc.text(noteLines, margin + 3, yPosition);
    ...
    bulletPoints.forEach((point, index) => {
      ...
      doc.setTextColor(0, 0, 0);
      yPosition += Math.max(wrappedText.length * 5, 8);
    });

    yPosition += 5;

    // Related Secondary Conditions
    if (result.relatedSecondaryConditions && result.relatedSecondaryConditions.length > 0) {
      checkPageBreak(50);
      ...
      result.relatedSecondaryConditions.forEach((condition, index) => {
        ...
        doc.text(displayText, margin + 5, yPosition);
        doc.setTextColor(0, 0, 0);
        yPosition += 6;
      });
      ...
      doc.roundedRect(margin, yPosition - 3, contentWidth, 20, 1, 1, 'F');
      doc.setFontSize(8);
      doc.setFont(undefined, 'bold');
      doc.setTextColor(120, 53, 15);
      doc.text('IMPORTANT: Medical Nexus Required', margin + 3, yPosition);
      doc.setFont(undefined, 'normal');
      yPosition += 4;
      const disclaimerText = 'Veterans must provide medical evidence establishing a nexus (medical link) between their service-connected condition and any secondary condition. This requires a medical opinion stating it is "at least as likely as not" (50% or greater probability) that the secondary condition is related to the service-connected disability.';
      const disclaimerLines = doc.splitTextToSize(disclaimerText, contentWidth - 6);
      doc.text(disclaimerLines, margin + 3, yPosition);
      yPosition += disclaimerLines.length * 3.5 + 5;
      doc.setTextColor(0, 0, 0);
      doc.setFontSize(10);
      yPosition += 5;
    }
    ...
    doc.roundedRect(margin - 3, yPosition - 5, contentWidth + 6, 8, 1, 1, 'F');
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(12);
    doc.setFont(undefined, 'bold');
    doc.text('EMERGENCY & CRISIS SUPPORT', margin, yPosition);
    doc.setTextColor(0, 0, 0);
    yPosition += 12;
    ...
    VA_RESOURCES.emergency.forEach((resource) => {
      ...
      doc.roundedRect(margin, yPosition - 4, contentWidth, 12, 1, 1, 'F');
      doc.setFontSize(10);
      doc.setFont(undefined, 'bold');
      doc.setTextColor(...colors.warningRed);
      doc.text(resource.label, margin + 3, yPosition);
      yPosition += 5;
      doc.setFont(undefined, 'normal');
      doc.setFontSize(11);
      doc.setTextColor(...colors.darkGray);
      doc.text(resource.value, margin + 3, yPosition);
      doc.setTextColor(0, 0, 0);
      yPosition += 9;
    });
    ...
    VA_RESOURCES.benefits.forEach((benefit, index) => {
      ...
      if (index % 3 === 0) {
        ...
        doc.roundedRect(margin - 2, yPosition - 4, contentWidth + 4, itemsRemaining * 14 - 2, 1.5, 1.5, 'F');
      }
      
      doc.setFontSize(10);
      doc.setFont(undefined, 'bold');
      doc.setTextColor(...colors.darkGray);
      doc.text(`> ${benefit.label}`, margin + 2, yPosition);
      yPosition += 5;
      doc.setFont(undefined, 'normal');
      doc.setFontSize(8);
      doc.setTextColor(...colors.accentBlue);
    ...
    doc.roundedRect(margin, yPosition - 4, contentWidth, 15, 1.5, 1.5, 'FD');
    doc.setFontSize(10);
    doc.setFont(undefined, 'bold');
    doc.setTextColor(...colors.darkGray);
    doc.text(VA_RESOURCES.ratingSchedule.label, margin + 3, yPosition);
    yPosition += 6;
    doc.setFont(undefined, 'normal');
    doc.setFontSize(8);
    doc.setTextColor(...colors.accentBlue);
    doc.text(VA_RESOURCES.ratingSchedule.url, margin + 3, yPosition);
    doc.setTextColor(0, 0, 0);
    yPosition += 12;
    ...
    GLOSSARY_TERMS.slice(0, termsPerPage).forEach((item, index) => {
      ...
      doc.setFont(undefined, 'bold');
      doc.setTextColor(...colors.vaBlue);
      doc.text(`> ${item.term}`, margin + 3, yPosition);
      yPosition += 6;
      doc.setFont(undefined, 'normal');
      doc.setFontSize(9);
      doc.setTextColor(...colors.darkGray);
    ...
    CLAIMS_TOOLKIT.fourPillars.forEach((pillar, index) => {
      ...
      const descLines = doc.splitTextToSize(pillar.description, contentWidth - 12);
      const boxHeight = descLines.length * 4.5 + 10;
      doc.roundedRect(margin, yPosition - 4, contentWidth, boxHeight, 1.5, 1.5, 'FD');
      
      doc.setFontSize(11);
      doc.setFont(undefined, 'bold');
      doc.setTextColor(...colors.vaBlue);
      doc.text(`${index + 1}. ${pillar.name}`, margin + 4, yPosition);
      yPosition += 6;
      doc.setFont(undefined, 'normal');
      doc.setFontSize(9);
      doc.setTextColor(...colors.darkGray);
      doc.text(descLines, margin + 6, yPosition);
      doc.setTextColor(0, 0, 0);
    ...
    CLAIMS_TOOLKIT.goldenRules.forEach((rule, index) => {
      ...
      doc.roundedRect(margin, yPosition - 4, contentWidth, 9, 1, 1, 'F');
      ...
      doc.text(ruleText, margin + 8, yPosition);
      doc.setTextColor(0, 0, 0);
      yPosition += Math.max(ruleText.length * 5, 9);
    });
    ...
    CLAIMS_TOOLKIT.doctorGuidance.requirements.forEach((req, index) => {
      ...
      doc.setFont(undefined, 'bold');
      doc.setTextColor(...colors.vaBlue);
      doc.text(`${index + 1}. ${req.name}`, margin + 4, yPosition);
      yPosition += 6;
      doc.setFont(undefined, 'normal');
      doc.setFontSize(9);
      doc.setTextColor(...colors.darkGray);
    ...
    const ratingPrinciples = [
      {
        ...
        description: 'Ratings represent the average impairment in earning capacity from disabilities in civil occupations. Medical examinations must emphasize the limitation of activity imposed by the disabling condition.',
      },
      {
        name: '§4.2 - Interpretation of Examination Reports',
        description: 'Each disability must be considered from the point of view of the veteran working or seeking work. If a diagnosis is not supported by findings or lacks detail, the rating board must return it as inadequate.',
      },
      {
        name: '§4.3 - Resolution of Reasonable Doubt',
        description: 'When reasonable doubt arises regarding the degree of disability, that doubt will be resolved in favor of the claimant. This is the "benefit of the doubt" principle.',
      },
      {
        name: '§4.7 - Higher of Two Evaluations',
        description: 'Where there is a question as to which of two evaluations shall be applied, the higher evaluation will be assigned if the disability picture more nearly approximates the criteria required for that rating.',
      },
      {
        name: '§4.10 - Functional Impairment',
        description: 'Evaluations are based on lack of usefulness of body parts or systems, especially in self-support. Medical examiners must furnish full descriptions of the effects of disability upon ordinary activity. A person may be too disabled to work even if fairly comfortable at home.',
      },
      {
        name: '§4.14 - Avoidance of Pyramiding',
        description: 'The evaluation of the same disability under various diagnoses is to be avoided. The same manifestation cannot be rated under different diagnoses.',
      },
      {
        name: '§4.16 - Total Disability Based on Unemployability (TDIU)',
        description: 'Total disability ratings may be assigned when the veteran is unable to secure or follow substantially gainful occupation due to service-connected disabilities. Generally requires one disability at 60%+ or multiple disabilities with one at 40%+ combining to 70%+.',
      },
    ];
    ...
    ratingPrinciples.forEach((principle, index) => {
      ...
      doc.setFont(undefined, 'bold');
      doc.setTextColor(...colors.vaBlue);
      doc.text(`> ${principle.name}`, margin + 3, yPosition);
      yPosition += 6;
      doc.setFont(undefined, 'normal');
      doc.setFontSize(9);
      doc.setTextColor(...colors.darkGray);
    ...
    doc.setLineWidth(0.5);
    doc.roundedRect(margin - 2, yPosition - 5, contentWidth + 4, 50, 2, 2, 'FD');

    doc.setFontSize(11);
    doc.setFont(undefined, 'bold');
    doc.setTextColor(147, 51, 234);
    doc.text('IMPORTANT LEGAL DISCLOSURES', margin + 2, yPosition);
    yPosition += 8;

    doc.setFont(undefined, 'bold');
    doc.setFontSize(9);
    doc.setTextColor(...colors.darkGray);
    doc.text('DATA PRIVACY & SECURITY:', margin + 3, yPosition);
    yPosition += 5;
    doc.setFont(undefined, 'normal');
    doc.setFontSize(8);
    ...
    doc.text(privacy, margin + 3, yPosition);
    yPosition += privacy.length * 4 + 5;

    doc.setFont(undefined, 'bold');
    doc.setFontSize(9);
    doc.text('LEGAL & MEDICAL NOTICE:', margin + 3, yPosition);
    yPosition += 5;
    doc.setFont(undefined, 'normal');
    doc.setFontSize(8);
    ...
    doc.text(legal, margin + 3, yPosition);
    
    doc.setTextColor(0, 0, 0);

    // Support SupplyLocker.org at bottom of last page
    yPosition += legal.length * 4 + 15;
    
    // Prominent background box
    doc.setFillColor(255, 251, 235);
    doc.setDrawColor(...colors.vaGold);
    doc.setLineWidth(1);
    doc.roundedRect(margin - 5, yPosition - 8, contentWidth + 10, 40, 3, 3, 'FD');

    // Centered heading
    doc.setFontSize(14);
    doc.setFont(undefined, 'bold');
    doc.setTextColor(...colors.vaBlue);
    doc.text('Support SupplyLocker.org', pageWidth / 2, yPosition, { align: 'center' });
    yPosition += 10;
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.setFontSize(7);
      doc.text(
        'Generated by SupplyLocker.org - VA Disability Rating Information System | www.SupplyLocker.org',
        pageWidth / 2,
        pageHeight - 5,
        { align: 'center' }
      );
