...
const PII_PATTERNS = {
  ...
  address: /\b\d+\s+[A-Za-z0-9\s]+\b(Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Lane|Ln|Drive|Dr|Court|Ct|Circle|Cir|Way|Plaza|Place|Pl)\b/gi
};
...
export const scrubPII = (text, options = {}) => {
  ...
  customPatterns.forEach(({ pattern, label }) => {
    ...
    if (matches) {
      ...
      scrubbed = scrubbed.replace(pattern, `[REDACTED_${label?.toUpperCase() || 'INFO'}]`);
    }
  });
...
export const analyzePII = (text) => {
  ...
  return {
    ...
    riskLevel: score === 0 ? 'none' : score < 5 ? 'low' : score < 10 ? 'medium' : 'high'
  };
};
