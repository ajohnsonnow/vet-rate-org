...
export function findSecondaryClaims(userDisabilities) {
    ...
    const nsaidSecondaries = [
      {
        ...
        nexusTheory: 'Chronic use of nonsteroidal anti-inflammatory drugs (NSAIDs) to manage service-connected pain disrupts the gastric mucosal barrier by inhibiting prostaglandin synthesis. This permits gastric acid to damage the esophageal lining and reduces lower esophageal sphincter pressure, directly causing GERD. The medically necessary NSAID therapy for service-connected pain management proximately causes this secondary gastrointestinal condition.',
        probability: 'High'
      },
      {
        condition: 'Peptic Ulcer Disease',
        mechanism: 'Medication',
        nexusTheory: 'Chronic NSAID use required for managing service-connected pain directly causes peptic ulcer disease through disruption of gastric mucosal defenses. NSAIDs inhibit prostaglandin synthesis, reducing mucus production and mucosal blood flow, permitting gastric acid to damage the gastric and duodenal mucosa.',
        probability: 'High'
      },
      {
        condition: 'Chronic Kidney Disease (CKD)',
        mechanism: 'Medication',
        nexusTheory: 'Long-term NSAID use for service-connected pain management causes chronic kidney disease through multiple mechanisms: reduced renal blood flow by inhibiting vasodilatory prostaglandins, acute tubular necrosis, and chronic tubulointerstitial damage. The cumulative renal toxicity from medically necessary NSAID therapy proximately causes progressive kidney disease.',
        probability: 'Medium'
      }
    ];
  ...
  normalizedDisabilities.forEach(({ original, slug }) => {
    if (isLowerBodyJoint(slug)) {
      ...
      if (contralateral) {
        ...
        const hasContralateral = normalizedDisabilities.some(d => d.slug === contralateral);
      ...
      if (slug !== 'lumbar_spine') {
        const hasLumbar = normalizedDisabilities.some(d => d.slug === 'lumbar_spine');
  ...
  const hasMigraines = userDisabilities.some(d => 
    d.toLowerCase().includes('migraine') || d.toLowerCase().includes('headache')
  );
  
  if (hasTinnitus && !hasMigraines && !alreadySuggested.has('Migraine Headaches')) {
    suggestions.push({
  ...
  suggestions.sort((a, b) => {
    const priorityDiff = priorityMap[b.probability] - priorityMap[a.probability];
    ...
    return (mechanismPriority[b.mechanism] || 0) - (mechanismPriority[a.mechanism] || 0);
  });
...
export function getSecondaryClaimsSummary(suggestions) {
  return {
    ...
    mediumProbability: suggestions.filter(s => s.probability === 'Medium').length,
    byMechanism: {
      direct: suggestions.filter(s => s.mechanism === 'Direct').length,
      medication: suggestions.filter(s => s.mechanism === 'Medication').length,
      alteredGait: suggestions.filter(s => s.mechanism === 'Altered Gait').length
    }
  };
}
...
export function filterByProbability(suggestions, minProbability = 'Medium') {
  ...
  return suggestions.filter(s => s.probability === 'High' || s.probability === 'Medium');
}
