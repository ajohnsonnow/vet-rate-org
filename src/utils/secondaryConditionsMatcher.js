...
function normalizeMentalHealthCondition(normalized) {
  ...
  if (
    ...
    normalized.includes("generalized anxiety")
  ) {
    return "anxiety";
  }
  if (
    normalized.includes("depression") ||
    normalized.includes("depressive disorder") ||
    normalized.includes("major depression")
  ) {
    return "depression";
  }
  return null;
...
function normalizeAuditoryCondition(normalized) {
  if (
    ...
    normalized.includes("ear ringing")
  ) {
    return "tinnitus";
  }
  if (
    normalized.includes("hearing loss") ||
    normalized.includes("hearing impairment") ||
    normalized.includes("deafness")
  ) {
    return "hearing_loss";
  }
  return null;
...
function normalizeSleepCondition(normalized) {
  if (
    ...
    normalized.includes("obstructive sleep") ||
    normalized.includes("apnea") ||
    (normalized.includes("sleep disorder") && normalized.includes("breathing"))
  ) {
    return "sleep_apnea";
  }
  return null;
...
function normalizePainCondition(normalized) {
  if (
    (normalized.includes("pain") ||
      ...
      normalized.includes("degenerative") ||
      normalized.includes("osteoarthritis")) &&
