...
export function isStaleData(disability) {
  ...
  try {
    ...
    const daysDifference = Math.floor(
      (currentDate - verifiedDate) / (1000 * 60 * 60 * 24),
    );
    ...
    console.warn("[staleDataDetection] Failed to parse lastVerifiedDate:", error);
    return true;
  }
}
...
export function generateReportOutdatedLink(disability) {
  const subject = encodeURIComponent(
    `Report Outdated Rating Criteria - ${disability.conditionName}`,
  );
