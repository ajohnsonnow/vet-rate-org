...
export const PHASES = {
  ...
  DOCUMENTATION: { id: 'documentation', title: 'Phase 4: Documentation', order: 4 },
...
const MILESTONES = [
    ...
    checkCompleted: (data, altData) => {
      ...
      try {
        if (data) {
          ...
          if (parsed.filed || parsed.status === 'filed' || parsed.date) return true;
        }
      } catch {}
  ...
  // ========== PHASE 2: DISCOVERY ==========
  {
    ...
    description: 'Medical conditions identified via Blue Button or manual entry',
    ...
    checkCompleted: (data, altData) => {
      ...
      try {
        ...
        return Array.isArray(claims) ? claims.length > 0 : Object.keys(claims || {}).length > 0;
      } catch {
        return false;
      }
    },
  ...
  },
  {
    ...
    description: 'Service treatment records and private medical records gathered',
  ...
  // ========== PHASE 3: EVIDENCE BUILDING ==========
  {
    ...
    description: 'In-service event/injury identified via C-File analysis or records',
  ...
  },
  {
    ...
    description: 'Supporting statements from family, friends, or fellow service members',
    ...
    checkCompleted: (data, altData) => {
      ...
      try {
        ...
        return !!(parsed?.count > 0 || parsed?.statements?.length > 0 || parsed?.gathered);
      } catch {
        return false;
      }
    },
...
const setManualProgress = (milestoneId, completed, notes = '') => {
  ...
  window.dispatchEvent(new CustomEvent('claimProgressUpdate', { detail: { milestoneId, completed } }));
};
...
const checkMilestoneCompletion = (milestone) => {
  ...
  const altData = milestone.alternateKeys?.map(key => localStorage.getItem(key)).filter(Boolean) || [];
...
const calculateReadyToFile = (completedMilestones) => {
  ...
  const missingCritical = criticalMilestones.filter(cm =>
    !completedMilestones.some(m => m.id === cm.id)
  );
  
  const readyToFile = missingCritical.length === 0;
  const criticalProgress = Math.round((completedCritical.length / criticalMilestones.length) * 100);
...
export default function useClaimProgress() {
  const [progress, setProgress] = useState({
    ...
    readyToFile: { ready: false, criticalCompleted: 0, criticalTotal: 0, criticalProgress: 0, missingCritical: [] },
    byPhase: {}
  });
  ...
  const checkProgress = useCallback(() => {
    ...
    MILESTONES.forEach(milestone => {
      const { completed: isCompleted, source } = checkMilestoneCompletion(milestone);
      if (isCompleted) {
    ...
    Object.values(PHASES).forEach(phase => {
      ...
      byPhase[phase.id] = {
        ...
        percentage: Math.round((phaseCompleted.length / phaseMilestones.length) * 100)
      };
    });
...
export const getMilestoneStatus = (milestoneId) => {
  ...
  if (!milestone) return { completed: false, source: null };

  return checkMilestoneCompletion(milestone);
};

/**
 * Mark a milestone as completed (for manual workflow or tools without standard storage)
 */
export const markMilestoneCompleted = (milestoneId, notes = '') => {
  const milestone = MILESTONES.find(m => m.id === milestoneId);
  if (!milestone) return false;
  
  if (milestone.manualAllowed) {
    setManualProgress(milestoneId, true, notes);
    return true;
  }

  // For legacy boolean flags
  if (['service_connection', 'secondary', 'itf_filed', 'medical_records', 'buddy_statements', 'exam_prep'].includes(milestoneId)) {
    localStorage.setItem(milestone.storageKey, 'true');
    window.dispatchEvent(new CustomEvent('claimProgressUpdate', { detail: { milestoneId, completed: true } }));
    return true;
  }
...
/**
 * Unmark a milestone (for corrections)
 */
export const unmarkMilestone = (milestoneId) => {
  const manualProgress = getManualProgress();
  if (manualProgress[milestoneId]) {
    delete manualProgress[milestoneId];
    localStorage.setItem(MANUAL_PROGRESS_KEY, JSON.stringify(manualProgress));
    window.dispatchEvent(new CustomEvent('claimProgressUpdate', { detail: { milestoneId, completed: false } }));
    return true;
  }
  return false;
};
...
/**
 * Get all milestones with current status
 */
 ...
 */
export const getMilestonesByPhaseExport = getMilestonesByPhase;

/**
 * Get Ready-to-File checklist
 */
export const getReadyToFileStatus = () => {
  const completed = MILESTONES.filter(m => checkMilestoneCompletion(m).completed);
  return calculateReadyToFile(completed);
};
