    ...
    check: (data) => {
      ...
      if (data.statements && data.symptomLogs) {
        Object.values(data.statements).forEach((statement) => {
          ...
          if (
            statementLower.includes("daily") ||
            statementLower.includes("constant") ||
            statementLower.includes("weekly") ||
            statementLower.includes("monthly")
          ) {
            ...
            if (frequencyInStatement && data.symptomLogs) {
              ...
              const loggedFrequency = calculateLoggedFrequency(
                data.symptomLogs,
                statement.condition,
              );
    ...
    check: (data) => {
      ...
      if (data.statements && data.ratings) {
        Object.values(data.statements).forEach((statement) => {
          ...
          if (
            statementLower.includes("completely unable") ||
            statementLower.includes("cannot work")
          ) {
        ...
        Object.values(data.forms).forEach((form) => {
          if (form.condition) {
            ...
            if (matchingStatement) {
              ...
              const stmtLaterality = extractLaterality(
                matchingStatement.content,
              );
    ...
    check: (data) => {
      ...
      if (data.profile && data.statements) {
        ...
        Object.values(data.statements).forEach((statement) => {
          ...
          if (
            content.includes("since service") ||
            content.includes("during deployment")
          ) {
...
function extractFrequency(text) {
  ...
  if (text.includes("weekly") || text.includes("few times a week"))
    return "weekly";
  if (text.includes("monthly") || text.includes("occasionally"))
    return "monthly";
  return null;
}
...
function calculateLoggedFrequency(logs, condition) {
  ...
  const totalDays =
    (new Date(sortedLogs[sortedLogs.length - 1].date) -
