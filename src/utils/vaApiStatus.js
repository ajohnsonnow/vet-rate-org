...
export const VA_API_MAPPING = {
  ...
  'appeals': ['Appeals Status API', 'Appealable Issues API', 'Decision Reviews API', 'Legacy Appeals API'],
  ...
  // Health APIs (FHIR)
  'health': ['Clinical Health API (FHIR)', 'Patient Health API (FHIR)', 'Health Care Costs and Coverage API'],
...
export const STATUS_LEVELS = {
  ...
  degraded_performance: { level: 1, label: 'Degraded Performance', color: 'yellow', icon: '⚠️' },
  partial_outage: { level: 2, label: 'Partial Outage', color: 'orange', icon: '🔶' },
  major_outage: { level: 3, label: 'Major Outage', color: 'red', icon: '🔴' },
  under_maintenance: { level: 4, label: 'Under Maintenance', color: 'blue', icon: '🔧' },
};
...
export async function fetchVaApiStatus() {
  ...
  if (statusCache && lastFetchTime && (Date.now() - lastFetchTime < CACHE_DURATION_MS)) {
    return statusCache;
  }
  ...
  try {
    ...
    const scheduled = scheduledRes.ok ? await scheduledRes.json() : { scheduled_maintenances: [] };
...
function processStatusData(summary, incidents, scheduled) {
  ...
  for (const component of components) {
    ...
    if (component.components && component.components.length > 0) {
      ...
      componentStatus[component.name] = {
        ...
        description: component.description,
        updatedAt: component.updated_at,
        group: components.find(c => c.id === component.group_id)?.name || null,
      };
    }
  }
  ...
  const scheduledMaintenance = (scheduled.scheduled_maintenances || []).map(maint => ({
    id: maint.id,
    name: maint.name,
    status: maint.status,
    impact: maint.impact,
...
export function getFeatureStatus(feature, statusData) {
  ...
  for (const apiName of apiNames) {
    ...
    if (componentStatus) {
      const statusInfo = STATUS_LEVELS[componentStatus.status] || STATUS_LEVELS.operational;
...
export function hasAnyIssues(statusData) {
  ...
  return statusData.overallStatus !== 'operational' || 
...
export function getStatusSummary(statusData) {
  ...
  const activeMaintenance = statusData.scheduledMaintenance.filter(m => m.status === 'in_progress').length;
  const upcomingMaintenance = statusData.scheduledMaintenance.filter(m => m.status === 'scheduled').length;
  ...
  if (statusData.overallStatus === 'partial_outage' || statusData.overallStatus === 'degraded_performance') {
    return {
      hasIssues: true,
