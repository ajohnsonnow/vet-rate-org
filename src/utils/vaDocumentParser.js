...
const CONDITION_PATTERNS = {
  ...
  CONDITION_WITH_PERCENT: /([A-Za-z\s\-,()]+?)(?:\s*(?:\.{2,}|–|-)\s*)(\d{1,3})\s*percent/gi,
  
  // Service connection granted/denied
  SERVICE_CONNECTED: /(?:service[- ]?connection|sc)\s+(?:is\s+)?(?:granted|established|allowed)/gi,
  SERVICE_DENIED: /(?:service[- ]?connection|sc)\s+(?:is\s+)?(?:denied|not\s+established|not\s+warranted)/gi,
  
  // Effective date pattern
  EFFECTIVE_DATE: /effective\s*(?:date)?[:\s]*(\w+\s+\d{1,2},?\s+\d{4}|\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4})/gi,
  
  // Diagnostic code pattern
  DIAGNOSTIC_CODE: /(?:diagnostic\s*code|DC)[:\s#]*(\d{4})/gi,
  
  // Combined rating
  COMBINED_RATING: /(?:combined|overall|total)\s*(?:service[- ]?connected)?\s*(?:evaluation|rating|disability)[:\s]*(\d{1,3})\s*percent/gi,
};
...
export function parseDecisionLetter(text) {
  ...
  try {
    ...
    const nameMatch = text.match(/(?:Dear\s+(?:Mr\.|Mrs\.|Ms\.)\s+)?([A-Z][A-Za-z'-]+(?:,?\s+[A-Z][A-Za-z'-]+)*)/);
    if (nameMatch) result.veteranName = nameMatch[1].trim();
    
    const claimMatch = text.match(/(?:claim|file)\s*(?:number|#)[:\s]*([A-Z0-9\-]+)/i);
    ...
    if (decisionStart !== -1) {
      const decisionEnd = evidenceStart !== -1 ? evidenceStart : 
                          reasonsStart !== -1 ? reasonsStart : 
                          Math.min(decisionStart + 3000, text.length);
      result.sections.decision = text.substring(decisionStart, decisionEnd).trim();
      
      // Parse conditions from decision section
      const conditionMatches = result.sections.decision.matchAll(CONDITION_PATTERNS.CONDITION_WITH_PERCENT);
      for (const match of conditionMatches) {
        const conditionName = match[1].trim();
        const percent = parseInt(match[2]);
        
        // Extract diagnostic code if present nearby
        const codeMatch = result.sections.decision.match(new RegExp(`${conditionName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}[^]*?DC[:\\s#]*(\\d{4})`, 'i'));
    ...
    if (evidenceStart !== -1) {
      const evidenceEnd = reasonsStart !== -1 ? reasonsStart : 
                          appealStart !== -1 ? appealStart : 
                          Math.min(evidenceStart + 5000, text.length);
      result.sections.evidence = text.substring(evidenceStart, evidenceEnd).trim();
      ...
      for (const line of evidenceLines) {
        ...
        const docMatch = line.match(/(?:•|\d+\.|-)?\s*(.+?(?:record|report|statement|exam|letter|rating|decision|medical|treatment|VA|private|physician|doctor)[^.]*)/i);
    ...
    if (reasonsStart !== -1) {
      const reasonsEnd = appealStart !== -1 ? appealStart : Math.min(reasonsStart + 5000, text.length);
    ...
    for (const match of effectiveDates) {
      ...
      for (const cond of result.conditions) {
        if (context.toLowerCase().includes(cond.name.toLowerCase().substring(0, 20))) {
    ...
    for (const match of deniedMatches) {
      ...
      const condMatch = context.match(/([A-Za-z\s\-,()]+?)(?:\s+(?:is|was))?\s+(?:denied|not\s+established)/i);
      if (condMatch) {
    ...
    if (result.conditions.length === 0) {
      result.extractionNotes.push('No conditions could be extracted. Document may be a notification letter or different format.');
    }
    if (result.evidenceConsidered.length === 0) {
      result.extractionNotes.push('No evidence items found. Check if this is a summary letter.');
    }
...
export function parseDBQReport(text) {
  ...
  try {
    // === EXTRACT EXAM DATE ===
    const dateMatch = text.match(/(?:exam(?:ination)?\s*date|date\s*of\s*exam)[:\s]*(\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4})/i);
    if (dateMatch) result.examDate = dateMatch[1];

    // === EXTRACT EXAMINER ===
    const examinerMatch = text.match(/(?:examiner|physician|doctor)[:\s]*([A-Z][A-Za-z\s,\.]+(?:MD|DO|PA|NP)?)/i);
    ...
    if (diagnosisStart !== -1) {
      ...
      const diagMatches = diagSection.matchAll(/(?:\d+\.|•|-)?\s*([A-Za-z\s\-]+)(?:\s*\(?\s*(?:ICD[:\s]*)?([A-Z]\d{2}(?:\.\d+)?)\)?)?/gi);
      for (const match of diagMatches) {
        if (match[1] && match[1].length > 5 && !match[1].match(/^(?:the|and|or|with|for|from|this|that)\s*$/i)) {
    ...
    if (functionalStart !== -1) {
      result.sections.functionalImpact = text.substring(functionalStart, functionalStart + 1500);
      result.functionalImpact = result.sections.functionalImpact.substring(0, 500);
    }

    // === EXTRACT EXAMINER REMARKS ===
    const remarksStart = text.search(VA_SECTION_HEADERS.EXAMINER_REMARKS);
    if (remarksStart !== -1) {
      result.sections.examinerRemarks = text.substring(remarksStart, remarksStart + 2000);
    }
...
export function parseVADocument(text) {
  ...
  if (textSample.match(/rating\s*decision|service\s*connection|combined.*evaluation|percent.*disab/i)) {
    return parseDecisionLetter(text);
  }
  
  // DBQ indicators
  if (textSample.match(/disability\s*benefits\s*questionnaire|dbq|c&p\s*exam|compensation.*pension/i)) {
    return parseDBQReport(text);
  }
  
  // Code Sheet indicators
  if (textSample.match(/code\s*sheet|rating.*code.*sheet|diagnostic.*code.*\d{4}/i)) {
    return parseCodeSheet(text);
  }
  
  // BVA Decision
  if (textSample.match(/board\s*of\s*veterans|bva|findings\s*of\s*fact|conclusions\s*of\s*law/i)) {
  ...
  return {
    ...
    extractionNotes: ['Document type could not be determined. Raw text preserved for manual review.'],
  };
}
...
export function parseCodeSheet(text) {
  ...
  try {
    ...
    const dcPattern = /(\d{4})\s*[:\-]?\s*([A-Za-z\s\-,()]+?)\s*[:\-]?\s*(\d{1,3})%/g;
...
export function parseBVADecision(text) {
  ...
  try {
    ...
    const docketMatch = text.match(/(?:docket|citation)\s*(?:no\.?|number)?[:\s]*(\d{2}-\d{2}\s*\d{3}|\d{7})/i);
...
export function parseSOC(text) {
  ...
  try {
    ...
    if (issueStart !== -1) {
      ...
      const issueMatches = issueSection.matchAll(/(?:\d+\.|•|-)?\s*((?:Entitlement|Service\s*connection|Increased)[^.\n]+)/gi);
...
export function extractBigThree(text) {
  ...
  const bigThreePattern = /([A-Za-z\s\-,()]{5,50}?)(?:\s*(?:\.{2,}|–|-|:)\s*)(\d{1,3})\s*(?:percent|%)[^]*?effective\s*(?:date)?[:\s]*(\w+\s+\d{1,2},?\s+\d{4}|\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4})/gi;
