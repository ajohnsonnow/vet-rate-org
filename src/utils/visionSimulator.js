  ...
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      const isLikelyDocument =
        avgBrightness > 180 && lightPixels / pixelCount > 0.6;
...
function detectDD214Elements(text) {
  ...
  if (
    upperText.includes("DD FORM 214") ||
    upperText.includes("DD214") ||
    upperText.includes("CERTIFICATE OF RELEASE") ||
    upperText.includes("DISCHARGE FROM ACTIVE DUTY")
  ) {
  ...
  elements.decorations = decorationKeywords.filter((kw) =>
 ...
 */
function buildVisionSimulatorPrompt(
  structureInfo,
  ocrResult,
  dd214Elements,
  userQuery,
...
export class VisionSimulator {
  ...
  async quickDD214Check(imageBlob) {
    ...
    if (!structure.isLikelyDocument)
      return { isLikely: false, reason: "Not a document" };
    if (!structure.hasHighTextDensity)
      return { isLikely: false, reason: "Low text density" };
