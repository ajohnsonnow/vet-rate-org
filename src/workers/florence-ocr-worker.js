...
async function loadModel() {
  ...
  try {
    ...
    self.postMessage({ 
      ...
      progress: 10, 
      stage: 'model',
      message: 'Loading vision model (this may take 1-2 minutes on first load)...' 
    });
    ...
    if (err.message.includes('WebGPU')) {
      errorType = 'webgpu';
      errorMessage = 'WebGPU not supported. Please use Chrome/Edge 113+ on desktop.';
    } else if (err.message.includes('memory') || err.message.includes('OOM')) {
      errorType = 'memory';
      errorMessage = 'Insufficient GPU memory. Try closing other tabs or applications.';
    } else if (err.message.includes('network') || err.message.includes('fetch')) {
      errorType = 'network';
      errorMessage = 'Network error downloading model. Check internet connection.';
    }
